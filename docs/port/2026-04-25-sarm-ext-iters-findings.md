...
- Use iter-4 champ ckpt (type=sarm upstream, wrist-only).
- Merged ds = _filtered (leaky: overlap iter-4 train) + _2 (unseen).
- **Per-bucket**:
...
- **Root cause**: wrist-only lose scene ctx → bg motion trigger false progress on 0/4. Partial plateau drift = stage confusion past last-done breakpoint.
- **Fix**: 2-cam front+wrist (iter-5).

### iter-5 (plan)
- Ext SARM, 2-cam (front+wrist), merged ds, dual mode. 5000 steps batch16.
- Expect: stage_ne up (2-cam add ctx), lin_mad steady, maybe mean_mid down w/ partials.

## Brainstorm backlog (T6 knob sweep if iter-5 insufficient)
- `stage_loss_weight`: 3→5→10
- `rewind_probability`: 0.8 default → lower? Maybe cut noise on partials
- `drop_n_last_frames`: 1 → 3
- `n_obs_steps`: 8 → 12
- `frame_gap`: 5 → 3 (tighter window, more frames/sec)
- Force feats (proprio + wrench) as extra state channel
- τ-cap: clamp tau pred to [0, breakpoint_of_stage]
- Cross-attn fusion (front↔wrist) vs concat
- Aug: color jitter, random crop (train-time only)