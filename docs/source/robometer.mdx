...
Offline dataset scoring exposes both signals explicitly through the shared [offline scoring workflow](./reward_scoring). It does not read the compatibility-only `reward_output` field.
...
### Score a LeRobot Dataset

Install both dataset and ROBOMETER dependencies, then run:

```bash
pip install -e ".[dataset,robometer]"
...
The default sidecar, `reward_signals/robometer.parquet`, contains:

- `reward.robometer.progress`
- `reward.robometer.success_probability`

Both are dense float32 frame signals in `[0, 1]`. ROBOMETER's frame scorer uses the characterized fixed-prefix sampling rule, while the shared scoring workflow handles validation, atomic episode writes, resume, and deterministic merging.

For RA-BC weighting, select the namespaced progress signal explicitly:

```yaml
sample_weighting:
  type: rabc
  progress_path: /path/to/reward_signals/robometer.parquet
