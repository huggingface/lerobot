...
The default artifact, `reward_signals/robometer.parquet`, contains:

- `reward.robometer.progress`
- `reward.robometer.success_probability`

Both are dense float32 frame signals in `[0, 1]`. ROBOMETER's adapter uses the characterized fixed-prefix sampling rule, while the shared scoring runner handles validation, atomic episode commits, resume, and deterministic merging.
