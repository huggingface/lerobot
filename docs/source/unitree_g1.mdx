...
`--robot.sim_env.end_effector` picks the MuJoCo model: `dex1` for the parallel grippers (the default), `dex3` for the three-finger hands, or `dummy` for bare wrists. It decides which finger actuators the model has and which cameras the scene carries — the wrist cameras are mounted on the end effector, so `dummy` only has `head_camera`.

```bash
lerobot-teleoperate \
  --robot.type=unitree_g1 \
  --robot.is_simulation=true \
  --robot.sim_env.end_effector=dex3 \
  --teleop.type=unitree_g1 \
  --teleop.id=wbc_unitree \
  --robot.controller=GrootLocomotionController
```
