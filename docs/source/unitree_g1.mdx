...
<Tip>
Pinocchio must be installed from conda-forge (not pip) to include the CasADi bindings needed for arm IK.
</Tip>
...
<Tip>
Always use `python -m pip` instead of `pip` on the G1. The Orin's system `pip` points to Python 3.8, not the conda environment's Python 3.12.
</Tip>
...
You can run the teleoperation client from your laptop over Ethernet, over WiFi (experimental), or directly on the robot itself.
