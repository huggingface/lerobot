...
5. Metrics include `pc_success` (success rate), `avg_sum_reward`, and `avg_max_reward`. Every success rate is reported with its counts (`n_success` out of `n_episodes`) and a 95% Wilson interval in percent (`pc_success_ci95`), at task, suite and overall level. The interval covers sampling noise over episodes only; it says nothing about variation between training runs of the same recipe. Two `eval_info.json` files can be compared task by task with `lerobot-eval-compare`, which exits 1 when a task regressed beyond its interval.
