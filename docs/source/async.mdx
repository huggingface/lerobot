...
For released MolmoAct2 Hugging Face checkpoints, set
`--policy_type=molmoact2`, set `--pretrained_name_or_path` to the HF model
repo, and pass the required MolmoAct2 config values through
`--policy_config_overrides`, for example
