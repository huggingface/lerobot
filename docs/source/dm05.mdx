...
Training uses the standard LeRobot state/action statistics in `meta/stats.json`.

Relative-action training changes the arm targets to `action - state`, while excluded dimensions such as the
gripper remain absolute. The statistics are still the dataset's own, so the normalization scale is fitted to
absolute actions rather than to deltas. This costs some signal but trains; it is a general LeRobot issue with
`RelativeActionsProcessorStep`, shared with the other policies that use it, not a DM05-specific one.
