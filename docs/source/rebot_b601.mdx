...
This pulls in `motorbridge` (CAN motor control for both follower builds) and `motorbridge-smart-servo` (FashionStar UART servos for the reBot Arm 102 leader). MotorBridge uses the platform CAN runtime: SocketCAN on Linux, MacCAN PCBUSB on macOS, and PCAN-Basic on Windows.
...
The follower talks to its motors over a CAN bus. DM supports either its Damiao serial bridge (`--robot.can_adapter=damiao`) or MotorBridge's native CAN transport (`--robot.can_adapter=socketcan`). RS requires the native CAN transport because the Damiao serial transport is Damiao-only. The `socketcan` option name is retained for compatibility; MotorBridge selects the platform backend, so this path is not limited to Linux. The default is `damiao` for `motor_family=dm` and `socketcan` for `motor_family=rs`.
...
- RS uses MIT control and a force-limited impedance gripper. `gripper_torque_limit` defaults to `3.5` N·m while moving and `gripper_hold_torque_limit` defaults to `1.0` N·m while holding.
