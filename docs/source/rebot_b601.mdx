...
## B601-DM MIT gravity feedforward

When the B601-DM follower uses `control_mode="mit"`, LeRobot evaluates the
gravity torque from the latest measured pose for every arm-joint MIT command.
The gripper is not part of the supplied arm dynamics model and continues to use
...
```bash
# Option 1: clone the SDK; installing its Python package is optional.
