...
At startup, LeRobot requires a successful current feedback refresh from all seven motors before it enables torque. During operation, a transient refresh failure may reuse the last snapshot for up to `0.1` seconds. Missing or expired feedback raises an error, disables torque and disconnects the robot rather than substituting a fabricated `0°` position.
...
The follower's motor directions are fixed by its internal family profile. RS motors are mounted opposite to the DM build, so the RS profile uses `-1` for every joint. The follower applies this conversion in both directions, keeping observations and actions in one public robot coordinate frame. Adjust the leader's directions for teleoperation preferences.
