...
For a complete dataset in an HF Storage Bucket, use `--dataset.repo_id=OWNER/BUCKET`,
`--dataset.repo_type=bucket` and `--dataset.streaming=true`. The bucket must contain the dataset's
`meta/` directory as well as its Parquet and video files.
