...
To see what a dataset in this format holds before reading about its layout, open one in the [LeRobot Dataset Visualizer](https://huggingface.co/spaces/lerobot/visualize_dataset?path=/lerobot/svla_so101_pickplace/episode_0): camera recordings, synchronized state and action charts, and every episode, in the browser. See [Explore a Dataset in the Browser](./using_dataset_tools#explore-a-dataset-in-the-browser).
