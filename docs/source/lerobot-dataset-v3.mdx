...
- Entry point names are the format's public identity — pick one specific to your project, the
  way a package name is. Two installed packages claiming the same name is treated as an error,
  below.
...
If two installed packages claim the same `storage_format`, neither is used. Which one
`entry_points()` lists first depends on `sys.path`, so picking it would mean the same set of
installed packages reading a dataset differently on different machines. Opening a dataset in
that format raises instead, naming every claimant:

```
storage_format 'my_format' is claimed by more than one installed package, so which reader
serves it is ambiguous:
  robolake-reader 0.4.1 -> robolake.lerobot_reader
  acme-lerobot-store 1.2.0 -> acme.store.reader
Uninstall all but one, or choose explicitly before opening the dataset:
  from lerobot.datasets.storage import register_dataset_reader
  register_dataset_reader("my_format", "<module>")
```

Only that format is affected — every other format, built-in ones included, keeps loading.
