...
- Supports every LeRobot transform except `JPEGCompression`, and the torchvision `ColorJitter`, `RandomAffine`, `RandomRotation` and `Identity`. Others raise at startup.
- The custom transforms (`GaussianNoise`, `MotionBlur`, `RandomShadow`, ...) are silent no-ops on the dataloader backend because it sees `uint8` frames. On the GPU they apply, so recipes using them will train differently.
...
- Results are not bit-identical to the dataloader backend, which rounds to `uint8` between transforms.
