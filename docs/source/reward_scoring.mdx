...
Application code should use its model-specific entry point, such as `score_robometer_dataset()` or `score_rynnvalue_dataset()`. Adapter authors use `score_dataset(dataset, scorer, ...)`, and consumers use `read_frame_signals()`.
