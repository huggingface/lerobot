...
Fresh `pip` installs do not use the repository's lock or `uv` source settings. On Linux, they may select PyTorch 2.14 and newer CUDA packages within the allowed bounds. To reproduce the locked source environment, run `uv sync --locked` from a source checkout with the extras you need.
...
### PyTorch 2.14 in a separate Linux environment

Linux dependency bounds allow PyTorch 2.14, torchvision 0.29, and TorchCodec 0.13. The locked source environment still uses PyTorch 2.11 and torchvision 0.26. Ordinary `uv` source installs also use the CUDA 12.8 index, which currently has no PyTorch 2.14 wheels, even without the lock. Fresh source installs can select TorchCodec 0.13 alongside PyTorch 2.11. macOS and Windows bounds are unchanged.

For a separate Python 3.12 environment using CUDA 12.6, run these commands from your source checkout after [installing system-wide FFmpeg using the uv instructions](#install-ffmpeg-for-video-decoding):

```bash
uv venv --python 3.12 .venv-cu126
uv pip install --python .venv-cu126/bin/python --no-sources --torch-backend cu126 \
    -e ".[dataset]" "torch==2.14.0" "torchvision==0.29.0" "torchcodec==0.13.0"
source .venv-cu126/bin/activate
python -c "import torch; import torchcodec; print(torch.__version__, torch.version.cuda)"
uv pip check --python .venv-cu126/bin/python
```

The version check should print `2.14.0+cu126 12.6`. The installation and video decoding were tested on Linux x86_64 with `uv` 0.11.30.

`--no-sources` bypasses the checkout's CUDA 12.8 source selection for this install only. Use `python` from the activated `.venv-cu126` environment to run your code. Ordinary `uv run` can sync the project environment, which is `.venv` by default. `uv sync --locked` restores the locked versions in the environment it manages; configuration or `--active` can change that target. Keep `.venv-cu126` separate from the locked source environment.
