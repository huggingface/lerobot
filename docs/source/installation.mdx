...
Fresh `pip` installs do not use the checkout's lock or `uv` source settings. On Linux, they may select PyTorch 2.14 and newer CUDA packages within the allowed bounds. To reproduce the committed source environment, use `uv sync --locked` with the extras you need.
...
`--no-sources` bypasses the checkout's CUDA 12.8 source selection for this install only. Use `python` from the activated environment to run your code. Ordinary `uv run` can sync the project environment, and `uv sync --locked` restores the locked versions in the environment it manages. Keep this environment separate from the locked source environment.
