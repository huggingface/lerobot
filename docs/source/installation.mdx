...
This guide uses `conda` (via miniforge) to manage environments (recommended). If you prefer another environment manager (e.g. `uv`, `venv`), ensure you have Python >=3.12, then skip ahead to [Environment Setup](#step-2-environment-setup). You may also need `ffmpeg` — see [Install ffmpeg](#install-ffmpeg-for-video-decoding) for platform-specific instructions.
...
LeRobot uses [TorchCodec](https://github.com/meta-pytorch/torchcodec) for video decoding, which requires `ffmpeg`.

> [!NOTE]
> **Platform support:** TorchCodec is **not available** on macOS Intel (x86_64), Linux ARM (aarch64, arm64, armv7l), or Windows with PyTorch < 2.8. On these platforms, LeRobot automatically falls back to TorchVision's VideoReader — **you do not need to install `ffmpeg`** and can skip to [Step 3](#step-3-install-lerobot-).
>
> For the full compatibility matrix, see the [TorchCodec installation table](https://github.com/meta-pytorch/torchcodec?tab=readme-ov-file#installing-cpu-only-torchcodec).
...
> [!TIP]
> This usually installs `ffmpeg 7.X` with the `libsvtav1` encoder. If you run into issues (e.g. `libsvtav1` missing — check with `ffmpeg -encoders` — or a version mismatch with `torchcodec`), you can:
>
...
<hfoption id="System package manager (PyTorch ≥ 2.10 only)">
