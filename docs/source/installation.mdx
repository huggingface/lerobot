...
> **Blackwell GPUs (RTX 5090 / RTX PRO 6000, `sm_120`):** the default wheels (`torch 2.11+cu128`, driver >= 570.86) already ship `sm_120` kernels — no changes needed. CUDA 13 wheels are also verified: installing `torch==2.14.0+cu130` / `torchvision==0.29.0+cu130` from the `cu130` index (see the note in `pyproject.toml`) passes the policy test-suite and ACT / pi0 / pi0.5 forward+backward on RTX 5090, RTX PRO 6000 and RTX 4090 (validation details in the [xzwgit/lerobot fork README](https://github.com/xzwgit/lerobot)). Caveat: `torchcodec` wheels paired with `torch>=2.12` are not released yet, so on the cu130 stack video-dataset decoding is unavailable (image datasets unaffected) — see the torchcodec pairing note in `pyproject.toml`.
