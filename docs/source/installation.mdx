...
| `trackio`  | `trackio`                                   | Experiment tracking without wandb   |
...
Training runs pick a tracker with `--tracker.type`. Passing it is what enables tracking; without it
metrics are only written to the local log. Pass `--tracker=null` to switch tracking off again for a
run started from a config that has a tracker in it.

**Weights and Biases** is included in the `training` extra. To use
[Weights and Biases](https://docs.wandb.ai/quickstart), log in once:
...
lerobot-train \
    ...
    --tracker.type=wandb \
    --tracker.project=my-project
```

**Trackio** ([trackio](https://github.com/gradio-app/trackio)) is an alternative that needs no
account and no API key: runs are written to a local SQLite database. Install it with the `trackio`
extra:

```bash
pip install 'lerobot[trackio]'
```
...
Then browse the run locally:

```bash
trackio show --project my-project
```

To view a run from anywhere instead (and to track a run submitted to HF Jobs with `--job.target`,
whose local database would not outlive the job), host the dashboard on a Hugging Face Space by
adding `--tracker.space_id=${HF_USER}/my-dashboard`. The Space is created on first use.

> [!NOTE]
> The older `--wandb.enable=true` flags still work and are equivalent to `--tracker.type=wandb`.
