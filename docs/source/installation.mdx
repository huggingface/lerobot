...
On Linux, the install path determines which CUDA wheel you get. macOS installs use the PyPI default (MPS) and can skip this section. On Windows with an NVIDIA GPU, `uv sync` may install a CPU-only PyTorch build — see [Windows with NVIDIA CUDA](./torch_accelerators#windows-with-nvidia-cuda) for the override steps.
