...
This guide uses [`uv`](https://docs.astral.sh/uv/) to manage environments (recommended, requires PyTorch >= 2.10). If you prefer `conda` (via miniforge) or another environment manager, ensure you have Python >=3.12, then follow the `conda` tab in each step below.

## Step 1: Install an environment manager

<!-- prettier-ignore-start -->

<hfoptions id="install_env_manager">
...
**Feature Extras:**
LeRobot provides **feature-scoped extras** that map to common workflows. The commands below use `uv pip install`; if you are using `conda` or plain `pip`, drop the `uv ` prefix.
...
```bash
uv pip install 'lerobot[core_scripts]'          # Record, replay, calibrate
uv pip install 'lerobot[training]'              # Train policies
uv pip install 'lerobot[core_scripts,training]' # Record + train
uv pip install 'lerobot[all]'                   # Everything
```

**Policy, environment, and hardware extras** are still available for specific dependencies:

```bash
uv pip install 'lerobot[pi]'             # Pi0/Pi0.5/Pi0-FAST policy deps
uv pip install 'lerobot[smolvla]'        # SmolVLA policy deps
uv pip install 'lerobot[diffusion]'      # Diffusion policy deps (diffusers)
uv pip install 'lerobot[aloha,pusht]'    # Simulation environments
uv pip install 'lerobot[feetech]'        # Feetech motor support
```
...
LeRobot provides optional extras for specific functionalities. Multiple extras can be combined (e.g., `.[aloha,feetech]`). For all available extras, refer to `pyproject.toml`. The commands below use `uv pip install`; if you are using `conda` or plain `pip`, drop the `uv ` prefix.
...
```bash
uv pip install -e ".[aloha]" # or "[pusht]" for example
```
...
```bash
uv pip install -e ".[feetech]" # or "[dynamixel]" for example
```
