...
camera_config = {"front": OpenCVCameraConfig(index_or_path=0, width=640, height=480, fps=fps)}s
...
dataset = LeRobotDataset.create(
    repo_id="pepijn223/so100_api_11",
    fps=fps,
...
while episode_idx < num_episodes:
    ...
    if events["exit_early"]:
        log_say(f"Stop recording episode {episode_idx + 1}")
        events["exit_early"] = False
...
dataset = LeRobotDataset("pepijn223/so100_api_11", episodes=[episode_idx])
...
You can use the `record` script from [`lerobot/record.py`](https://github.com/huggingface/lerobot/blob/main/lerobot/record.py) but with a policy checkpoint as input. For instance, run this command or API example to run inference and record 10 evaluation episodes:
...
policy = ACTPolicy.from_pretrained("pepijn223/act_api_test_1")
...
dataset = LeRobotDataset.create(
    repo_id="pepijn223/eval_so100_api_17",
    fps=fps,
    features=dataset_features,
    robot_type=robot.name,
)
