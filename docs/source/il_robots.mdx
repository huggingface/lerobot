...
Locally, your dataset is stored in this folder: `~/.cache/huggingface/lerobot/{repo-id}`. At the end of data recording, your dataset will be uploaded on your Hugging Face page (e.g. `https://huggingface.co/datasets/${HF_USER}/record-test`) that you can obtain by running:
...
Your dataset is uploaded to the Hub by default (unless you passed `--dataset.push_to_hub=False`), so you can open it in the [LeRobot Dataset Visualizer](https://huggingface.co/spaces/lerobot/visualize_dataset) right away: enter your repo id on its home page, or go straight to the first episode with the link this prints:

```bash
echo "https://huggingface.co/spaces/lerobot/visualize_dataset?path=/${HF_USER}/record-test/episode_0"
```

Sign in on the visualizer to open a private dataset. See [Explore a Dataset in the Browser](./using_dataset_tools#explore-a-dataset-in-the-browser) for a walkthrough.
