...
By default, `lerobot-record` encodes video on-the-fly during recording using background threads (one per camera). If you don't have a powerful computer, you can disable this by setting `--dataset.streaming_encoding=false`.

```bash
# Default (streaming ON)
lerobot-record --dataset.repo_id=user/dataset --dataset.single_task="test" ...

# Old behavior (streaming OFF)
lerobot-record --dataset.repo_id=user/dataset --dataset.single_task="test" --dataset.streaming_encoding=false
```
