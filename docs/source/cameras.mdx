...
| `ZedCamera`       | Stereolabs ZED stereo (with depth)  |
| `ZedOneCamera`    | Stereolabs ZED X One (monocular)    |
...
`OpenCVCamera`, `RealSenseCamera` and `ZedCamera` support auto-discovery. Run the command below to list available devices and their identifiers. Note that these identifiers may change after rebooting your computer or re-plugging the camera, depending on your operating system.

```bash
lerobot-find-cameras opencv # or realsense for Intel Realsense, zed for Stereolabs ZED cameras
```
...
<hfoption id="Stereolabs ZED Camera">
...
# Create a `ZedCameraConfig` with your camera's serial number and enable depth.
# `resolution` drives the stream, so leave fps/width/height unset unless you need to pin them.
config = ZedCameraConfig(
    serial_number=12345678,  # replace with your camera's serial
    resolution="AUTO",
    depth_mode="NEURAL",
    color_mode=ColorMode.RGB,
    use_depth=True,
...
# Instantiate and connect a `ZedCamera` with warm-up read (default).
camera = ZedCamera(config)
camera.connect()

# `read_latest()` and `read_latest_depth()` return the color and depth of the same grab.
try:
    color_frame = camera.read_latest()
    depth_map = camera.read_latest_depth()
    print("Color frame shape:", color_frame.shape)
    print("Depth map shape:", depth_map.shape)
finally:
    camera.disconnect()
...
> [!WARNING]
> On `ZedCamera`, `read()` and `read_depth()` each wait for the _next_ frame, so calling them one after the other returns images from two different captures. Use `read_latest()` with `read_latest_depth()` when you need a color and depth pair from the same capture.
