...
| `OrbbecCamera`    | Orbbec (with depth)                 |
...
Orbbec support relies on the `pyorbbecsdk` library. Install it with the optional extra
(prebuilt wheels are published for Linux x86_64):
...
# Create an `OrbbecCameraConfig` specifying your camera's serial number (or unique name).
config = OrbbecCameraConfig(
    serial_number_or_name="CP1234567890",
    fps=30,
    width=1280,
    height=720,
    color_mode=ColorMode.RGB,
...
# Capture a color frame via `read()`.
try:
    color_frame = camera.read()
    print("Color frame shape:", color_frame.shape)
finally:
...
> [!NOTE]
> `OrbbecCamera` is color-only: `read()` returns an `(H, W, 3)` `uint8` RGB frame. Depth capture is not implemented yet, so LeRobot dataset image features keep the usual RGB shape.
