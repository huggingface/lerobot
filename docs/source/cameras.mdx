...
| `OrbbecCamera`    | Orbbec depth cameras (with depth)   |
...
`OpenCVCamera` and `RealSenseCamera` and `OrbbecCamera` support auto-discovery. Run the command below to list available devices and their identifiers. Note that these identifiers may change after rebooting your computer or re-plugging the camera, depending on your operating system.
