...
| `OrbbecCamera`    | Orbbec RGBD cameras (Structured Light, Stereo Vision, etc) |
...
`OpenCVCamera`, `RealSenseCamera`, and `OrbbecCamera` support auto-discovery. Run the command below to list available devices and their identifiers. Note that these identifiers may change after rebooting your computer or re-plugging the camera, depending on your operating system.

```bash
lerobot-find-cameras opencv # or realsense for Intel Realsense cameras, or orbbec for Orbbec cameras
```
...
- **Color and depth capture** using an Orbbec camera — To leverage depth data from Orbbec cameras, we developed a 4-channel RGBD variant of ACT, extending the standard 3-channel RGB input to include a depth channel. This model has been validated on the SO101 robotic arm. See the [`so101-act-rgbd`](https://github.com/orbbec/lerobot/tree/feat/orbbec-so101-act-rgbd) branch for details.
...
# Create an `OrbbecCameraConfig` specifying index or serial number and enabling depth.
config = OrbbecCameraConfig(
    index_or_serial_number=0,  # or use serial number string like "CL4K2310053"
    fps=30,
    width=640,
    height=480,
    color_mode=ColorMode.RGB,
...
# Instantiate and connect an `OrbbecCamera` with warm-up read (default).
