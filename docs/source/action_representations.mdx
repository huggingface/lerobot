...
```
action = [shoulder: 45.0, elbow: -30.0, wrist_pitch: 10.0, wrist_roll: 0.0, wrist_yaw: 5.0, gripper: 0.8]
```
...
> [!NOTE]
> OpenPI's upstream code names this transform `DeltaActions`, but what it implements is UMI's **relative trajectory** representation, not true delta. We use the term "relative" in LeRobot to match the correct semantics.

### Delta Actions (sequential differences)

Each action is an offset from the **previous action** (or from the current state for the first step). This is NOT what OpenPI/pi0 uses:
...
The figure below (from [UMI, Chi et al., 2024](https://arxiv.org/abs/2402.10329)) illustrates the key difference. With **relative trajectory**, every action in the chunk points back to the same origin (current state), so a new inference step cleanly resets the reference. With **delta**, each action depends on the previous one, so small prediction errors compound.
...
Pose
 ...
 │      ╱
 │    ●──→ ● ──→ ● ──→ ●     delta
 │    │                        (each step relative to previous)
 │    │
 │    ●──────────→ ●
 │    │            │          relative trajectory
 │    ●────→ ●     │          (all relative to same origin)
 │    ▲            │
 │    state(t=0)   state(t=4)
 └──────────────────────────→ Time
```

At each new inference step, relative actions re-anchor to the current state, avoiding drift. Delta actions carry forward any error from previous steps.
...
### Enabling relative actions for pi0/pi0.5
...
- [OpenPI](https://github.com/Physical-Intelligence/openpi) - Physical Intelligence's open-source pi0 implementation. Uses relative actions (named `DeltaActions` in their code).
