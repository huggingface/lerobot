...
This tutorial will explain how to integrate your own robot design into the LeRobot ecosystem and have it access all of our tools (data collection, control pipelines, policy training an inference).

To that end, we provide the [`Robot`] base class in the LeRobot which specifies a standard interface for physical robot integration. Let's see how to implement it.
...
- [`FeetechMotorsBus`] – for controlling Feetech servos
- [`DynamixelMotorsBus`] – for controlling Dynamixel servos

Please refer to the [`MotorsBus`] abstract class to learn about its API.
...
It should implement the logic for calibration and updates the `self.calibration` dictionary. If you're using Feetech or Dynamixel motors, our bus interfaces already include methods to help with this.
