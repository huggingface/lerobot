...
- **CUDA**: acceleration with NVIDIA GPUs
...
After installation, accelerator availabiliry can verified by running
```python
import torch
...
However, in most cases, presence of an accelerator is detected automatically and `policy.device` parameter can be omitted form CLI commands.
