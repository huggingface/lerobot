...
You can pass a `HubEnvConfig` object to LeRobot's `make_env` to configure a Hub environment. The factory receives this object when it accepts `cfg` as a keyword argument or accepts `**kwargs`; a factory with only `n_envs` and `use_async_envs` also works. When loading through a string Hub reference, LeRobot does not pass `cfg`, so a factory that declares a `cfg` parameter must give it a default value to support that entry point.
