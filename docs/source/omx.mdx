...
To connect the OMX robot to your computer, plug both the leader arm and the follower arm into USB ports. Once connected, identify which serial port corresponds to each arm using:

```bash
lerobot-find-port
```

This command lists the connected serial devices and helps you determine which port is associated with the leader arm and which port is associated with the follower arm.
