...
FineART-VLA builds on **π0.5 from Physical Intelligence** and its
[openpi implementation](https://github.com/Physical-Intelligence/openpi),
through LeRobot's [Pi05](./pi05) policy. It extends this foundation with a trainable PaliGemma language head and a
...
Use Pi05 when you only need task-conditioned actions. Use FineART-VLA when the policy
...
FineART-VLA can initialize from a Pi05 checkpoint. The policy architecture remains
compatible, while FineART-VLA builds its own processors so recipe labels and FAST
...
The default factorizes inference as π(a|o, subtask)·π(subtask|o, task).
A custom joint-sequence recipe needs a matching runtime processor that renders
the task and generated assistant span with the same causal marks as training.
The old policy-side joint prompt builder is no longer used by rollout.
...
`subtask_replan_steps`, `joint_subtask_conditioning`, and `apply_chat_template`
are no longer policy CLI options. Older checkpoint configs with the default
joint setting load through a checkpoint migration hook; checkpoints explicitly
requesting the old joint layout fail with guidance rather than changing prompts.

Combined memory/subtask scratchpads are not the default and require a custom
recipe plus a memory-aware runtime controller. Keep `memory_scratchpad=false`
for ordinary subtask generation; older scratchpad checkpoints retain their
...
<Tip>
  ...
  FineART-VLA falls back to the plain Pi05 prompt path. This is useful for
  compatibility but does not train the language planner.
</Tip>

## Train FineART-VLA

This example initializes FineART-VLA from `lerobot/pi052_base`, the existing
Hub initialization checkpoint, and trains the embedded 70/30 subtask recipe.
That checkpoint contains π0.5 base weights converted to the native policy layout;
it does not contain additional FineART language training. Its repository name is
retained for compatibility. Legacy `pi052` configuration and tokenizer identifiers
remain loadable; newly saved configurations use `fineart_vla`.

```bash
lerobot-train \
    --dataset.repo_id=${HF_USER}/my_language_annotated_dataset \
    --policy.type=fineart_vla \
...
Start with a small run and confirm that W&B examples show
...
For conditioning-gradient spikes, opt in to a separate time-MLP/adaptive-normalization
group with `policy.conditioning_lr_scale` (default `1.0`) and
`policy.conditioning_grad_clip_norm` (default `None`).
`policy.action_expert_grad_clip_norm` independently limits the remaining expert
group. Group clipping runs before global clipping, measures norms in float64,
and rejects nonfinite gradients before updating the optimizer. It supports
single-device and DDP FP32/BF16 training without a GradScaler, not sharded training.
These controls contain spikes; they do not establish their underlying cause.

To explicitly sample stored `task_aug` annotations during training even when the
dataset supplies a canonical task, set the recipe binding `task: sample_task()`.
Sampling is deterministic by frame index. Recipes without this binding retain
the default behavior in which runtime task overrides take precedence.

### Dataset-specific FAST tokenizer

The universal FAST tokenizer works out of the box. Dataset-specific fitting is
an explicit preparation step, not a hook in the shared training loop.
Call `lerobot.policies.fineart_vla.fit_fast_tokenizer.fit_fast_tokenizer` with the
dataset source, split, action normalization statistics and relative-action
settings used by training. Then pass the returned directory with
`--policy.action_tokenizer_name=/path/to/fitted/tokenizer` and leave
`--policy.auto_fit_fast_tokenizer=false`. Saved processor artifacts remain
self-contained.
...
FineART-VLA has two inference loops, and both avoid repeatedly encoding the expensive
multimodal prefix:

1. **Action denoising** encodes the image/language prefix once, reuses its KV
   cache across flow steps, constructs action-suffix masks on-device, and
   crops temporary suffix K/V instead of cloning the prefix cache. It creates
   the timestep schedule on the device once per chunk, preserving the historical
   timestep arithmetic. This implementation detail does not establish an
   end-to-end speedup without benchmarking.
...
- **Large flow-loss gradients:** KI-off joint prefix/action attention uses PyTorch
  SDPA's math backend with FP32 attention arithmetic by default, casting its output back to the
  model dtype. This avoids reduced-precision attention, which can destabilize gradients at
  ...
  values. Monitor the separate `flow_loss`, `text_loss`, and pre-clipping gradient
  norm rather than relying on the finite total loss alone.
