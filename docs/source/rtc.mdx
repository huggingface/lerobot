...
The diffusion path is experimental. At each denoising timestep, it blends the overlap toward a forward-noised previous chunk. `LINEAR` and `EXP` extend the known prefix with a soft transition mask. The known region receives fresh noise at every timestep, so even DDIM with `eta=0` remains stochastic when RTC guidance is active. This is inspired by RePaint, but it does not implement RePaint's resampling schedule and should not be treated as full RePaint or a standard reference diffusion RTC algorithm. Two known approximations: RePaint uses a hard mask, whereas the soft transition band mixes two independent noise realisations, so at weight `w` the blended latent carries `w² + (1 - w)²` of the expected noise variance (half of it at `w = 0.5`, a standard deviation of about 0.71×) and the denoiser sees a slightly under-noised latent there; and the final clean-level blend pins the first `inference_delay` actions exactly rather than letting the sampler reconcile them. A smoother boundary in a benchmark on one task supports that behavior in that setting; it does not establish broad task success-rate improvements.

Multi-Task DiT's default `n_obs_steps=2` conditions on two consecutive policy ticks; the rollout engine assembles that history as described in [Observation History](#observation-history). If you call `predict_action_chunk()` yourself, pass the complete preprocessed history on every call: state tensors have shape `(batch, n_obs_steps, state_dim)`, and each camera tensor has shape `(batch, n_obs_steps, channels, height, width)`. Do not pass only the latest frame or advance history once per inference request.
