...
Real-Time Chunking (RTC) is an inference-time method that allows large, chunk-generating robotic policies, such as [Pi0](./pi0), [Pi0.5](./pi05), [SmolVLA](./smolvla), and [Multi-Task DiT](./multi_task_dit), to produce smooth, continuous, and reactive motion despite having high inference latency. RTC guidance is formulated for flow-matching policies; Multi-Task DiT additionally supports it with the diffusion (DDPM/DDIM) objective via RePaint-style inpainting.
