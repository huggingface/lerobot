...
If you're using Feetech or Dynamixel motors, LeRobot provides built-in bus interfaces. Build one
with [`SerialMotorsBus`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/motors/motors_bus.py):
it hands back the bus for the motors you pass it, the way `pathlib.Path` hands back a `PosixPath`.
...
Naming either directly also works. Refer to [`SerialMotorsBus`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/motors/motors_bus.py) to learn about the API.
...
In the next sections, the examples use Feetech motors. Replace them and adapt to your motors if necessary.
