...
This prefix system is how `lerobot` automatically finds your plugin in the Python environment. Use **underscores**: the distribution `Name` is imported verbatim, so a hyphenated `lerobot-robot-foo` matches no prefix and is silently skipped.
