...
If you're using Feetech, Dynamixel or Robstride motors, LeRobot provides built-in bus interfaces:
...
- [`RobstrideMotorsBus`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/motors/robstride/robstride.py) and [`RobstridePrivateMotorsBus`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/motors/robstride/robstride_private.py) – for controlling Robstride motors over CAN. The first targets motors switched to the MIT communication mode, the second speaks the factory-default private protocol; see [Robstride Motors and CAN Bus](./robstride) for choosing between them.
