...
- `lerobot_policy_` for a policy.
- `lerobot_env_` for an environment.
- `lerobot_strategy_` for a `lerobot-rollout` strategy (see [Bring Your Own Rollout Strategy](./bring_your_own_rollout_strategies)).

This prefix system is how `lerobot` automatically finds your plugin in the Python environment. Use **underscores**: the distribution `Name` is imported verbatim, so a hyphenated `lerobot-robot-foo` matches no prefix (discovery logs a warning telling you so).
...
The device class (`MyAwesomeTeleop`) must be located in a predictable module relative to its configuration class (`MyAwesomeTeleopConfig`). `lerobot` searches these locations, in order, and takes the first module that both imports and has a `MyAwesomeTeleop` attribute — where the config class lives in `lerobot_teleoperator_my_awesome_teleop/config_my_awesome_teleop.py`:

1. The config class's **parent package**: `lerobot_teleoperator_my_awesome_teleop`.
2. The parent package plus the **lower-cased device class name**: `lerobot_teleoperator_my_awesome_teleop.myawesometeleop`.
3. Only when the config module is named `config_*`, the parent package plus that name with the prefix dropped: `config_my_awesome_teleop` → `lerobot_teleoperator_my_awesome_teleop.my_awesome_teleop`.

> [!WARNING]
> The config class's **own module is not on that list**. Defining `MyAwesomeTeleop` next to `MyAwesomeTeleopConfig` inside `config_my_awesome_teleop.py` and stopping there resolves to nothing. Either give the device its own module (candidate #2 or #3), or re-export it from the package `__init__.py`, which is what satisfies candidate #1 — **Convention \#4** below does exactly that, so following it makes candidate #1 succeed no matter how you lay the files out.
