...
Decorate it with `idempotent_connect`: calling `connect()` on a robot that is already connected then becomes a no-op, and if anything in the body raises, `disconnect()` is called for you so no port or camera is left half-open. Skip the parts that are already connected, so that a retry after a partial failure resumes instead of failing on an already-open port.
...
Here, we already handle this in our `MotorsBus` and `Camera` classes so we just need to call their own `disconnect()` methods. Wrap each call in a `Cleanup` step: every step runs even if an earlier one failed, a resource that is already released counts as success, and the failures (if any) are raised together once everything has been attempted. This makes `disconnect()` safe to call at any time, which both the `connect()` rollback and the `with robot:` context manager rely on.
