...
checkpoint recipe before `${subtask}`. WALL-X inherits the single public
`generate_text(processed_batch)` API and implements only the protected decoder
hook for its Qwen vision-language expert.
