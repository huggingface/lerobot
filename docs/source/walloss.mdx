...
WALL-X stores its language recipe in the policy configuration and uses it to
train from LeRobot's optional language columns:
...
(default `768`). Set `policy.recipe=null` to disable recipe rendering.
