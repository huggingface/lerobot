...
queries, or `kind="subtask"` for WALL-OSS's language-level next action. The
text comes from the policy's own Qwen vision-language expert.
...
subtask generation and send the task directly to action inference.
