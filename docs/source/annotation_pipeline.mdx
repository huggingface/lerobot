...
1. **flag** whether the view is blurry / unusable (including a visible human
   teleoperator), and
2. **label** the view with a canonical name from a closed vocabulary describing
   where the camera sits relative to the robot: `top` (overhead), `bottom` (from
   below), `wrist` (on the arm, moving with it — qualified `left_wrist` /
   `right_wrist`), and `side` for any fixed around-the-robot view, qualified with
   a direction suffix — `front_side`, `rear_side`, `left_side`, `right_side` (or
   plain `side` when the direction is unclear).

The VLM first decides, from motion across the sampled frames, whether each camera
is `fixed` or `robot_mounted` (recorded as `mount_type`); a `robot_mounted` camera
is the only one that can be a `wrist`, and this mount verdict deterministically
overrides the finer label (so a prominent gripper on a fixed overhead camera stays
`top`, never `wrist`). Any residual label collisions are resolved deterministically
in Python — the VLM never sees the existing dataset key names.
...
Some repos are **collections** of many independent datasets under per-dataset
subfolders (e.g. `lerobot/community_dataset_v3` → `<user>/<task>/…`) and have no
root `meta/info.json`. List the sub-datasets to curate with `--subpaths`; each is
materialized (its `meta/` + first episode only) and curated in turn, and — in
`--mode=rename` — renamed in place under its own prefix. Leave `--subpaths` off to
auto-discover and curate **every** sub-dataset; `--parallelism` processes several
at once against the shared VLM server (the sweep is I/O-bound, so this is the main
throughput knob). Results are written to a single aggregated
`camera_curation_collection.json` (and echoed to stdout).

That aggregated file is rewritten **after each sub-dataset finishes**, so it
doubles as a progress/resume log: it lists which sub-datasets were `renamed`,
`failed`, or had `conflicts`. Pass `--resume=true` to skip sub-datasets already
recorded there without an error and process only the rest (failures are retried) —
so an interrupted sweep continues instead of redoing the whole collection. The
file must have survived, so point `--report_path` at a persistent location (a
plain local run keeps it; an HF-Jobs pod is ephemeral unless the path is mounted).

```bash
uv run lerobot-curate-cameras \
    --repo_id=lerobot/community_dataset_v3 \
    --subpaths='[00ri/so100_battery, 1g0rrr/demo2_frame_holder]' \
    --mode=report
```
