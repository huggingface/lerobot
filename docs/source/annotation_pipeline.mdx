...
memory, interjections, speech, visual Q&A, and dense Embodied
Chain-of-Thought (ECoT) reasoning traces that a policy can be trained on.
...
```text
  ...
  └──────────────────────────┬──────────────────────────┘
        ...
        ▼                    ▼                     ▼
  ┌──────────┐      ┌───────────────┐        ┌──────────┐        ┌──────────┐   one shared Qwen-VL
  │   plan   │      │ interjections │        │   vqa    │        │   ecot   │   server (vLLM, OpenAI
  └────┬─────┘      └───────┬───────┘        └────┬─────┘        └────┬─────┘   API) drives all four
       └────────────────────┼──────────────────┴─────────────┘
...
Four modules (`plan`, `interjections`, `vqa`, `ecot`) all talk to **one**
shared VLM. Each module stages its output to disk, a validator checks it,
and a single writer rewrites the dataset shards in place.
...
| `ecot` (dense ECoT reasoning trace)         | `language_persistent` | `ecot`          |
...
3. runs the `plan` / `interjections` / `vqa` / `ecot` modules across the
   dataset with `lerobot-annotate`,
...
| `--ecot.enabled`          | `true`  | the dense ECoT reasoning traces     |
...
The `ecot` module emits dense Embodied Chain-of-Thought (ECoT) reasoning
traces — scene perception, object identification, task plan, and
sub-task decomposition — as persistent, camera-agnostic `ecot` rows in
`language_persistent`. Like the `plan` module's `plan` / `memory` rows,
one trace stays active from its anchor until the next, so every frame
has exactly one reasoning trace to train on. It reuses the same
contact-sheet grounding as the other modules.

| Flag                           | Default | What it does                                                           |
| ------------------------------ | ------- | ---------------------------------------------------------------------- |
| `--ecot.enabled`               | `true`  | Emit the dense ECoT reasoning traces.                                  |
| `--ecot.emission_hz`           | `0.5`   | One reasoning trace every `1/hz` seconds (mirrors the VQA cadence).    |
| `--ecot.window_seconds`        | `2.0`   | Sampled window centered on each anchor so the VLM reasons over motion. |
| `--ecot.frames_per_second`     | `2.0`   | Frame sampling rate within the window (`2.0` = one frame every 0.5s).  |
| `--ecot.max_frames_per_prompt` | `12`    | Frame budget per prompt; windows exceeding this are clamped.           |

Iterate on it alone by disabling the other modules, and tune the cadence
with the nested config form (never the legacy `--ecot=...` override):

```bash
lerobot-annotate --root=/path/to/dataset \
    --plan.enabled=false --interjections.enabled=false --vqa.enabled=false \
    --ecot.emission_hz=0.5
```
...
`plan` / `interjections` / `vqa` / `ecot` modules.
...
- a VQA branch reads the `(vqa, user)` and `(vqa, assistant)` pairs from
  `language_events`.
- a reasoning branch reads the active `ecot` trace from
  `language_persistent` (same `active_at(t, ...)` semantics as
  `subtask` / `memory`).
...
1. **Persistent state vs. exact events.** Persistent rows (`subtask`,
   `plan`, `memory`, `ecot`) apply to the whole episode and answer "what's true
...
`--interjections.enabled` / `--vqa.enabled` / `--ecot.enabled`) to test
one at a time.
...
- **`ecot`.** ZR-0
  ([2026](https://arxiv.org/abs/2606.30552v1)): supervise the VLA with a
  dense structured reasoning process — scene perception, object
  identification, task plan, and sub-task decomposition — that the paper
  shows transfers across embodiments. This module ports that _annotation_
  contribution as persistent, camera-agnostic rows; the dual-stream
  architecture and flow-matching action expert are policy-training
...
`max_interjections_per_episode` interjection calls,
`ecot.emission_hz × episode_seconds` ECoT calls, and
...
~50 VLM calls before ECoT; the `ecot` module adds roughly
`emission_hz × episode_seconds` more (~15 traces at the default 0.5 Hz).
