...
2. **label** the view with a canonical name from a closed vocabulary describing
   where the camera sits relative to the robot: `front` (facing the robot),
   `rear` (behind it), `side` (qualified as `left_side` / `right_side`), `top`
   (overhead), `bottom` (from below), and `wrist` (on the arm, qualified as
