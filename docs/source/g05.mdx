...
`predict_cot=true`. The interactive panel shows the chain-of-thought on a
`reasoning` line via `last_reasoning`, and `predict_action_chunk_with_runtime`
returns the full telemetry alongside the matching action chunk. The reasoning is
display-only: it never re-enters the observation batch, so it cannot replace the
operator's task.
