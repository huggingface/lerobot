...
G0.5 emits reasoning and actions from one inference stream, so it has no
separate text head: it does not implement `generate_text`, and the interactive
runtime therefore never generates a subtask for it. The operator's task is
forwarded byte-for-byte and drives action prediction directly. G0.5 never
samples `task_aug` text, launches a second planner, or feeds generated CoT back
as a replacement task.

`policy.runtime_system` on the checkpoint selects the path. System 1 executes
the selected ActionCodec or flow chunk directly:
...
`predict_cot=true`. `predict_action_chunk_with_text` returns the chain-of-thought
generated in the same pass as the chunk it belongs to:

```python
prediction = policy.predict_action_chunk_with_text(batch)
print(prediction.text)  # "Subtask: grasp the cup ..."
prediction.action  # the chunk that reasoning produced
```

The interactive panel shows it on a `reasoning` line, and
`predict_action_chunk_with_runtime` returns the full telemetry when you want the
generated token ids too. The reasoning is display-only: it never re-enters the
observation batch, so it cannot replace the operator's task. System 1 emits none
and reports `text=None`.
