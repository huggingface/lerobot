...
Fit a dataset-specific tokenizer explicitly before launching training, then
select it with `--policy.action_tokenizer_name=/path/to/fitted/tokenizer`.
The shared training and checkpoint-loading paths do not automatically fit or
replace tokenizers.
