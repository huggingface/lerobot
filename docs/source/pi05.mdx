...
recomputing them can change Pi0.5 training targets and the loss scale. Run the
augment script before training when global estimates are required. Updating a
dataset's `stats.json` does not change normalization statistics already saved
...
To recompute only the action/state statistics of a dataset that already contains
(possibly inaccurate) quantile statistics:
