...
By default, π₀.₅ predicts absolute actions. You can enable **delta actions** so the model predicts offsets relative to the current robot state. This can improve training stability for certain setups.

To use delta actions, first recompute your dataset stats in delta space via the CLI:
...
recompute_stats(dataset, delta_action=True, chunk_size=50, delta_exclude_joints=["gripper"])
```

The `chunk_size` should match your policy's `chunk_size` (default 50 for π₀.₅). `delta_exclude_joints` lists joint names that should remain in absolute space (e.g. gripper commands).

Then train with delta actions enabled:
