...
With `compile_model=False`, action inference skips vision encoding for camera features absent from the observation batch, including slots added by `empty_cameras`. Their fully masked prefix slots are retained. Compiled inference, training, and loss evaluation keep the existing camera padding and encoding. Keep the checkpoint's camera configuration; no checkpoint conversion is needed.
