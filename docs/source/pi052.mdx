...
Pi052 can optionally use short-horizon visual context based on
[MEM](https://arxiv.org/abs/2603.03596). Visual memory is disabled by default,
so existing checkpoints and training commands keep the single-frame Pi052
behavior.
...
Only the current-frame tokens leave the vision tower, keeping the downstream
language/action prefix length unchanged.

At the beginning of an episode, unavailable history is padded and masked. At
inference, Pi052 maintains the same strided image history internally and clears
...
`memory_stride` settings, but either path can be enabled on its own.
