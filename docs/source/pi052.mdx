...
The prompt contains the task, the previous annotated memory (when available), and
an instruction to update memory from observed progress and choose a subtask.
`prior_memory` resolves the memory before the currently active memory row;
`${memory}` and `${subtask}` supply the two fields of the same supervised target.
...
missing memory row skips the high-level target; an absent previous memory omits
that context turn. Annotate memory using only events observed by its timestamp,
...
**Runtime integration is still required.** The older language runtime on this
branch generates subtask and memory in separate calls. Its PI052 subtask prompt
does not consume memory, and it does not parse the combined response. The newer
rollout autosteer path also expects a plain subtask string. Neither should receive
the combined response unchanged as an action instruction. A compatible adapter
must validate both fields, retain Memory for the next planning prompt, apply only
Subtask to actions, and clear memory on episode reset. Invalid responses must not
partially update either field. Simply changing a recipe does not implement this loop.
