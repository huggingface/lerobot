...
updates make timestamped history necessary. The opt-in PI052 adapter below uses
...
**Adapter-based runtime:** set `--policy.memory_scratchpad=true` when training a
new checkpoint with these recipes. The saved flag opts its PI052 adapter into one
combined generation call. The adapter renders the input template from the recipe,
using generated memory/subtask state active one second before the observation.
It validates both output fields before updating either, stores a timestamped
memory/subtask update, and conditions actions on only the subtask. Repeated
subtasks are accepted normally and memory can update without a subtask change.
A task change or scene reset that clears the subtask invalidates scratchpad
history. Actions are refused until an initial valid response is available.
Leave runtime memory and subtask generation enabled for these checkpoints.

The flag defaults to `false`, preserving the older separate-call behavior of
existing checkpoints. **This adapter integration is not the newer interactive
rollout autosteer/RTC integration used in the separate bussing-evaluation checkout.**
That engine still needs the same timestamped history, parsing, and reset contract
before it can use scratchpad-trained checkpoints. Do not pass a combined response
unchanged as its action instruction.
