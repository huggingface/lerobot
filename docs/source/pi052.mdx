...
PI052 embeds its default recipe in `PI052Config`, like EO-1. It samples
...
PI052 does not replace them. For goal-only training, use an inline recipe with
...
and synchronous/RTC scheduling. PI052 only generates text when queried; ordinary
...
and trains the embedded 70/30 subtask recipe:
...
Call `lerobot.policies.pi052.fit_fast_tokenizer.fit_fast_tokenizer` with the
