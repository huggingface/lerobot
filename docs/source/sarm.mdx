...
- **stage**: integer stage index \(k \in \{0,\dots,K-1\}\)
- **\(\tau\)**: within-stage progress \(\tau \in [0,1]\)
- **target encoding**: \(y = k + \tau\) (this is what the dataset processor produces)

At inference time (and in downstream RA-BC), SARM converts the raw \(k+\tau\) value into a **normalized progress** in \([0,1]\) using dataset-level **temporal proportions** \(\bar{\alpha}_k\) (stored in `meta/temporal_proportions_*.json`).
...
- **Builds targets** as `sparse_targets` (and `dense_targets` in `dense_only`/`dual`) using the stage+tau encoding \(y = k + \tau\)
...
| `--policy.max_rewind_steps`       | Max rewind augmentation frames appended during training                      | `4`                       |
| `--policy.rewind_probability`     | Probability to apply rewind augmentation (training only)                    | `0.8`                     |
| `--policy.language_perturbation_probability` | Probability to replace the task text with random text (training only) | `0.2`                     |
...
> **Note**: SARM’s total sequence length is `1 + n_obs_steps + max_rewind_steps`. During evaluation/inference, rewind is disabled and the extra frames are masked out via a `lengths` tensor.
...
| `--output-path`        | Custom output path for parquet file                         | `<dataset_root>/sarm_progress.parquet` |
...
| `--rabc_epsilon`       | Small constant ε for numerical stability                 | `1e-6`    |
...
# Option 3: Set kappa = delta_mean + 2*delta_std (very selective, top ~5%)
--rabc_kappa=0.07
```

**Rule of thumb:** Start with `kappa = delta_mean`. This ensures roughly half your samples get full weight and half get soft-weighted, giving RA-BC meaningful filtering power.

**When RA-BC may not help:**

If your dataset is already high quality (consistent progress across all demonstrations), RA-BC won't provide much benefit since there's nothing to filter. Consider:
1. Using vanilla BC instead
2. Mixing in lower-quality demonstrations to create contrast
...
3. **Start with `kappa = delta_mean`**: Check the logged `rabc_delta_mean` and set kappa accordingly
4. **RA-BC needs contrast**: Works best when dataset has mix of good/bad demonstrations
