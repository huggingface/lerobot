...
def encode_depth(depth: np.ndarray) -> av.VideoFrame:
    """Log-scale depth before encoding (reduces dynamic range)."""
    depth = np.log1p(np.clip(depth, a_min=0, a_max=None))
    frame = av.VideoFrame(width=64, height=96, format="yuv420p12le")
    frame.planes[0].update(depth.astype(np.uint16).tobytes())
    return frame

def decode_depth(frame: np.ndarray) -> np.ndarray:
    """Inverse of encode_depth."""
    return np.expm1(np.clip(frame, a_min=0, a_max=None))
...
# Reading (decode is applied automatically)
dataset = LeRobotDataset(repo_id="my/depth_dataset")
