...
2. New frames are **not enqueued** (not blocked) — the capture loop continues uninterrupted
...
4. A warning is logged: `"Encoder queue full for {camera}, repeated the previous frame N time(s)"`
5. At episode end, total repeated frames per camera are reported
