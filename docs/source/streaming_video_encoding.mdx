...
### Backpressure and Repeated Frames

Each camera has a bounded queue (`encoder_queue_maxsize`, default 30 frames). When the encoder can't keep up:

1. The queue fills up (consuming RAM)
2. A frame that can't be enqueued waits at most one frame period (`1/fps`) for a free slot, then is **not enqueued**, so the capture loop stays on cadence
3. The encoder fills each missing slot by **repeating the previous frame**, so the video always has exactly one frame per recorded frame and stays aligned with the tabular data
4. Repeated frames are **excluded from the dataset statistics** (they would otherwise skew the distribution)
5. A warning is logged: `"Encoder queue full for {camera}, repeated the previous frame N time(s)"`
6. At episode end, total repeated frames per camera are reported

A repeated frame is a brief visual freeze (one frame period) in the video, not a missing sample. Sustained warnings still mean the encoder is a bottleneck and the video quality is degraded; see below.

To avoid silently saving a heavily degraded episode, recording **stops with an error** once a camera repeats the previous frame for roughly one second in a row (`fps` consecutive repeats). Episodes recorded before the failure are preserved.

### Symptoms of Encoder Falling Behind

- **System feels laggy and freezes**: all CPUs are at 100%
- **Repeated frame warnings** in the log
...
- **Recording stops with an error**: ~1s of consecutive repeated frames aborts recording
...
- CPU is the bottleneck (repeated frames, choppy robot, rerun lag)
...
| "Encoder queue full" warnings or repeated frames in dataset        | Encoder can't keep up (Queue overflow)       | If CPU is not at 100%: Increase `encoder_threads`, increase `encoder_queue_maxsize` or use HW encoding (`--dataset.rgb_encoder.vcodec=auto`).                                                                                                                                                    |
| Recording stops with an "encoder ... fell behind" error            | Encoder repeated a frame for ~1s straight    | Same fixes as "Encoder queue full" above (more `encoder_threads`, larger `encoder_queue_maxsize`, HW encoding, or lower throughput). Episodes saved before the error are preserved.                                                                                                               |
