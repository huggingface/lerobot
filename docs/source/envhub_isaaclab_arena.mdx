...
- 🖼️ **RTX Rendering**: Evalute vision-based policies with realistic rendering, reflections and refractions.
...
The following example environments are currently available in IsaacLab Arena:
...
# Example for a random-action agent
obs, info = env.reset()
for _ in range(100):
...
# LigtWheel LW-Benchub

Lightwheel AI are bringing `Lightwheel-Libero-Tasks` and `Lightwheel-RoboCasa-Tasks` with 268 tasks to the LeRobot ecosystem.
...
| Policy                   | Architecture | Task                  | Link                                                                                  |
| :----------------------- | :----------- | :-------------------- | :------------------------------------------------------------------------------------ |
| smolvla-double-piper-pnp | SmolVLA      | Double-Piper PickBowl | [HuggingFace](https://huggingface.co/LightwheelAI/smolvla-double-piper-pnp/tree/main) |
