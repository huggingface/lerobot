...
By default, no camera is recorded, all cameras arguments are set to `false`.  
If you want to, you can use custom `width` and `height` parameters for Reachy 2's cameras using the `--robot.cameras` argument:  

```bash
--robot.cameras="{ custom_right_teleop: {type: reachy2_camera, ip_address: 192.168.0.200, name: teleop, image_type: right, width: 1920, height: 1080}}"
```

where the parameters can be `name: teleop, image_type: right`, `name: teleop, image_type: left` or `name: depth, image_type: rgb`.  
You can also add images from other cameras than the one provided on the robot in your dataset.
