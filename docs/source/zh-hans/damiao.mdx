...
```bash
# 设置多个接口（例如，带有 2 个 CAN 总线的 OpenArms Followers）
...
| 参数 | 默认值 | 描述 |
| ------ | --------- | ----------------------------------------------------------- |
| `port` | - | CAN 接口（`can0`）或串行端口（`/dev/cu.usbmodem*`） |
...
- `id`：用于发送命令的 CAN ID
- `motor_type`：支持的电机类型之一（如 `"dm8009"`、`"dm4340"`）
- `recv_id`：用于接收响应的 CAN ID

OpenArms 默认 ID 遵循模式：发送 ID `0x0N`，接收 ID `0x1N`，其中 N 是关节编号。
