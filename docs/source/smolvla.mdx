...
## Run SmolVLA base model (zero-shot) on NERO Follower

If you want to test the SmolVLA base model without fine-tuning on a NERO Follower robot:
...
2. Download the pretrained model (use mirror if network is slow):

```bash
# Option 1: huggingface_hub
...
<Warning>
The base model has not been fine-tuned for the NERO Follower robot. Expect poor performance. Fine-tune on your own dataset for meaningful results.
</Warning>
