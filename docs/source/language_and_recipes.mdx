...
- `language_persistent`: a list of rows broadcast across every frame in an episode for state that remains active, such as `subtask`, `plan`, `memory`, and `ecot`.
...
Recipes are YAML files backed by `TrainingRecipe` and `MessageTurn` from
`lerobot.datasets.recipe`. They
