...
1. **Data in the dataset** — language annotations stored next to frames in
   ...
   behavior.
2. **Recipe** — a dictionary in the policy configuration that declares which
   annotation rows to bind and how to lay them out as chat turns (`role`,
   `content`, optional images, optional tool calls). The recipe is serialized
...
Recipes are policy-configuration dictionaries validated by `TrainingRecipe`
and `MessageTurn` from `lerobot.datasets.recipe`. They declare which annotation
rows to pull (via `bindings`) and how to compose them into chat turns
(`messages`).
...
index. Define the blend in the policy's `recipe` dictionary so the exact
language contract is stored with each checkpoint.
