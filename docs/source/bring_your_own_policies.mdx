...
The policy config stores `recipe: dict | None`. `RenderMessagesStep` validates
that dictionary as a `TrainingRecipe` from `lerobot.datasets.recipe` before
rendering. A recipe enables recipe training, and `None` makes the training
renderer a no-op. Keep the recipe in the policy config so it is serialized with
the model and does not depend on a separate local file. Recipe-backed rendering
requires the `dataset` extra because recipes currently live under
`lerobot.datasets`.
