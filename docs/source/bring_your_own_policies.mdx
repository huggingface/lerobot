...
```python
input_steps = [RenderRuntimeMessagesStep(config.recipe)]
if config.use_language_recipe:
    input_steps.append(RenderTrainingMessagesStep(config.recipe))
input_steps += [
...
runtime queries and processes samples with raw annotations or action targets;
ordinary observation-only inputs pass through. Annotated text-only training is
supported without an action tensor. Training without annotations needs action
targets to enable the task fallback.

The shared factory only constructs or loads processors. Fresh training and
fine-tuning construct them from the active config and dataset statistics;
resume and inference load saved pipelines, including their recipe and statistics.
Fresh factories derive normalization and relative-action settings themselves.
The training script supplies the live device and retains the rename map without
reconstructing processor steps.
