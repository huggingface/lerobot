...
| `query_kind`   | Caller-owned `query_text`        | Recipe behavior                                      | Result                     |
| -------------- | -------------------------------- | ---------------------------------------------------- | -------------------------- |
| `vqa`          | a question about the observation | no recipe wording; preserve text as the user payload | best-effort generated text |
| `next_subtask` | a high-level task or instruction | render the checkpoint recipe before `${subtask}`     | next executable subtask    |
...
request = {
    **observation,
    "query_kind": "vqa",
    "query_text": "Which cup is closest to the gripper?",
}
batch = preprocessor(request)
answer = policy.generate_text(batch)
```

`vqa` does not claim the checkpoint was trained for that question and does not require a
recipe. The policy attempts generation whenever it has a text head; a poor or empty
answer is allowed. The text still passes through policy-native multimodal formatting,
including system messages, roles, image markers, special tokens, and tokenization.

For recipe-backed subtask generation, runtime selects `next_subtask` in complementary
data and explicitly decides whether to commit the result before predicting actions:
...
# Empty text leaves the active runtime task unchanged.
if subtask:
    runtime_state.set_task(subtask)

batch = preprocessor({**observation, "task": runtime_state.task})
action_chunk = policy.predict_action_chunk(batch)
```

The message-rendering step consumes `query_kind` and `query_text`; they do not leak into model
inputs. Inputs without `query_kind` pass through unchanged, so the same preprocessor
...
Text-capable policies explicitly place one `RenderMessagesStep` in their normal input
pipeline before batching or policy-native message formatting. The step owns both runtime
queries and recipe-training annotations: `render_training` enables raw dataset rendering,
while runtime `query_kind` plus `query_text` works in either mode. Existing semantic `messages`
pass through unchanged. Subsequent policy-specific steps apply the model-native chat
template, image markers, delimiters, and tokenization. The policy class implements only
text decoding from the already-prepared batch:
...
input_steps = [
    ...
    steps.add_batch_dim,
    MyPolicyMessageFormatter(),
    steps.normalize,
    steps.to_device,
]
```

The unified message step must also be present in the processor configuration shipped
with a text-capable checkpoint. Policies supporting older processor configurations
should add a policy-local migration when loading them rather than changing all policy
pipelines.
...
```text
runtime adds query_kind + query_text
              ↓
RenderMessagesStep
              ↓
...
The same step renders complete training recipe messages, including assistant targets.
Subtask inference uses the same turn-rendering and substitution primitive but stops
immediately before the assistant turn supervising `${subtask}`. Runtime never reads
`config.recipe`, selects blend components or bindings, or constructs model prompts.
...
The runtime selector is intentionally restricted to `vqa` and `next_subtask`; recipe
binding names such as `subtask` are not runtime kinds. Structured outputs such as
bounding boxes should keep their own typed interface. Existing policy PRs that override
`generate_text(batch, prompt)` should move prompt formatting into their policy input
processor and override `generate_text(processed_batch)` directly.
