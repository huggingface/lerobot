...
The policy API above is sufficient for training and standard evaluation. Interactive `lerobot-rollout` adds one optional method: `generate_text`. Implement it and your policy gains high-level subtask generation, `/ask` grounded VQA, and the interactive panel. Skip it and the policy still runs — on the operator's own instruction.

There is nothing to register. The runtime owns the tick loop, generation cadence, pause/resume, action queues, and dispatch; the policy only turns an observation into text.

During a rollout, `RuntimeState` holds the operator's task and the active language context:

```text
task ──> policy.generate_text(kind="subtask") ──> state.language_context["subtask"]
                                                       │
observation ──> processors ──> policy.predict_action_chunk() ──> action chunk ──> robot
```

### Policies without a text head

Nothing to do. The observation provider writes the active instruction into the batch before your preprocessor runs, so `predict_action_chunk` already sees it. PI0.5 and MolmoAct2 work this way:
...
`--direct_subtask` makes the operator's typed text the instruction the action expert consumes. It is implied for any policy without a text head.
...
That one method covers everything: VQA and `/ask`, captioning, grounding, and subtask prediction. Subtask prediction is not a separate method — the runtime composes the prompt itself and calls `generate_text` with the result.

### Registering prompt templates

Declare the prompts your checkpoint was trained to answer in a `PROMPT_TEMPLATES` dict, keyed by kind:
...
Lookups fall back to the base defaults, so you list only the kinds you change. The base ships `"subtask"` with the wording the WALL-OSS and EO-1 family were trained on, which more released checkpoints recognise than a sentence written for the occasion — so a policy with a text head predicts subtasks with no extra code at all.

Declare your own anyway when the wording matters. For a model that declares its mode in this very turn, the exact phrasing is the difference between an in-distribution answer and a guess, down to the trailing newline. And a policy that pins its own template will not silently follow if the default is retuned later.

Callers fill a template through `build_prompt`, which substitutes with `str.replace` rather than `str.format` — templates carry chat-control tokens, and a task may contain braces that `format` would choke on:
...
Adding a kind is a dict entry and nothing else. A `"memory"` prompt taking two values needs no change to the base class or the runtime:

```python
PROMPT_TEMPLATES = {"memory": "Given the goal {task}, summarise what is already done: {history}"}
```

| Piece                          | Owned by   | Used for                                  |
| ------------------------------ | ---------- | ----------------------------------------- |
| `PROMPT_TEMPLATES`             | the policy | the trained wording, per kind             |
| `build_prompt(kind, **values)` | the base   | filling a template                        |
| `generate_text(batch, prompt)` | the policy | every text call, subtask and `/ask` alike |
| the values                     | the caller | the high-level goal, a question, history  |

The batch is a preprocessed observation with `batch["task"]` set to the operator's goal and `batch["subtask"]` set to the active subtask once one exists. Return `""` when the head produces nothing — the runtime keeps the previous subtask and counts it in the panel diagnostics.
...
The `text_` prefix keeps them distinct from action-sampling settings such as `PI0FastConfig.temperature`. Override them per run through the usual policy path — no runtime-specific flags:

```bash
lerobot-rollout --language --policy.path=... --policy.text_temperature=0.5
```
...
The runtime asks for actions through `predict_action_chunk(batch, with_text=True)`. There is no second method: `with_text` asks for the text your policy generated **inside that pass**, returning `(chunk, text)` instead of the bare chunk.
...
The runtime panel shows the text on a `reasoning` line. It is display-only: unlike a generated subtask it never re-enters the observation batch, so it cannot replace the operator's task. Returning the pair rather than caching it on the policy is what makes that safe — the runtime commits text and chunk together under one lock, so a chunk invalidated mid-inference takes its text with it.
...
- Return a full action chunk from `predict_action_chunk`; the runtime handles control-rate dispatch.
- Keep optional model dependencies inside lazy imports.
- Test it with a lightweight fake batch: the method is stateless, so it needs no runtime to exercise.
