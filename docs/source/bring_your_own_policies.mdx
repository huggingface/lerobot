...
class MyCustomPolicyConfig(PreTrainedConfig):
    ...
    def get_scheduler_preset(self):
        return CosineDecayWithWarmupSchedulerConfig(peak_lr=self.optimizer_lr)
...
class MyCustomPolicy(PreTrainedPolicy):
    ...
    def reset(self, done: torch.Tensor | None = None):
        ...
        `done` is a bool tensor of shape (B,) for vectorised environments. When
        provided, only reset the environments where `done[b]` is True.
        """
        ...
