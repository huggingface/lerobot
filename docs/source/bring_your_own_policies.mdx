...
The prompt-rendering step consumes `query_kind` and `text`; they do not leak into model
...
Text-capable policies explicitly place `RenderGenerationPromptStep(config.recipe)` in
their normal input pipeline before batching or policy-native message formatting. This
keeps its ordering and serialization policy-owned; the generic policy factory does not
mutate processor pipelines. Subsequent policy-specific steps apply the model-native chat
...
The generation step must also be present in the processor configuration shipped with a
text-capable checkpoint. Policies supporting older processor configurations should add
a policy-local migration when loading them rather than changing all policy pipelines.
