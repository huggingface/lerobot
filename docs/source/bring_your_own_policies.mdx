...
### Reasoning emitted with the action

Some policies produce reasoning and an action from a single inference pass rather than a separate planning call. Expose it by overriding `last_reasoning`:
...
The runtime panel shows it on a `reasoning` line. It is display-only: unlike a generated subtask it never re-enters the observation batch, so it cannot replace the operator's task.
