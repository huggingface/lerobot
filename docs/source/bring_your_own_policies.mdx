...
- `RenderTrainingMessagesStep` resolves annotated samples and emits
  `messages_rendered`, `message_streams`, and `target_message_indices`. It remains
  a general `ProcessorStep` because dropping sparse samples also filters their
  observations and actions.
- `RenderRuntimeMessagesStep` is a `ComplementaryDataProcessorStep`. It renders
  text-generation requests and leaves ordinary action inputs unchanged.

The policy config exposes `use_language_recipe: bool`, `recipe_path: str | None`,
and the resolved `recipe: TrainingRecipe | None` (from `lerobot.language.recipe`). Serialize the resolved recipe
in the config and processor so deployment does not depend on a local YAML path.

Put the renderer before policy-native chat formatting and tokenization:

```python
input_steps = [
    (RenderTrainingMessagesStep(config.recipe)
     if config.use_language_recipe else RenderRuntimeMessagesStep(config.recipe)),
...
`make_pre_post_processors(..., for_training=True)` preserves the training
renderer chosen by the policy factory; the default replaces training renderers
with runtime renderers. Policies with recipe training disabled retain their
runtime-only step. When loading a checkpoint, the factory
changes only the renderer and keeps its saved recipe and the other saved steps,
including normalization statistics. Resume passes `for_training=True`.
Fresh training and fine-tuning build processors from the active config and
training dataset statistics; only resume and inference load saved pipelines.
Fresh factories derive normalization and relative-action settings themselves,
and receive device and observation-renaming overrides from the training script.
