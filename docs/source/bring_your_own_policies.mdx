...
- [ ] `policies/__init__.py` re-exports the config under `if LAZY_IMPORTS:`, and the `register_subclass` name matches the package directory (the policy registers on first use, by that name; the factory resolves modeling/processor by naming convention).
