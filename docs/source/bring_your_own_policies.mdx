...
Decoding parameters live on the checkpoint, in [`PreTrainedConfig.generation`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/configs/policies.py), so a head decodes the way it was trained:

```python
gen = self.config.generation  # min_new_tokens, temperature, top_p
```

Override them per run through the usual policy path — no runtime-specific flags:

```bash
lerobot-rollout --language --policy.path=... --policy.generation.temperature=0.5
```

Temperature `0.0` is greedy argmax and matches most training setups. Raising `min_new_tokens` to 3-5 is a useful probe for an under-trained head whose prior at position 0 still favours EOS.
