...
That one method covers everything: VQA and `/ask`, captioning, grounding, and subtask prediction. Subtask prediction is not a separate method — the runtime composes the prompt itself, by filling your `subtask_prompt_template` with the operator's high-level goal and calling `generate_text` with the result.

The base class ships a default, so a policy with a text head can predict subtasks with no extra code:

```python
"{task}\nPredict the next action in language."
```

That is the wording the WALL-OSS and EO-1 family were trained on, which more released checkpoints recognise than a generic English sentence would.

Override it whenever your checkpoint was trained differently. For a model that declares its mode in this very turn, the exact phrasing is the difference between an in-distribution answer and a guess — down to the trailing newline:
...
Override it explicitly even when your wording matches the default. The default may be retuned later, and a policy that pins its own template will not silently follow.

The runtime substitutes with `str.replace`, not `str.format`, so a template may contain chat-control tokens and an operator's task may contain braces.

| Piece                          | Owned by     | Used for                                  |
| ------------------------------ | ------------ | ----------------------------------------- |
| `subtask_prompt_template`      | the policy   | filled with `{task}` by the runtime       |
| `generate_text(batch, prompt)` | the policy   | every text call, subtask and `/ask` alike |
| the high-level goal            | the operator | substituted into the template             |
