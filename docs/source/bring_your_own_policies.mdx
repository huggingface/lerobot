...
The base class ships a generic default, so a policy with a text head can predict subtasks with no extra code:

```python
"predict next subtask, given this high level goal: {task}"
```

Override it whenever your checkpoint was trained on specific wording. That phrasing will beat the generic one, and for a model that declares its mode in this turn — WALL-OSS answers "Predict the next action in language." differently from anything else — it is the difference between an in-distribution answer and a guess:
