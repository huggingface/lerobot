...
The policy API above is sufficient for training and standard evaluation. Language-conditioned policies can additionally expose a **text head** — for high-level subtask generation, grounded VQA, or captioning — through one optional method, `generate_text`, plus the recipe declaring the wording the checkpoint was trained on. Deployment tooling drives a policy exclusively through this contract; a policy without a text head still runs, on the operator's own instruction.

### Policies with a text head

Implement one method, `generate_text`, which answers an arbitrary prompt about the current observation:

```python
# modeling_my_policy.py
class MyPolicy(PreTrainedPolicy):
    def generate_text(self, batch: dict[str, Tensor], prompt: str) -> str:
        """Answer `prompt` about the current observation."""
        return self._decode(prompt, batch)
```

That one method covers everything: VQA, captioning, grounding, and subtask prediction. Subtask prediction is not a separate method — the caller composes the prompt from the policy's recipe and calls `generate_text` with the result.

### The recipe is the wording contract

Training renders language supervision through a [`TrainingRecipe`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/configs/recipe.py) — chat-style message turns whose `${placeholder}` bindings are filled per sample. That same recipe ships with the checkpoint as `config.recipe` in `config.json`, and inference replays it: `prompt_messages(kind, **values)` returns the turns preceding the recipe's target turn for a _kind_ (the binding that target supervises, e.g. `subtask`), filled with the caller's values.
...
The runtime supplies the live values; the checkpoint's recipe supplies the wording. A
subtask-and-action loop therefore does not contain a policy-specific prompt template:

```python
task = "clear the table"
batch["task"] = task

# Replays the recipe turns before the assistant turn targeting `${subtask}`.
prompt = policy.build_prompt("subtask", task=task)
subtask = policy.generate_text(batch, prompt).strip()

# Empty text means that the runtime keeps its previous subtask.
if subtask:
...
For VQA or another free-form query, the caller already owns the complete prompt and
passes it directly:

```python
answer = policy.generate_text(batch, "Which cup is closest to the gripper?")
```

The runtime always calls the same `generate_text(batch, prompt)` surface. The policy
implementation owns its tokenizer and any native chat template; it must not require the
runtime to know policy-specific role tokens or accept a separate `kind=` argument.

`build_prompt` flattens the turns into one string; a policy applying a real chat template reads `prompt_messages` internally, which preserves the roles. Substitution uses `str.replace` rather than `str.format` — turns carry chat-control tokens, and a task may contain braces that `format` would choke on.

Because training and inference read the same turns, the wording cannot drift. For a model that declares its mode in this very turn, the exact phrasing is the difference between an in-distribution answer and a guess — down to the newline.

A checkpoint without a recipe falls back to `DEFAULT_LANGUAGE_RECIPE`, the subtask wording the WALL-OSS and EO-1 family were trained on, which more released checkpoints recognise than a sentence written for the occasion. Declare your own anyway: the default may be retuned, and a checkpoint that pins its recipe will not silently follow. Make it your config's default so training and deployment share one source:
...
Adding a kind is a recipe component and nothing else. A blend component whose target turn supervises `${current_memory}` makes `build_prompt("current_memory", task=..., completed_subtask=...)` work with no change to the base class.

| Piece                              | Owned by       | Used for                                 |
| ---------------------------------- | -------------- | ---------------------------------------- |
| `config.recipe`                    | the checkpoint | the trained wording, per kind            |
| `prompt_messages` / `build_prompt` | the base       | replaying the recipe's prompt turns      |
| `generate_text(batch, prompt)`     | the policy     | every text call, subtask and VQA alike   |
| the values                         | the caller     | the high-level goal, a question, history |

The batch is a preprocessed observation with `batch["task"]` set to the operator's goal and `batch["subtask"]` set to the active subtask once one exists. Return `""` when the head produces nothing — callers keep the previous subtask.
