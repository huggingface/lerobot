...
subtask = policy.generate_text(batch, prompt)
```

`build_prompt` flattens the turns into one string; a policy applying a real chat template reads `prompt_messages` instead, which preserves the roles. Substitution uses `str.replace` rather than `str.format` — turns carry chat-control tokens, and a task may contain braces that `format` would choke on.
