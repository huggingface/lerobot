...
Override it when your policy has text to attribute to the chunk. Policies that produce reasoning and an action from a single inference pass return the text generated in that pass:
...
Policies whose text head runs as its own pass return the subtask the chunk was conditioned on, which the runtime supplies in `batch["subtask"]`:

```python
def predict_action_chunk_with_text(self, batch, **kwargs) -> ActionChunkPrediction:
    return ActionChunkPrediction(
        action=self.predict_action_chunk(batch, **kwargs),
        text=batch.get("subtask") or None,
    )
```

Never run an extra generation pass here — a caller wanting fresh text calls `generate_text`.
