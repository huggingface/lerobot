...
Implement two methods next to `predict_action_chunk`. They are split because they need different things from the caller:

```python
# modeling_my_policy.py
class MyPolicy(PreTrainedPolicy):
    def predict_subtask(self, batch: dict[str, Tensor]) -> str:
        """The next instruction to condition actions on — your trained template."""
        return self._decode(self._subtask_prompt(batch.get("task", "")), batch)

    def generate_text(self, batch: dict[str, Tensor], prompt: str) -> str:
        """Answer an arbitrary question about the current observation."""
        return self._decode(prompt, batch)
```

`predict_subtask` takes no prompt on purpose. The subtask wording is part of the model contract — WALL-OSS declares its mode in that turn, and drifting from the trained phrasing is answered out of distribution. A caller cannot know it, so it is not a caller's parameter.

`generate_text` takes the whole prompt, which covers VQA, captioning, grounding and anything else the checkpoint can answer. The runtime calls it for `/ask`.

| Method                         | Caller supplies | Used for                                    |
| ------------------------------ | --------------- | ------------------------------------------- |
| `predict_subtask(batch)`       | nothing         | `state.language_context["subtask"]`         |
| `generate_text(batch, prompt)` | the full prompt | the `/ask` command, printed to the operator |
...
The runtime asks for actions through `predict_action_chunk_with_text`, which returns `(action_chunk, text)`. The default implementation on the base class calls your `predict_action_chunk` and reports `None` for the text, so a policy without a text head needs nothing.

Override it **only** if your policy emits text and actions from one inference stream, where the action is conditioned on the text rather than merely accompanied by it:

```python
def predict_action_chunk_with_text(self, batch, **kwargs) -> tuple[Tensor, str | None]:
    action, metadata = self._run_inference(batch)
    return action, first_text(metadata)
```

If your text head runs as its own prefill, do not override this to echo `batch["subtask"]`. That string is already runtime state, and returning it here would claim a causal link inside this pass that does not exist. Expose `generate_text` and leave the default alone.

The override must not run an extra generation pass. Whether text appears is a property of the checkpoint and its configuration, so the call costs no more than `predict_action_chunk` — a caller wanting fresh text on demand calls `generate_text`.
