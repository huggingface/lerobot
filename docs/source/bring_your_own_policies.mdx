...
Policies with a text head expose one flat method. Runtime and client code passes an
already-preprocessed observation batch, caller-owned natural language, and one of two
prompt templates:

| Public template | Caller-owned `text`              | Recipe behavior                                      | Result                     |
| --------------- | -------------------------------- | ---------------------------------------------------- | -------------------------- |
| `raw` (default) | any question or free-form prompt | no recipe wording; preserve text as the user payload | best-effort generated text |
| `subtask`       | a high-level task or instruction | render the checkpoint recipe before `${subtask}`     | next executable subtask    |

For an arbitrary question, use the ergonomic raw default:

```python
answer = policy.generate_text(
    batch,
    "Which cup is closest to the gripper?",
)
```

`raw` does not claim the checkpoint was trained for VQA and does not require a VQA
recipe. The policy attempts generation whenever it has a text head; a poor or empty
answer is allowed. Raw text still passes through policy-native multimodal formatting,
including system messages, roles, image markers, special tokens, and tokenization.

For recipe-backed subtask generation, the runtime selects the public `subtask` mode and
then explicitly decides whether to commit the result before predicting actions:

```python
task = "clear the table"
subtask = policy.generate_text(batch, task, template="subtask").strip()
...
`generate_text` itself does not mutate `batch`, replace the active task, enqueue actions,
or run a System-2/System-1 loop. A checkpoint without a valid subtask recipe raises a
contract error instead of silently treating the request as raw.

### Policy implementation

The base class owns public prompt selection and recipe rendering. A policy implements
protected hooks for the parts only its model can know:

```python
# modeling_my_policy.py
class MyPolicy(PreTrainedPolicy):
    def _format_text_generation_input(self, semantic_inputs):
        ...
        return self._tokenize_multimodal(prompt, semantic_inputs)

    def _generate_preprocessed_text(self, model_inputs) -> str:
...
The internal text preprocessor is deliberately separate from the ordinary observation
preprocessor. It consumes the existing batch without repeating image or state
normalization, renders role-preserving semantic messages, and then hands them to the
policy formatter:
...
before the assistant turn supervising `${subtask}`. Callers never read `config.recipe`,
select blend components or bindings, or construct model prompts.
...
The public selector is intentionally restricted to `raw` and `subtask`; recipe binding
names are not public templates. Structured outputs such as bounding boxes should keep
their own typed interface. Existing policy PRs that override `generate_text(batch,
prompt)` should migrate their model formatting into `_format_text_generation_input` and
their decoding into `_generate_preprocessed_text`.
