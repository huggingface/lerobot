...
### RTC Rollout With Original MolmoAct2 Weight

Use `policy.type=molmoact2` with `policy.checkpoint_path` for RTC rollout from
an original MolmoAct2 HF checkpoint.
...
### Async Inference With Original MolmoAct2 Weight

For async inference, start the policy server normally, then pass the original HF
checkpoint path and the required MolmoAct2 config values from the robot client.
...
python -m lerobot.async_inference.robot_client \
  ...
  --policy_config_overrides='{"norm_tag":"libero","inference_action_mode":"continuous"}' \
  --policy_device=cuda \
...
- `policy.inference_action_mode`: defaults to `continuous` for flow-matching
  inference. Set this to `discrete` for action-token inference. It must be
