...
Gradient accumulation lets you simulate a larger effective batch size without increasing GPU memory usage. Instead of processing all samples in one forward pass, it splits the batch into smaller micro-batches and accumulates gradients before updating.
...
This gives an effective batch size of 16 × 2 = 32, while only using memory for batch size 16.

### `compile_mode` Compatibility

When using `gradient_accumulation_steps > 1` with `compile_model=true`, CUDAGraphs-based compile modes (`max-autotune`, `reduce-overhead`) are incompatible because multiple forward passes before backward overwrite tensors captured in the graph.

LeRobot handles this automatically. Each policy declares its own preferred and safe compile modes:

| Policy | `DEFAULT_COMPILE_MODE` | `SAFE_COMPILE_MODE` (used with grad. accumulation) |
|---|---|---|
| `pi0`, `pi05`, `pi0_fast`, `smolvla` | `max-autotune` | `max-autotune-no-cudagraphs` |
| `diffusion` | `reduce-overhead` | `default` |

Resolution rules:

| `compile_mode` | `gradient_accumulation_steps > 1` | Behavior |
|---|---|---|
| Not set (default) | No | Policy's `DEFAULT_COMPILE_MODE` |
| Not set (default) | Yes | Policy's `SAFE_COMPILE_MODE` (logged as a warning) |
| `max-autotune` or `reduce-overhead` | Yes | `ValueError` — pick a non-CUDAGraphs mode |
| `max-autotune-no-cudagraphs` or `default` | Yes | Used as-is |

### Combining with Multi-GPU

Gradient accumulation works with multi-GPU training. The effective batch size becomes `batch_size × num_gpus × gradient_accumulation_steps`:
