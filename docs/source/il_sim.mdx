...
To use `gym_hil` with LeRobot, you need to use a configuration file. An example config file is stored under `lerobot/examples/configs/env_config_gym_hil_il.json`.
...
By default the config file assumes you use a controller. To use your keyboard please change `"task"` in config file to `"PandaPickCubeKeyboard-v0"`.
...
```bash
python lerobot/scripts/rl/gym_manipulator.py --config_path examples/configs/env_config_gym_hil_il.json
```
...
```bash
mjpython lerobot/scripts/rl/gym_manipulator.py --config_path examples/configs/env_config_gym_hil_il.json
```
...
To evaluate your policy we have to use the config file that can be found here: `lerobot/examples/configs/eval_config_gym_hil.json`
...
```bash
python lerobot/scripts/rl/eval_policy.py --config_path=examples/configs/eval_config_gym_hil.json
```
...
```bash
mjpython lerobot/scripts/rl/eval_policy.py --config_path=examples/configs/eval_config_gym_hil.json
```
