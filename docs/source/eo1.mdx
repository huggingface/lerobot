...
schema and recipe structure. EO-1 stores its recipe in the policy
configuration, so checkpoints retain the language contract they were trained
with.
...
| `policy.recipe`                        | built-in                      | Defines recipe-driven text and per-sample action supervision            |
