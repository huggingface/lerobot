...
EO-1 exposes the Qwen backbone's native text generation using the same batched
images and task field used for action inference:

```python
answers = policy.generate_texts(
    batch,
    kind="vqa",
    user_text="Which object is closest to the gripper?",
)
```

Use `kind="caption"` for a scene description, `kind="grounding"` for a spatial
query, or `kind="subtask"` for a language-level next action. Text is generated
by EO-1 itself; no separate VLM is loaded.
...
Use `/ask <question>` for image-grounded VQA or `--direct_subtask` to bypass
subtask generation and condition actions directly on the supplied task.
...
Set `recipe_path` to train EO-1 from LeRobot's optional language columns. The
joint subtask recipe supervises a selected assistant response and its
continuous action chunk in the same sequence:
...
padding are excluded from text loss. Without `recipe_path`, EO-1 keeps its
