...
- It latches its engage origin on the squeeze **engage edge** (the frame the squeeze first crosses
  ...
  teleport the arm. The position delta is scaled by `clutch_position_scale` (orientation stays
  1:1). `Clutch.rebase` returns the absolute base-frame target as a `(pos, quat)`
