...
`SO101EngageRelativeClutchRetargeter`, so it emits an absolute base-frame **EE pose** rather than a
...
Isaac Teleop's `SO101EngageRelativeClutchRetargeter` runs inside `XRController`'s retargeting
...
```text
  ...
  XRController.get_action()          ── absolute base-frame ee_pose (7) + trigger + engaged
        ...
        │   SO101EngageRelativeClutchRetargeter — engage-relative delta on the EE home, pos+orient)
...
- **`ModuleNotFoundError: isaacteleop`** — the `isaacteleop` package is not installed in the
  ...
  `uv pip install "isaacteleop[cloudxr,retargeters-lite]~=1.4.0"`.
