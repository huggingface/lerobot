...
`IsaacTeleopTeleoperator`). The devices available today are the **XR controller**
(`XRController`) and a back-drivable **SO-101 leader arm** (`SO101LeaderArm`); Manus gloves and
hand/full-body tracking are the natural next devices. This guide focuses on the XR controller; the
SO-101 leader is summarized under [Run the example](#step-3-run-the-example).
...
Isaac Teleop support is the `isaac-teleop` extra. `isaacteleop` is published on public PyPI, so a
plain install picks it up. Both forms below pull `isaacteleop[cloudxr,retargeters]~=1.3.0` — the
...
install the extra in editable mode:
...
`uv sync --locked` does **not** install this extra, so use the explicit `uv pip install -e` above.
You can also install the underlying package on its own, which is handy when developing against a
local Isaac Teleop checkout (see [Build Isaac Teleop from
source](https://nvidia.github.io/IsaacTeleop/main/getting_started/build_from_source/index.html)):
...
`uv sync --locked` will **not** pull the `isaac-teleop` extra. Install it explicitly with `uv pip
...
The example loop's `Clutch` class (in `examples/isaac_teleop_to_so101/teleoperate.py`) mirrors
Isaac Teleop's `SO101ClutchRetargeter`, but lives in-loop so the device can stay a thin reader:
...
- It latches its engage origin on the squeeze **engage edge** (the frame the squeeze first crosses
  ...
  teleport the arm. `Clutch.rebase` returns a 7D absolute `ee_pose` (position + grip quaternion) in
  the base frame.
...
teleport on engage. It seeds its home from the `home_base_T_ee` transform the example computes by
forward kinematics on the arm's **measured joints** right after startup — so the seeded home equals
where the arm already is, the loop never commands the arm at startup, and the first engage is
jump‑free. On a mid‑task re‑clutch it latches a fresh home from the last commanded pose.
...
back-drive the arm to the pose you want and run `python override_reset_pose.py` (in the example
folder); it writes the current joints to `reset_pose.json`, which then takes priority over the
built-in default on the next run. That file is git-ignored (user-local), so your override stays on
your machine, and both `teleoperate.py` and `record.py` honor it.
...
(Note `so101_leader` here is the *Isaac* leader, resolved against the Isaac Teleop device
...
warns and the plugin uses built-in defaults. Run `python teleoperate.py --help` for all flags. Its
...
- **`ModuleNotFoundError: isaacteleop`** — the `isaac-teleop` extra is not installed (note `uv sync
  --locked` does not pull it). Re-run the install command at the top of this guide: `uv pip install
  'lerobot[isaac-teleop]'`.
