...
back-drive the arm to the pose you want and run `python override_reset_pose.py --id <robot.id>`
(in the example folder); it writes the current joints to a per-arm file in the LeRobot cache
...
the repo), your override stays on your machine, and both `teleoperate.py` and `record.py` honor it
