...
With `--inference.type=sync` (default) the new instruction takes effect on the very next tick. With `--inference.type=rtc`, it takes effect once the currently in-flight action chunk drains — a bounded lag of roughly `execution_horizon / fps` seconds, not a bug. If stdin is not a TTY (e.g. piped input, some CI/headless setups), dynamic updates are disabled and the rollout runs with the fixed `--task` instruction only.
