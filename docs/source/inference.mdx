...
**How `/autosteer` works.** You give the policy a goal instead of an instruction, and every `--autosteer_interval_s` seconds (default 10) the engine asks it for the next subtask and applies that through the same path as `/subtask`. So RTC blends the switch into the next chunk and sync drops precomputed actions, exactly as with a hand-typed instruction. Successful subtasks are not announced separately — the task itself is what tells you the robot's intent, so watch it with `/subtask`.
