...
Select a strategy with `--strategy.type=<name>`. Each strategy defines a different control loop with its own recording and interaction semantics. The strategy registry is open: the built-ins below are the ones LeRobot ships, and an installed third-party package can register its own strategy under the same flag — see [Bring Your Own Rollout Strategy](./bring_your_own_rollout_strategies).
...
### Third-party strategies (`--strategy.type=<custom>`)

`--strategy.type` accepts any strategy registered in the environment, not only the five above. Install a package named `lerobot_strategy_<name>` (it is imported automatically before the CLI is parsed), or point `--strategy.discover_packages_path=<package>` at a package with any name, and its strategy becomes selectable exactly like a built-in:
...
Everything documented on this page that is not specific to a built-in — dataset creation, `--teleop.type` and `--interactive` validation, the inference backends, cadence reporting — is driven by the capabilities the strategy's config declares, so it applies to custom strategies unchanged. See [Bring Your Own Rollout Strategy](./bring_your_own_rollout_strategies) for the naming rules, the capability declarations and the `run()` contract.
...
`--interactive=true` is validated against that declaration rather than against a fixed list of strategies: it works with any strategy — built-in or third-party — whose config declares `supports_interactive`, which means honouring the restartable-`run()` contract (see [Bring Your Own Rollout Strategy](./bring_your_own_rollout_strategies)). Today the built-ins that declare it are `base` and `sentry`. The other built-in recording strategies (episodic, DAgger, highlight) do not: they bind their own keyboard controls and finalize the dataset when `run()` exits, so they cannot be restarted segment by segment.
...
| `--interactive`                   | Chat session on stdin; robot idles until `/start`. Needs a strategy declaring `supports_interactive` (built-ins: base, sentry) | false   |
