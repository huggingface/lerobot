...
Two notes for [interactive sessions](#interactive-sessions). Each `/start` segment owns its own timer, so a session reports once per segment rather than once per process — and because these summaries are logged at INFO while the session mutes everything below WARNING, none of them reach the console. Run without `--interactive` to read them. The slow-loop warning still shows, except on a tick that answered a `/vqa` or `/autosteer` query: inline text generation is far slower than a control tick and is _expected_ to overrun, so that tick is exempted rather than warned about.
