...
The `examples/rac` HIL scripts require **teleoperators with active motors** that can:

- Enable/disable torque programmatically
- Move to target positions (to mirror the robot state when pausing)

**Compatible teleoperators in the current `examples/rac` scripts:**

- `openarm_mini` - OpenArm Mini

> [!IMPORTANT]
> The provided `examples/rac` commands are wired to `bi_openarm_follower` + `openarm_mini`.
> The scripts do not import/register SO100 configs by default.
...
- **RaC** (Hu et al., 2025) scales this loop to long-horizon tasks by explicitly decomposing interventions into **recovery** (teleoperating back to a good state) and **correction** (demonstrating the right behavior from there). This decomposition is the protocol followed by the HIL scripts in `examples/rac`.

- **RECAP** (Physical Intelligence, 2025) applies the same iterative collect-and-finetune loop at scale with VLA models, showing that even large pretrained policies benefit substantially from targeted human corrections on their own failure modes.
