...
With `--interpolation_multiplier=N`, the robot receives `N` interpolated commands per policy action (control runs at `fps × N` Hz) while policy inference and dataset recording stay at `--fps`, so the recorded dataset matches its declared FPS.
