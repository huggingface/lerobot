...
It combines three key ingredients:
	...
	3.	**Safety & efficiency tools:** joint/end-effector (EE) bounds, impedance control, crop region of interest (ROI) preprocessing and WandB monitoring keep the data useful and the hardware safe.
...
## 0. Install LeRobot with HIL-SERL
...
Create a configuration file for recording demonstrations (or edit an existing one) that can be found here: `lerobot/examples/rl_config/env_config_so100.json`.
...
Start the recording process, an example of the config file can be found here: `lerobot/examples/rl_config/env_config_so100.json`.

```bash
python lerobot/scripts/rl/gym_manipulator.py --config_path examples/rl_config/env_config_so100.json
```
...
Create a training configuration file (example available here: `lerobot/examples/rl_config/train_config_hilserl_so100.json`). The training config is based on the main `TrainRLServerPipelineConfig` class in `lerobot/configs/train.py`.
...
```bash
python lerobot/scripts/rl/learner.py --config_path examples/rl_config/train_config_hilserl_so100.json
```
...
```bash
python lerobot/scripts/rl/actor.py --config_path examples/rl_config/train_config_hilserl_so100.json
```
...
Example configuration for training the reward classifier can be found here: `lerobot/examples/rl_config/reward_classifier_train_config.json`:
...
```bash
python lerobot/scripts/train.py --config_path examples/rl_config/reward_classifier_train_config.json
```
