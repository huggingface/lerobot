...
Add `--private true` to create the Hub repository as private. This only applies when the repository is created: it does not change the visibility of an existing one.
