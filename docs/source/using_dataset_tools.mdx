...
- `feature_names`: List of names for the new features to add (e.g., `["observation.images.side", "action.new"]`)
...
- For numerical data (.safetensors files), each array in the file must have shape `(frames, ...)` where `frames` matches the dataset's total frames. If the safetensors file contains multiple keys, each key becomes a separate feature (optionally prefixed with the corresponding feature_name if provided).
