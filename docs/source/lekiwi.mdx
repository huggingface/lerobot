...
python -m lerobot.robots.lekiwi.lekiwi_host \
            --port=/dev/tty.usbmodem58FA1027991 \ # <- The port of your robot
            --id=lekiwi_follower \
            --connection_time_s=300 \
            --cameras="{ front: {type: opencv, index_or_path: 0, width: 640, height: 480, fps: 30}, wrist.right: {type: opencv, index_or_path: 1, width: 640, height: 480, fps: 30}}" \
            --id=my_awesome_lekiwi
```
...
python -m lerobot.teleoperate \
        ...
        --robot.remote_ip="127.0.0.1" \# <- The IP address of your machine which running host.
        --robot.id=lekiwi_follower \
        --robot.cameras="{ front: {type: opencv, index_or_path: 0, width: 640, height: 480, fps: 30}, wrist.right: {type: opencv, index_or_path: 1, width: 640, height: 480, fps: 30}}" \
        --teleop.type=lekiwi_leader \
        --teleop.port=/dev/tty.usbserial-15140 \ # <- The port of your leader arm.
        --teleop.id=lekiwi_leader \
        --display_data=true
```

You should see on your laptop something like this: `INFO 2025-08-31 10:04:20 eoperate.py:137 {'display_data': True,` Now you can move the leader arm and use the keyboard (w,a,s,d) to drive forward, left, backwards, right. And use (z,x) to turn left or turn right. You can use (r,f) to increase and decrease the speed of the mobile robot. There are three speed modes, see the table below:
...
> [!TIP]
> If you use a different keyboard, you can change the keys for each command with `--teleop.teleop_keys`.
...
Now you can record a dataset. To record episodes and upload your dataset to the hub, execute below command on your client after running host command on LeKiwi. Make sure to first adapt the `remote_ip`, `repo_id`, `port` and `task` in the options. If you would like to run the script for longer you can increase `NB_CYCLES_CLIENT_CONNECTION`.

```bash
python -m lerobot.record \
    --robot.type=lekiwi_client \
    --robot.remote_ip="127.0.0.1" \ # <-The IP address of your machine which running host.
    --robot.id=lekiwi_folower \
    --robot.cameras="{ front: {type: opencv, index_or_path: 0, width: 640, height: 480, fps: 30}, wrist.right: {type: opencv, index_or_path: 1, width: 640, height: 480, fps: 30}}" \
    --teleop.type=lekiwi_leader \
    --teleop.port=/dev/tty.usbserial-15140 \ # <- The port of your leader arm.
    --teleop.id=lekiwi_leader \
    ...
    --dataset.single_task="Move to front of block and pick up it." \
    --dataset.repo_id=${HF_USER}/lekiwi_data \
    --display_data=true \
    --resume=false
```
...
To replay an episode run the command example below, make sure to change `remote_ip`, `port`, LeRobotDatasetId and episode index.

```bash
python -m lerobot.replay \
    --robot.type=lekiwi_client \
    --robot.remote_ip="127.0.0.1" \
    --robot.id=lekkiw_follower \
...
To evaluate your policy run the `record` command example, make sure to change `remote_ip`, `port`, model..
...
python -m lerobot.record \
   --robot.type=lekiwi_client \
    ...
    --teleop.port=/dev/tty.usbserial-15140 \ # <- The port of your leader arm.
    --teleop.id=lekiwi_leader \
    --policy.path=${HF_USER}/act_lekiwi \ # <- Your own policy path.
    --dataset.push_to_hub=true \
    --dataset.num_episodes=5 \
    --dataset.episode_time_s=90 \
    --dataset.reset_time_s=30 \
    --dataset.single_task="Move to front of block and pick up it." \
    --dataset.repo_id=${HF_USER}/eval_lekiwi_data \
    --display_data=true \
    --resume=false
```
