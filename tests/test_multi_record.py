...
def test_multi_dataset_config():
    ...
    dataset1 = DatasetRecordConfig(
        repo_id="test/dataset1",
        single_task="Test task 1"
    )
    
    dataset2 = DatasetRecordConfig(
        repo_id="test/dataset2", 
        single_task="Test task 2"
    )
    
    # Test multi-dataset config creation
    multi_config = MultiDatasetRecordConfig(
        datasets=[dataset1, dataset2],
        stage_switch_keys=["space", "tab"]
    )
