...
"""End-to-end joint-ordering tests for the rollout action path.

A checkpoint trained on a dataset whose joint enumeration differs from the robot's is
supported: the state fed to the policy and the action read back out are both reordered.
For a **relative-action** checkpoint that reordering carries a load-bearing invariant,
because ``AbsoluteActionsProcessorStep`` reconstructs ``action = delta + state`` by
adding the cached ``observation.state`` *positionally*:

    dataset_features["observation.state"]["names"] == ordered_action_keys

Break it and every joint is anchored on some *other* joint's measurement. On a bimanual
robot whose two arms are mirrored that is a positive-feedback runaway, not a one-off
glitch: the commanded target moves away from the measured position as the joint tracks
it, so the error compounds every tick until the arm slams.

These tests drive the real engines and the real ``send_next_action`` against a stub robot
and a stub policy, and assert the invariant *per joint*. They are parameterized over the
historical wiring variants (see ``_VARIANTS``) so that the two broken ones are reproduced
rather than merely absent — a happy-path-only test is what let this regression land.
"""
...
def _observation(offset: float = 0.0) -> dict:
    """A raw robot observation: every joint measurement plus a frame per camera.

    ``build_dataset_frame`` looks cameras up by bare name, so they have to be present even
    though nothing here inspects the pixels.
    """
    import numpy as np
...
def _delta(dim: int) -> float:
    """The stub policy's relative offset for output dimension *dim* (1-based, 1..16).

    Kept strictly below the 1000-unit measurement spacing so ``anchor + delta`` is
    uniquely decomposable (see :func:`_provenance`).
    """
    return float(dim + 1)


def _expected_command(joint: str) -> float:
    """What a correctly wired pipeline commands for *joint*.

    The policy emits dimension ``CKPT_ORDER.index(joint)`` for this joint, and
    ``relative_exclude_joints=["gripper"]`` means the grippers are *not* anchored at all —
    their dimension is passed through absolute.
    """
...
def test_orders_are_a_pure_block_swap():
    """The premise: same set, and within each arm the order is identical.

    This is why an ordering bug can only ever swap a joint with its opposite-arm twin,
    and why the grippers (indices 7 and 15 in *both* orders) keep working even when the
    arms are swapped — ``relative_exclude_joints`` builds the same mask either way.
    """
...
class _StubRelativePolicy:
    """A relative-action chunk policy with a recognisable per-dimension offset.

    ``select_action`` returns offsets only; the postprocessor's
    ``AbsoluteActionsProcessorStep`` adds the anchor, exactly as it does for a real
    relative-action checkpoint. ``observed_states`` records what the policy was fed so a
    test can assert the state arrived in the checkpoint's own layout.
    """
...
class _AnchorPipeline:
    """Minimal stand-in for ``PolicyProcessorPipeline`` around one relative/absolute pair.

    Deliberately *not* a mock: the anchor bug lives in the interaction between
    ``observation.state`` ordering and the positional add in
    ``AbsoluteActionsProcessorStep``, so both real steps are exercised.
    """
...
def _build_features(*, align_state: bool, align_action: bool):
    ...
    Calls the production helpers rather than reimplementing the reordering, so a change
    to either one is caught here.
    """
...
def _run_sync_tick(*, align_state, align_action, remap):
    """One control tick through the real sync engine and the real ``send_next_action``.

    Returns ``(action_dict, dataset_features, ordered_action_keys, policy)``.
    """
...
def _provenance(action_dict):
    ...
    the thousands digit names the joint whose measurement was used as the anchor, and the
    remainder names the output dimension the value came from. A value below 1000 carries no
    anchor at all, which is the expected shape for a ``relative_exclude_joints`` dimension.
    """
    out = {}
...
def test_sync_anchor_provenance_per_variant(align_state, align_action, remap, anchor_ok, trained_order):
    """Every joint must be anchored on its own measurement, and only some variants are.

    ``anchor_ok=False`` is the runaway: the joint is anchored on its opposite-arm twin.
    """
...
def test_sync_head_wiring_is_fully_correct():
    """The shipped configuration: correct anchor *and* correct state layout.

    Asserted as exact values rather than via the provenance decomposition, so this test
    fails if the arithmetic model in ``_anchor_sources`` is ever wrong.
    """
    action_dict, dataset_features, ordered_action_keys, policy = _run_sync_tick(
...
def test_sync_grippers_swap_with_each_other_but_never_run_away():
    """Under a broken wiring the grippers fail differently from the arm joints.

    The gripper indices (7 and 15) are the same in both orders, so the
    ``relative_exclude_joints`` mask is identical either way and the grippers keep carrying
    *no* anchor — they cannot diverge. But the permutation still maps dimension 7 to 15, so
    the two grippers receive **each other's** command. Benign-looking on a folding task
    where both grippers do similar things, which is part of why the swap is easy to miss.
    """
...
def test_sync_anchor_is_pinned_across_a_chunk():
    ...
    Re-anchoring on every tick double-counts the motion already executed (the robot has
    moved by roughly the accumulated offset), which compounds into a runaway
    indistinguishable on hardware from the ordering bug. Guards
    ``SyncInferenceEngine``'s ``predict_action_chunk`` probe.
    """
    from lerobot.rollout.inference.sync import SyncInferenceEngine
    ...
    class _ChunkingPolicy(_StubRelativePolicy):
        """Serves a 3-step chunk, predicting only on the tick the queue runs dry.

        Routes through ``self.predict_action_chunk`` exactly as a real chunking policy
        does (``vla_jepa.select_action`` calls it too). That is the contract the engine's
        probe rests on: it swaps in an instance attribute, so a policy that reached the
        chunk predictor by any other route would leave the anchor silently unpinned.
        """
...
def test_rtc_get_action_matches_sync(align_state, align_action, remap, anchor_ok):
    """RTC must label its queued actions exactly as sync does.

    The two engines diverging is its own class of bug: the same checkpoint on the same
    robot would drive correctly on one backend and swap arms on the other.
    """
    ...
    # `_rtc_loop` merges the chunk *after* postprocessing, so the queue holds absolute
    # actions in the policy's own dimension order. Seed it with what THIS variant's
    # postprocessor would have produced: the anchor is added positionally from
    # `observation.state`, whose layout is the variant's own.
    state_names = dataset_features["observation.state"]["names"]
...
def test_rtc_and_sync_agree_on_the_shipped_wiring():
    """Same checkpoint, same robot, same commanded dict — whichever backend is used.

    A divergence here is its own class of bug: the arm would drive correctly on one engine
    and swap arms on the other, which is how the original sync/RTC discrepancy presented.
    """
    from lerobot.policies.rtc import ActionQueue
    from lerobot.policies.rtc.configuration_rtc import RTCConfig
    from lerobot.rollout.inference import RTCInferenceEngine
    ...
    absolute = torch.tensor([[_expected_command(joint) for joint in CKPT_ORDER]], dtype=torch.float32)
    engine._action_queue = ActionQueue(RTCConfig(enabled=True, execution_horizon=8))
    engine._action_queue.merge(absolute.clone(), absolute.clone(), 0, None, task="fold the t-shirt")
