...
def test_disable_enable_keys():
    dataset = LeRobotDataset("lerobot/pusht")
    item = dataset[0]
    all_data_keys = list(item.keys())
    disable_data_key = all_data_keys[0]
    dataset.disable_data_keys([disable_data_key])
    item = dataset[0]
    assert set(item.keys()) == set(all_data_keys[1:])
    dataset.enable_data_keys([disable_data_key])
    item = dataset[0]
    assert set(item.keys()) == set(all_data_keys)
