...
def test_delete_single_episode(sample_dataset, tmp_path):
    ...
    episode_indices = {int(idx.item()) for idx in new_dataset.reader.hf_dataset["episode_index"]}
...
def test_split_by_episodes(sample_dataset, tmp_path):
    ...
    train_episodes = {int(idx.item()) for idx in result["train"].reader.hf_dataset["episode_index"]}
    assert train_episodes == {0, 1, 2}

    val_episodes = {int(idx.item()) for idx in result["val"].reader.hf_dataset["episode_index"]}
...
def test_merge_two_datasets(sample_dataset, tmp_path, empty_lerobot_dataset_factory):
    ...
    episode_indices = sorted({int(idx.item()) for idx in merged.reader.hf_dataset["episode_index"]})
...
def test_delete_consecutive_episodes(sample_dataset, tmp_path):
    ...
    episode_indices = sorted({int(idx.item()) for idx in new_dataset.reader.hf_dataset["episode_index"]})
