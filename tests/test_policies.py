...
def test_backward_compatibility(env_name, policy_name, extra_overrides):
    ...
    for key in saved_output_dict:
        assert torch.isclose(output_dict[key], saved_output_dict[key], rtol=1e-4, atol=1e-7).all()
    for key in saved_grad_stats:
        assert torch.isclose(grad_stats[key], saved_grad_stats[key], rtol=1e-4, atol=1e-7).all()
    for key in saved_param_stats:
        assert torch.isclose(param_stats[key], saved_param_stats[key], rtol=1e-4, atol=1e-7).all()
