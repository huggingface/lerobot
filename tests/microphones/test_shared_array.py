...
def test_single_writer_single_reader():
    ...
    writer = Process(
        ...
        args=(shared_array, data_queue, stop_event, barrier, 1)
    )
...
def test_multiple_writers_single_reader():
    ...
    for i in range(num_writers):
        writer = Process(
            target=writer_process,
            args=(shared_array, data_queue, stop_event, barrier, i + 1)
        )
...
def test_data_integrity_with_concurrent_access():
    ...
    writer2 = Process(
        target=writer_process,
        args=(shared_array, data_queue, stop_event, barrier, 2)
    )
