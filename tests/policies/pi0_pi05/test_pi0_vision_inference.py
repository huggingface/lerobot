...
"""Tests for the pi0 vision-path inference optimizations.

Mirrors ``test_pi05_vision_inference.py``: cameras are embedded as one batched vision-tower call
at inference and one call each during training, and the tower runs under bfloat16 autocast at
inference only. Neither may leak into the training path.
"""
...
from lerobot.policies.pi0.modeling_pi0 import PaliGemmaWithExpertModel, PI0Pytorch
...
class _Harness(torch.nn.Module):
    """Exercises ``PI0Pytorch._embed_images`` without building a 3B backbone."""
    ...
    _embed_images = PI0Pytorch._embed_images
...
def test_inference_embeds_all_cameras_in_one_call():
    ...
    harness._embed_images(_distinct_images(3))  # noqa: SLF001
...
def test_mixed_shapes_fall_back_to_one_call_per_camera():
    ...
    harness._embed_images([torch.ones(1, 3, 4, 4), torch.ones(1, 3, 8, 8)])  # noqa: SLF001
...
def test_batched_and_looped_embeddings_match_on_the_real_vision_tower():
    torch.manual_seed(0)
    model = PI0Pytorch(PI0Config(paligemma_variant="gemma_300m")).to("cuda").eval()
