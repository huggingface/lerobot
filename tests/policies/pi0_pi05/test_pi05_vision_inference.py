...
class _RecordingVisionTower:
    ...
    def embed_image(self, image: torch.Tensor, **kwargs) -> torch.Tensor:
...
def _masks(images: list[torch.Tensor]) -> list[torch.Tensor]:
    return [torch.ones(img.shape[:2] if img.ndim == 5 else img.shape[:1], dtype=torch.bool) for img in images]
...
def test_inference_embeds_all_cameras_in_one_call():
    ...
    harness._embed_images(images, _masks(images))  # noqa: SLF001
...
def test_batching_returns_the_same_embeddings_in_the_same_order():
    ...
    looped = _Harness().train()._embed_images(images, _masks(images))  # noqa: SLF001
    batched = _Harness().eval()._embed_images(images, _masks(images))  # noqa: SLF001
...
def test_mem_video_inputs_stay_per_camera_with_their_frame_masks():
    # MEM video inputs are (B,T,C,H,W) and each carries its own frame mask, so they cannot be
    # concatenated into one call and must keep the existing per-camera path.
    harness = _Harness().eval()
    images = [torch.full((1, 3, 3, 4, 4), float(i + 1)) for i in range(2)]
    masks = _masks(images)
    masks[1][0, 0] = False

    harness._embed_images(images, masks)  # noqa: SLF001

    tower = harness.paligemma_with_expert
    assert tower.call_batch_sizes == [1, 1]
    assert [call["temporal_attention_every"] for call in tower.video_calls] == [4, 4]
    assert torch.equal(tower.video_calls[1]["frame_mask"], masks[1])


def test_mixed_shapes_fall_back_to_one_call_per_camera():
    harness = _Harness().eval()
    images = [torch.ones(1, 3, 4, 4), torch.ones(1, 3, 8, 8)]

    harness._embed_images(images, _masks(images))  # noqa: SLF001

    assert harness.paligemma_with_expert.call_batch_sizes == [1, 1]
...
def test_batched_and_looped_embeddings_match_on_the_real_vision_tower():
    ...
    masks = [torch.ones(1, dtype=torch.bool, device="cuda") for _ in images]

    with torch.no_grad():
        batched = model._embed_images(images, masks)  # noqa: SLF001
        model.train()
        looped = model._embed_images(images, masks)  # noqa: SLF001
