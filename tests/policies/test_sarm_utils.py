...
class TestProgressLabelsWithModes:
    ...
    def test_dense_mode_more_stages(self):
        ...
        proportions = {name: 1/8 for name in global_names}
...
class TestComputeAbsoluteIndices:
    ...
    def test_no_clamping_when_in_middle(self):
        ...
        indices, out_of_bounds = compute_absolute_indices(
            frame_idx, ep_start, ep_end, n_obs_steps, frame_gap
        )
...
class TestFindStageAndTau:
    ...
    def test_single_stage_mode_linear_progress(self):
        ...
        # Frame 0 -> tau = 0
        stage, tau = find_stage_and_tau(
            0, episode_length, None, None, None, ["task"], {"task": 1.0}
        )
        assert stage == 0
        ...
        stage, tau = find_stage_and_tau(
            50, episode_length, None, None, None, ["task"], {"task": 1.0}
        )
        assert stage == 0
        ...
        # Frame 99 -> tau = 1.0
        stage, tau = find_stage_and_tau(
            99, episode_length, None, None, None, ["task"], {"task": 1.0}
        )
        assert stage == 0
...
class TestNormalizeStageTau:
    ...
    def test_with_temporal_proportions(self):
        ...
        result = normalize_stage_tau(
            1.0, temporal_proportions=proportions, subtask_names=subtask_names
        )
        assert abs(result - 0.2) < 1e-6
        
        # Stage 1 end should map to 0.5
        result = normalize_stage_tau(
            2.0, temporal_proportions=proportions, subtask_names=subtask_names
        )
...
class TestRewindAugmentation:
    ...
    def test_rewind_indices_go_backwards_from_earliest_obs(self):
        ...
        rewind_step, rewind_indices = apply_rewind_augmentation(
            frame_idx, ep_start, n_obs_steps=n_obs_steps, max_rewind_steps=2, 
            frame_gap=frame_gap, rewind_step=2
        )
    ...
    def test_rewind_goes_backward_through_history(self):
        ...
        rewind_step, rewind_indices = apply_rewind_augmentation(
            frame_idx, ep_start, n_obs_steps=n_obs_steps, max_rewind_steps=4, 
            frame_gap=frame_gap, rewind_step=3
        )
