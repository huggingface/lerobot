...
@require_package("transformers")
@require_cuda
def test_smolvla_rtc_explicit_max_guidance_weight():
    from lerobot.policies.smolvla.modeling_smolvla import SmolVLAPolicy  # noqa: F401

    """Test SmolVLA respects explicit max_guidance_weight when provided."""
    set_seed(42)
...
@require_package("transformers")
@require_cuda
def test_smolvla_rtc_inference_with_different_sigma_d_and_auto_guidance():
    from lerobot.policies.smolvla.modeling_smolvla import SmolVLAPolicy  # noqa: F401

    """Test SmolVLA inference with different sigma_d values using Alex Soare optimization."""
    set_seed(42)

    config = SmolVLAConfig(
        max_action_dim=7,
        chunk_size=50,
        num_steps=10,  # Will be used as max_guidance_weight
    )
    ...
    with torch.no_grad():
        ...
        actions_sigma_05 = policy1.predict_action_chunk(
            batch,
            noise=noise.clone(),
    ...
    # Now test with sigma_d = 1.5 (weaker guidance)
    config.rtc_config = RTCConfig(
        enabled=True,
        execution_horizon=10,
        max_guidance_weight=None,  # Use Alex Soare optimization
    ...
    # Verify shapes
    assert actions_sigma_05.shape == (1, config.chunk_size, 7)
    assert actions_sigma_10.shape == (1, config.chunk_size, 7)

    # Different sigma_d values should produce different results
    assert not torch.allclose(actions_sigma_05, actions_sigma_10, rtol=1e-3)
