...
@pytest.mark.parametrize("query_kind", ["vqa", "next_subtask"])
def test_runtime_query_reaches_tokenizer(query_kind):
    config = BeingH05Config(device="cpu", use_language_recipe=True)
    preprocessor, _ = make_being_h05_pre_post_processors(config)
    _stub_tokenizer(preprocessor)
    processed = preprocessor(
        {
            **_named_state(),
            **{key: torch.zeros(1, 3, 224, 224) for key in ROBOCASA_CAMERA_KEYS},
            "task": ["original task"],
            "query_kind": query_kind,
            "query_text": "pick the cup",
        }
    )
    assert processed["being_h05_messages"] == [[{"role": "user", "content": "pick the cup"}]]
    assert processed["being_h05_predict_actions"].tolist() == [False]
...
def test_runtime_messages_do_not_require_training_sidecars():
    transition = {
        TransitionKey.COMPLEMENTARY_DATA: {
            "messages_rendered": [{"role": "user", "content": "What do you see?"}],
        }
    }
...
def test_recipe_text_targets_must_be_assistant_messages():
    transition = {
        TransitionKey.COMPLEMENTARY_DATA: {
            "messages_rendered": [[{"role": "user", "content": "question"}]],
...
def test_quantile_normalization_round_trips_continuous_and_binary_actions(tmp_path):
    ...
    assert isinstance(preprocessor.steps[5], BeingH05BinaryActionStep)
