...
To resume training from a checkpoint, below is an example command to resume from `040000` checkpoint of the `act_so100_test` policy:
```bash
python lerobot/scripts/train.py \
  --dataset.repo_id=${HF_USER}/so100_test \
  --policy.type=act \
  ...
  --wandb.enable=true \
  --resume=true \
  --config_path=outputs/train/act_so100_test/checkpoints/040000/pretrained_model/train_config.json
```
