...
"""Run a trained EE-space policy on SO100 (keyboard-trained) without recording.

Mirrors ``examples/phone_to_so100/rollout.py`` — the model was trained
with keyboard teleoperation in EE space, so at deployment we only need the
joint↔EE conversion on the robot side; the keyboard is not used.
