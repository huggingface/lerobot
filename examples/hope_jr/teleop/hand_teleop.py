...
def make_left_hand() -> tuple[HomunculusGlove, HopeJrHand]:
    glove_cfg = HomunculusGloveConfig("/dev/tty.usbmodem11101", id="left", side="left")
    glove = HomunculusGlove(glove_cfg)
    hand_cfg = HopeJrHandConfig("/dev/tty.usbmodem58760432281", id="left", side="left")
    hand = HopeJrHand(hand_cfg)
    return glove, hand


def main():
    glove, hand = make_left_hand()
    glove.connect()
    hand.connect()
