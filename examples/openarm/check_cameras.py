#!/usr/bin/env python
"""Grab one frame from each rollout camera and save a labeled PNG per camera to confirm mapping.
...
def main() -> None:
    for label, (dev, w, h) in CAMERAS.items():
        ...
        cv2.putText(
            frame, f"{label}  {dev}", (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3, cv2.LINE_AA
        )
