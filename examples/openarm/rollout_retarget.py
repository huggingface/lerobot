...
# Rotation matrices and Jacobians keep their conventional capitals (R, J, Rt, Jpinv) so the
# IK math below reads like the equations it implements.
# ruff: noqa: N803, N806
...
class RetargetRobot:
    ...
    def __init__(
        ...
        self._obs_seed: dict[str, np.ndarray | None] = dict.fromkeys(SIDES)
        self._act_seed: dict[str, np.ndarray | None] = dict.fromkeys(SIDES)
    ...
    def _solve(self, m, d, tcp_id, dofadr, qadr7, rng7, pt, Rt, seed, q_ref, iters):
        ...
        if self._null_gain > 0.0 and res > self._ee_tol:
            q0, res0 = self._ik(m, d, tcp_id, dofadr, qadr7, rng7, pt, Rt, seed, q_ref, iters, use_null=False)
        ...
        with self._lock:
            ...
            for s in SIDES:
                ...
                if seed is None:
                    ...
                    seed = self._last_short[s].copy() if s in self._last_short else q_ref.copy()
