...
class VideoAnnotator:
    ...
    def annotate(self, file_path: str | Path, fps: int, start_timestamp: float = 0.0,
                 end_timestamp: float | None = None, max_retries: int = 3) -> SubtaskAnnotation:
        ...
        try:
            messages = [
                ...
                {"role": "user", "content": [
                    ...
                    {"type": "text", "text": f"Video is {duration_str} (~{duration:.1f}s). Follow instructions."}
                ]}
            ]
...
def display_annotation(annotation: SubtaskAnnotation, console: Console, episode_idx: int, fps: int, prefix: str = ""):
    ...
    subtask_summary = ", ".join(f"{s.name}({s.timestamps.start}-{s.timestamps.end})" for s in annotation.subtasks)
    console.print(f"[green]Episode {episode_idx} {prefix}: {len(annotation.subtasks)} subtasks - {subtask_summary}[/green]")
...
def save_annotations_to_dataset(dataset_path: Path, annotations: dict[int, SubtaskAnnotation], fps: int, prefix: str = "sparse"):
    ...
    cols = [f"{prefix}_{c}" for c in ["subtask_names", "subtask_start_times", "subtask_end_times", "subtask_start_frames", "subtask_end_frames"]]
    for col in cols:
    ...
    for ep_idx, ann in annotations.items():
        ...
        for s in ann.subtasks:
            ...
            starts.append(st); ends.append(et); start_frames.append(int(st * fps)); end_frames.append(int(et * fps))
    ...
    for ep_idx in episodes_df.index:
        key = (episodes_df.loc[ep_idx, "meta/episodes/chunk_index"], episodes_df.loc[ep_idx, "meta/episodes/file_index"])
        path = dataset_path / DEFAULT_EPISODES_PATH.format(chunk_index=key[0], file_index=key[1])
        if path.exists():
            file_df = pd.read_parquet(path)
            for col in cols + (["subtask_names", "subtask_start_times", "subtask_end_times", "subtask_start_frames", "subtask_end_frames"] if prefix == "sparse" else []):
            ...
            if ep_idx in annotations:
                ...
                if prefix == "sparse":  # Legacy columns
                    for i, legacy in enumerate(["subtask_names", "subtask_start_times", "subtask_end_times", "subtask_start_frames", "subtask_end_frames"]):
                        file_df.at[ep_idx, legacy] = episodes_df.loc[ep_idx, cols[i]]
            file_df.to_parquet(path, engine="pyarrow", compression="snappy")


def generate_auto_sparse_annotations(dataset: LeRobotDataset, episode_indices: list[int], video_key: str) -> dict[int, SubtaskAnnotation]:
    ...
    for ep_idx in episode_indices:
        ...
        annotations[ep_idx] = SubtaskAnnotation(subtasks=[Subtask(name="task", timestamps=Timestamp(start="00:00", end=end_str))])
    return annotations
...
def load_annotations_from_dataset(dataset_path: Path, prefix: str = "sparse") -> dict[int, SubtaskAnnotation]:
    ...
    for ep_idx in df.index:
        ...
        annotations[int(ep_idx)] = SubtaskAnnotation(subtasks=[
            Subtask(name=n, timestamps=Timestamp(start=f"{int(s)//60:02d}:{int(s)%60:02d}", end=f"{int(e)//60:02d}:{int(e)%60:02d}"))
            for n, s, e in zip(names, starts, ends)])
    return annotations


def process_single_episode(ep_idx: int, dataset_root: Path, dataset_meta, video_key: str, fps: int,
                          annotator: VideoAnnotator, console: Console) -> tuple[int, SubtaskAnnotation | None, str | None]:
...
def worker_process_episodes(worker_id: int, gpu_id: int, episode_indices: list[int], repo_id: str, video_key: str,
                           sparse_subtask_list: list[str], dense_subtask_list: list[str] | None,
                           model_name: str, torch_dtype: torch.dtype) -> tuple[dict, dict | None]:
    ...
    dense_annotator = VideoAnnotator(dense_subtask_list, model_name, device, torch_dtype,
                                     sparse_annotator.model, sparse_annotator.processor) if dense_subtask_list else None
    
    sparse_annotations, dense_annotations = {}, {} if dense_subtask_list else None
    
    for ep_idx in episode_indices:
        _, sparse_ann, err = process_single_episode(ep_idx, dataset.root, dataset.meta, video_key, dataset.fps, sparse_annotator, console)
        if sparse_ann:
            sparse_annotations[ep_idx] = sparse_ann
        
        if dense_annotator:
            _, dense_ann, _ = process_single_episode(ep_idx, dataset.root, dataset.meta, video_key, dataset.fps, dense_annotator, console)
            if dense_ann:
...
def main():
    ...
    parser.add_argument("--sparse-subtasks", type=str, default=None, help="Comma-separated sparse subtask names")
    ...
    sparse_subtask_list = [s.strip() for s in args.sparse_subtasks.split(",")] if args.sparse_subtasks else None
    ...
    video_key = args.video_key if args.video_key in (dataset.meta.video_keys or []) else dataset.meta.video_keys[0]
    ...
    # GPU setup
    gpu_ids = args.gpu_ids or list(range(min(args.num_workers, torch.cuda.device_count() if torch.cuda.is_available() else 1)))
    ...
    if need_vlm:
        if args.num_workers > 1 and not auto_sparse:
            ...
            with ProcessPoolExecutor(max_workers=args.num_workers, mp_context=mp.get_context('spawn')) as executor:
                futures = [executor.submit(worker_process_episodes, w, gpu_ids[w], episodes_per_worker[w],
                          args.repo_id, video_key, sparse_subtask_list, dense_subtask_list, args.model, torch_dtype)
                          for w in range(args.num_workers) if episodes_per_worker[w]]
            ...
            sparse_annotator = VideoAnnotator(sparse_subtask_list, args.model, args.device, torch_dtype) if not auto_sparse and sparse_subtask_list else None
            dense_annotator = VideoAnnotator(dense_subtask_list, args.model, args.device, torch_dtype,
                                            sparse_annotator.model if sparse_annotator else None,
                                            sparse_annotator.processor if sparse_annotator else None) if dense_mode else None

            for i, ep_idx in enumerate(episode_indices):
                console.print(f"[cyan]Episode {ep_idx} ({i+1}/{len(episode_indices)})[/cyan]")
                
                if sparse_annotator:
                    _, sparse_ann, err = process_single_episode(ep_idx, dataset.root, dataset.meta, video_key, fps, sparse_annotator, console)
                    if sparse_ann:
                        sparse_annotations[ep_idx] = sparse_ann
                        save_annotations_to_dataset(dataset.root, sparse_annotations, fps, prefix="sparse")
                    elif err:
                ...
                if dense_annotator:
                    _, dense_ann, err = process_single_episode(ep_idx, dataset.root, dataset.meta, video_key, fps, dense_annotator, console)
                    if dense_ann:
                        dense_annotations[ep_idx] = dense_ann
                        save_annotations_to_dataset(dataset.root, dense_annotations, fps, prefix="dense")
                    elif err:
