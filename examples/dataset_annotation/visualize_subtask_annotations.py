...
Supports both sparse (high-level) and dense (fine-grained) annotations.

Usage:
    # Visualize sparse annotations only (default)
    python visualize_subtask_annotations.py --repo-id pepijn223/mydataset --video-key observation.images.base --num-episodes 1

    # Visualize dense annotations only
    python visualize_subtask_annotations.py --repo-id pepijn223/mydataset --annotation-type dense --num-episodes 1

    # Visualize both sparse and dense annotations
    python visualize_subtask_annotations.py --repo-id pepijn223/mydataset --annotation-type both --num-episodes 1
"""
...
# Different color palette for dense annotations
SUBTASK_COLORS_DENSE = [
    "#4E79A7",  # Steel blue
    "#F28E2B",  # Tangerine
    "#E15759",  # Red
    "#76B7B2",  # Teal
...
def visualize_episode(
    ...
    Args:
        ...
        annotation: SubtaskAnnotation object
        video_path: Path to the video file
        video_start_timestamp: Start timestamp in the video file
        video_end_timestamp: End timestamp in the video file
        fps: Frames per second
        output_path: Path to save the visualization
        video_key: Camera/video key name
        annotation_type: "sparse" or "dense" (affects colors and title)
    """
    ...
    # Select color palette based on annotation type
    colors = SUBTASK_COLORS_DENSE if annotation_type == "dense" else SUBTASK_COLORS_SPARSE
    ...
    # Adjust figure size based on number of subtasks
    fig_width = max(16, num_subtasks * 2.5)
    ...
    type_label = annotation_type.capitalize()
    
    # Add title
    fig.suptitle(
        f"Episode {episode_idx} - {type_label} Subtask Annotations",
        fontsize=18,
        fontweight='bold',
        color='white',
        y=0.96
    ...
    fig.text(
        0.5, 0.91,
        f"Camera: {video_key} | Duration: {episode_duration:.1f}s | {num_subtasks} {type_label.lower()} subtasks",
        ha='center',
        fontsize=11,
        color='#888888'
    )
    ...
    for i, subtask in enumerate(subtasks):
        ...
        if duration > total_duration * 0.08:
            # Use dark text for light colors (yellow, gold, etc.)
            light_colors = [3, 5] if annotation_type == "dense" else [3]
            text_color = 'black' if i in light_colors else 'white'
...
def visualize_episode_dual(
    ...
    """
    Create side-by-side visualization for both sparse and dense annotations.
    
    Shows:
    - Top row: Sparse timeline
    - Middle: Sample frames from START and END of each dense subtask
    - Bottom row: Dense timeline
    """
    sparse_subtasks = sparse_annotation.subtasks if sparse_annotation else []
    dense_subtasks = dense_annotation.subtasks if dense_annotation else []
    
    if len(sparse_subtasks) == 0 and len(dense_subtasks) == 0:
        print(f"No subtasks found for episode {episode_idx}")
        return
    ...
    # Get total duration from annotations
    total_duration = 0
    if sparse_subtasks:
        total_duration = max(total_duration, timestamp_to_seconds(sparse_subtasks[-1].timestamps.end))
    if dense_subtasks:
        total_duration = max(total_duration, timestamp_to_seconds(dense_subtasks[-1].timestamps.end))
    
    # Extract sample frames from START and END of each dense subtask
    sample_frames = []
    frame_timestamps = []
    frame_labels = []
    frame_colors = []
    
    source_subtasks = dense_subtasks if dense_subtasks else sparse_subtasks
    colors = SUBTASK_COLORS_DENSE if dense_subtasks else SUBTASK_COLORS_SPARSE
    
    for i, subtask in enumerate(source_subtasks):
        start_sec = timestamp_to_seconds(subtask.timestamps.start)
        end_sec = timestamp_to_seconds(subtask.timestamps.end)
        color = colors[i % len(colors)]
        
        # Add START frame
        video_timestamp_start = video_start_timestamp + start_sec
        frame_timestamps.append(start_sec)
        frame_labels.append(f"{subtask.name}\n(start)")
        frame_colors.append(color)
        frame = extract_frame_from_video(video_path, video_timestamp_start)
        sample_frames.append(frame)
        
        # Add END frame
        video_timestamp_end = video_start_timestamp + end_sec
        frame_timestamps.append(end_sec)
        frame_labels.append(f"{subtask.name}\n(end)")
        frame_colors.append(color)
        frame = extract_frame_from_video(video_path, video_timestamp_end)
        sample_frames.append(frame)
    
    num_frames = len(sample_frames)
    
    # Create figure with 3 rows: sparse timeline, frames, dense timeline
    # Adjust width based on number of frames (2 per dense subtask)
    fig_width = max(18, num_frames * 1.8)
    ...
    # Add title
    fig.suptitle(
        f"Episode {episode_idx} - Sparse vs Dense Annotations",
        fontsize=18,
        fontweight='bold',
        color='white',
        y=0.96
    ...
    fig.text(
        0.5, 0.92,
        f"Camera: {video_key} | Duration: {episode_duration:.1f}s | "
        f"Sparse: {len(sparse_subtasks)} stages | Dense: {len(dense_subtasks)} stages",
        ha='center',
        fontsize=11,
        color='#888888'
    )
    ...
    ax_sparse.set_facecolor('#16213e')
    ax_sparse.set_title("Sparse Annotations (High-Level Stages)", fontsize=12, color='#E69F00', pad=8)
    
    if sparse_subtasks:
        _draw_timeline(ax_sparse, sparse_subtasks, total_duration, SUBTASK_COLORS_SPARSE, [3])
    else:
        ax_sparse.text(0.5, 0.5, "No sparse annotations", ha='center', va='center', 
                      color='#888888', fontsize=12, transform=ax_sparse.transAxes)
    
    # --- Sample Frames (Middle) - Start/End of each dense subtask ---
    for i, (frame, label, ts, color) in enumerate(zip(sample_frames, frame_labels, frame_timestamps, frame_colors)):
        ...
        ax.set_title(label, fontsize=8, fontweight='bold', color=color, pad=5)
        ax.axis('off')
        ax.text(0.5, -0.05, f"t={ts:.1f}s", ha='center', fontsize=7, 
               color='#888888', transform=ax.transAxes)
    
    # --- Dense Timeline (Bottom) ---
    ax_dense = fig.add_subplot(gs[2, :])
    ax_dense.set_facecolor('#16213e')
    ax_dense.set_title("Dense Annotations (Fine-Grained Stages)", fontsize=12, color='#4E79A7', pad=8)
    
    if dense_subtasks:
        _draw_timeline(ax_dense, dense_subtasks, total_duration, SUBTASK_COLORS_DENSE, [3, 5])
    else:
        ax_dense.text(0.5, 0.5, "No dense annotations", ha='center', va='center', 
                     color='#888888', fontsize=12, transform=ax_dense.transAxes)
...
def _draw_timeline(ax, subtasks, total_duration, colors, light_color_indices):
    """Helper to draw a timeline with subtask segments."""
    bar_height = 0.6
    bar_y = 0.5
    ...
    for i, subtask in enumerate(subtasks):
        ...
        if duration > total_duration * 0.06:
            text_color = 'black' if i in light_color_indices else 'white'
            ax.text(
                mid_x, bar_y,
                subtask.name,
                ha='center', va='center',
    ...
    for t in boundary_times:
        ax.axvline(x=t, ymin=0.1, ymax=0.9, color='white', linestyle='--', linewidth=1.5, alpha=0.7)
    ...
    tick_interval = max(1, int(total_duration / 10))
    ax.set_xticks(np.arange(0, total_duration + tick_interval, tick_interval))


def main():
    parser = argparse.ArgumentParser(
        description="Visualize SARM subtask annotations (sparse and/or dense)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Visualize sparse annotations only (default)
  python visualize_subtask_annotations.py --repo-id pepijn223/mydataset --num-episodes 5

  # Visualize dense annotations only
  python visualize_subtask_annotations.py --repo-id pepijn223/mydataset --annotation-type dense

  # Visualize both sparse and dense side-by-side
  python visualize_subtask_annotations.py --repo-id pepijn223/mydataset --annotation-type both
"""
    )
    parser.add_argument(
        "--repo-id",
    ...
    console.print(f"[cyan]Annotation type: {args.annotation_type}[/cyan]")
    ...
    if args.annotation_type in ["sparse", "both"]:
        sparse_annotations = load_annotations_from_dataset(dataset.root, prefix="sparse")
        if sparse_annotations:
            console.print(f"[green]Found {len(sparse_annotations)} sparse annotated episodes[/green]")
        else:
            console.print("[yellow]No sparse annotations found[/yellow]")
    
    if args.annotation_type in ["dense", "both"]:
        dense_annotations = load_annotations_from_dataset(dataset.root, prefix="dense")
        if dense_annotations:
            console.print(f"[green]Found {len(dense_annotations)} dense annotated episodes[/green]")
        else:
            console.print("[yellow]No dense annotations found[/yellow]")
    
    # Determine available episodes
    if args.annotation_type == "sparse":
        available_episodes = set(sparse_annotations.keys())
    elif args.annotation_type == "dense":
        available_episodes = set(dense_annotations.keys())
    else:  # both
        # Use union of episodes that have at least one type of annotation
        available_episodes = set(sparse_annotations.keys()) | set(dense_annotations.keys())
    ...
    if args.episodes:
        ...
        for ep in episode_indices:
            if ep not in available_episodes:
                console.print(f"[yellow]Warning: Episode {ep} has no annotation, skipping[/yellow]")
        episode_indices = [ep for ep in episode_indices if ep in available_episodes]
    else:
        # Random selection
        num_to_select = min(args.num_episodes, len(available_episodes))
        episode_indices = random.sample(list(available_episodes), num_to_select)
    ...
    for ep_idx in episode_indices:
        ...
        try:
            if args.annotation_type == "both":
                # Create dual visualization
                sparse_ann = sparse_annotations.get(ep_idx)
                dense_ann = dense_annotations.get(ep_idx)
                
                output_path = output_dir / f"episode_{ep_idx:04d}_subtasks_dual.png"
                
                visualize_episode_dual(
                    episode_idx=ep_idx,
                    sparse_annotation=sparse_ann,
                    dense_annotation=dense_ann,
                ...
                annotations = sparse_annotations if args.annotation_type == "sparse" else dense_annotations
                annotation = annotations.get(ep_idx)
                
                if annotation is None:
                    console.print(f"[yellow]No {args.annotation_type} annotation for episode {ep_idx}, skipping[/yellow]")
                    continue
                
                suffix = "_sparse" if args.annotation_type == "sparse" else "_dense"
                output_path = output_dir / f"episode_{ep_idx:04d}_subtasks{suffix}.png"
    ...
    console.print(f"Annotation type: {args.annotation_type}")
