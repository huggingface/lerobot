...
def p_control_loop(
    robots, keyboard, target_positions, start_positions, current_positions, kp=0.5, control_freq=50
):
    """
    P控制循环
                ...
                for key, value in keyboard_action.items():
                    ...
                    # 第一臂xy控制
                        ...
                        if coord == "x":
                            ...
                            joint2_target, joint3_target = inverse_kinematics(
                                current_positions["arm1"]["x"], current_positions["arm1"]["y"]
                            )
                            target_positions["arm1"]["shoulder_lift"] = joint2_target
                            target_positions["arm1"]["elbow_flex"] = joint3_target
                            print(
                                f"第一臂更新x坐标: {current_positions['arm1']['x']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}"
                            )
                        elif coord == "y":
                            ...
                            target_positions["arm1"]["elbow_flex"] = joint3_target
                            print(
                                f"第一臂更新y坐标: {current_positions['arm1']['y']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}"
                            )
                    ...
                    # 第二臂xy控制
                        ...
                        if coord == "x":
                            ...
                            # 计算joint2和joint3的目标角度
                            joint2_target, joint3_target = inverse_kinematics(
                                current_positions["arm2"]["x"], current_positions["arm2"]["y"]
                            )
                            target_positions["arm2"]["shoulder_lift"] = joint2_target
                            target_positions["arm2"]["elbow_flex"] = joint3_target
                            print(
                                f"第二臂更新x坐标: {current_positions['arm2']['x']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}"
                            )
                        elif coord == "y":
                            ...
                            target_positions["arm2"]["elbow_flex"] = joint3_target
                            print(
                                f"第二臂更新y坐标: {current_positions['arm2']['y']:.4f}, joint2={joint2_target:.3f}, joint3={joint3_target:.3f}"
                            )

            # 对每个机械臂应用pitch调整到wrist_flex
            for arm_name in ["arm1", "arm2"]:
                if (
                    "shoulder_lift" in target_positions[arm_name]
                    and "elbow_flex" in target_positions[arm_name]
                ):
                    target_positions[arm_name]["wrist_flex"] = (
                        -target_positions[arm_name]["shoulder_lift"]
                        - target_positions[arm_name]["elbow_flex"]
                        + pitch[arm_name]
                    )
...
def main():
    ...
    try:
        ...
        robots = {"arm1": arm1_robot, "arm2": arm2_robot}
        ...
        # 初始化双臂x,y坐标控制
        x0, y0 = 0.1629, 0.1131
        current_positions = {"arm1": {"x": x0, "y": y0}, "arm2": {"x": x0, "y": y0}}
