...
"""
Analyze and visualize inference logs generated by InferenceLogger.
"""
...
def plot_servo_positions(robot_state_df):
    ...
    pos_columns = [col for col in robot_state_df.columns if '.pos' in col and not 'raw' in col]
...
def plot_policy_outputs(policy_df):
    ...
    if 'inference_time_ms' in policy_df.columns:
        ...
        axes[1].axhline(mean_time + std_time, color='orange', linestyle='--', alpha=0.7, label=f'+1σ: {mean_time + std_time:.1f}ms')
        axes[1].legend()
        
        print(f"📈 Inference timing stats:")
    ...
    with open('inference_summary.txt', 'w') as f:
        ...
        for key, df in data.items():
            ...
            if 'robot_state' in key and any('.pos' in col for col in df.columns):
                pos_columns = [col for col in df.columns if '.pos' in col and not 'raw' in col]
