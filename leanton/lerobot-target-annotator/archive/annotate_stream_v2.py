"""
LeRobot Target Annotator — ZMQ Publisher
=========================================
Streams the annotated overview frame to LeRobot's native ZMQCamera, enabling
lerobot-record to record the YOLO-World bbox annotation as a dedicated camera
channel with zero LeRobot code modifications.
...
CONF              = 0.05
DEFAULT_CLASSES   = ["small object", "desk object"]

# ── Stabilization buffer ─────────────────────────────────────
# Rolling median buffer that smooths raw YOLO detections into a stable bbox.
# A new detection is accepted into the buffer only if its IoU with the current
# median >= IOU_GATE (i.e. it's the "same" object).  Detections below the gate
# are routed to the challenger buffer instead.
BUFFER_SIZE       = 15     # number of bbox samples held for temporal median smoothing
IOU_GATE          = 0.4    # min IoU overlap to consider a new detection the same object
STABLE_FILL       = 0.8    # fraction of buffer that must be filled before bbox is "committed" (drawn on ZMQ stream)
STALE_FRAMES      = 20     # consecutive frames with no detection before buffer resets (unless in hysteresis)
HYSTERESIS_SECS   = 1.0    # grace period after stability: buffer holds its last bbox even if detections drop, preventing flicker

# ── Challenger buffer ─────────────────────────────────────────
# Secondary buffer that accumulates detections NOT matching the main buffer.
# When it fills to CHALLENGER_FILL, the main buffer is replaced wholesale —
# this handles genuine target switches (e.g. operator moves to a different
# object) without requiring the main buffer to drain through STALE_FRAMES.
CHALLENGER_SIZE   = 8      # number of samples the challenger must collect before it can take over
CHALLENGER_FILL   = 0.75   # fraction of challenger that must be filled before it replaces the main buffer

# ── Loss cooldown ────────────────────────────────────────────
# When a committed target is lost (gripped and moved away), incoming
# detections are suppressed for this many seconds to prevent a second
...
class DetectionBuffer:
    ...
    def update(self, detection):
        if detection is None:
            self._stale_count += 1
            if self._stale_count >= self._stale_limit and not self._protected():
                self.reset()
            return self.median()

        # Loss cooldown: suppress re-detection after a committed target is lost
        ...
        if current is None or _iou(new_box, current) >= self._iou_gate:
            ...
            if self._protected():
                return self.median()
            self._challenger.update(new_box)
            if self._challenger.ready:
                seed = self._challenger.drain()
                self.reset()
                for box in seed:
                    self._buf.append(box)
                self._last_label    = label
...
def main():
    ...
    # Auto-restore frozen bbox from saved state
    if state and state.get("frozen") and state.get("frozen_bbox"):
        frozen       = True
    ...
    while True:
        ...
        if frozen:
            ...
            rank0       = detections[0] if detections else None
            median_bbox = buf.update(rank0)
            if rank0 is not None:
                _, _, _, _, _, disp_label, disp_conf = rank0
            else:
