...
def load_base():
    spec = importlib.util.spec_from_file_location("csource_caligned_simseen6_eval_base", BASE_PATH)
...
def load_plan(path: Path) -> dict:
    ...
    if len(trials) != 12 or [row["order"] for row in trials] != list(range(1, 13)):
        raise RuntimeError("expected 12 contiguous Sim-seen6 trials")
    ...
    expected = [("S", "A") if pose % 2 else ("A", "S") for pose in range(1, 7)]
    if [tuple(row["model_key"] for row in pair) for pair in pairs] != expected:
        raise RuntimeError("SA/AS Sim-seen6 frozen order mismatch")
    if {key: sum(row["model_key"] == key for row in trials) for key in ("S", "A")} != {"S": 6, "A": 6}:
        raise RuntimeError("model trial counts mismatch")
    if any(row.get("restore_nominal_cube_pose_before_this_trial") is not True for row in trials):
        raise RuntimeError("each Sim-seen6 trial must independently restore the nominal cube pose")
    if plan["pose_bank"].get("automatic_full_eval24_fallback") is not False:
        raise RuntimeError("full Eval24 fallback is forbidden")
    return plan
...
def configure_base(base, plan: dict, plan_path: Path) -> None:
    ...
    def write_sidecar(plan_: dict, trial: dict, artifact_stem: str, replacement_for: str | None) -> None:
        ...
        if not evidence_path.exists():
            return
        sidecar_path = root / f"{artifact_stem}.paired_simseen6.json"
        if sidecar_path.exists():
            return
        evidence = json.loads(evidence_path.read_text())
        pre_action_path = root / f"{artifact_stem}.pre_action.png"
        if pre_action_path.exists():
            pre_action = {
                "status": "frozen",
                "path": str(pre_action_path),
                "sha256": sha(pre_action_path),
                "frame_index": 0,
                "width": 640,
                "height": 480,
        ...
        sidecar = {
            "schema": "task1_act_csource_caligned_response_v3_simseen6_trial_sidecar_v1",
            "evaluation_id": plan_["evaluation_id"],
            "trial": trial,
            "artifact_stem": artifact_stem,
            "replacement_for": replacement_for,
            "engine_evidence": {"path": str(evidence_path), "sha256": sha(evidence_path)},
            ...
            "success_contract": plan_["success_contract"],
...
def dry_run(base, plan: dict) -> dict:
    ...
    if fake["trials_exercised"] != 12 or fake["policy_reset_calls"] != {"S": 6, "A": 6}:
        raise RuntimeError("12-trial fake protocol mismatch")
