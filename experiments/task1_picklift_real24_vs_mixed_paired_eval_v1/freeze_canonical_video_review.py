...
EXPECTED_OPERATOR_MANIFEST_SHA256 = (
    "79f134d2e90004e645de2574206c685903ed7b9ef78bd72d387369112d59bc48"
)
EXPECTED_OPERATOR_SUMMARY_SHA256 = (
    "cbd243d94d3dc11d6f74eee1216a2647d483b0363b03475990f48e092acd1e94"
)
EXPECTED_OPERATOR_HASHES_SHA256 = (
    "ea6211fb4cd5a73c381b5b665fafbda1115451de95e61a60bea831b810c9b89a"
)
EXPECTED_OPERATOR_RESULT_SOURCE_COMMIT = (
    "bdb5b3b921649aadb01c93609207139b97512e73"
)
EXPECTED_REVIEW_ID = (
    "task1_picklift_real24_vs_mixed_paired_real24_eval24_"
    "canonical_video_review_v1"
)
...
def summarize_by_model(rows: list[dict], model_ids: list[str]) -> dict:
    ...
    for model_id in model_ids:
        ...
        result[model_id] = {
            ...
            "failure_types": dict(
                Counter(
                    row["review_failure_type"]
                    for row in selected
                    if not row["review_success"]
                )
            ),
        }
    return result
...
def summarize_pairs(rows: list[dict]) -> tuple[list[dict], dict]:
    ...
    for cell_id in sorted({row["cell_id"] for row in rows}):
        ...
        mixed_success = cell_rows["real24_questsim24"]["review_success"]
        ...
        by_cell.append(
            {
                ...
                "real24_questsim24_review_success": mixed_success,
...
def main() -> None:
    ...
    operator_summary = read_json(OPERATOR_SUMMARY_PATH)
    ...
    require(
        decisions_source["operator_result_source_commit"]
        == EXPECTED_OPERATOR_RESULT_SOURCE_COMMIT,
    ...
    require(len(operator_hashes) == 174, "Expected 174 frozen operator artifacts.")
    ...
    require(
        [row["trial_id"] for row in decisions]
    ...
    for operator_row in trials:
        ...
        require(
            steps["first_step"] == 0
            and steps["last_step"] == steps["lines"] - 1,
        ...
        if decision["review_success"]:
            ...
            require(
                decision["confirmation_time_seconds"]
                - decision["first_visible_qualifying_time_seconds"]
                >= 0.5,
        ...
        review_label_payload = {
            ...
            "first_visible_qualifying_time_seconds": decision[
                "first_visible_qualifying_time_seconds"
            ],
            ...
            "adjudication_status": (
                "not_required" if agreement else "append_only_adjudication_required"
            ),
        }
    ...
    future_output_paths = (
        [labels_root / name for name, _ in label_payloads]
        + [trials_path, summary_path, validation_path, manifest_path]
    )
