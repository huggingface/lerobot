...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write one immutable operator or canonical-video review label "
            "without modifying trial evidence."
        )
    )
...
def main() -> None:
    ...
    if not success and args.failure_category not in FAILURE_CATEGORIES:
        raise RuntimeError(
            "Failed labels require one frozen failure category: "
    ...
    label = {
        ...
        "time_to_first_valid_success_seconds": (
            args.time_to_first_valid_success_seconds
        ),
        "notes": args.notes,
        ...
        "canonical_video": (
            evidence["video"] if args.source == "review" else {"status": "not_reviewed"}
        ),
    }
