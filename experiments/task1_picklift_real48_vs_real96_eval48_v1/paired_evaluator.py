...
SOURCE_RESEARCH_CONTRACT = EXPERIMENT_DIR / "source_research_contract.json"
...
EXPECTED_EVALUATION_ID = "task1_picklift_real48_vs_real96_eval48_v1"
EXPECTED_RESEARCH_COMMIT = "73908355df1add52cd04753216c13f8b1c0b400a"
EXPECTED_RESEARCH_HASHES = {
    "experiment_design": "46b2ec8335d9b4415967efdd70dbe854d113d2966307a0eaaf38278744650af1",
...
def load_frozen_plan(path: Path = DEFAULT_PLAN) -> dict:
    ...
    expected_pair_models = [
        (
            MODEL_IDS[0] if pose["first_model"].startswith("ACT_Real48") else MODEL_IDS[1],
            MODEL_IDS[0] if pose["second_model"].startswith("ACT_Real48") else MODEL_IDS[1],
        )
...
def software_dry_run(plan: dict) -> dict:
    ...
    if fake["policy_reset_calls"] != {"real48": 48, "real96": 48}:
...
def write_eval48_sidecar(
    ...
    sidecar = {
        "schema": "task1_picklift_real48_vs_real96_eval48_trial_sidecar_v1",
        "evaluation_id": plan["evaluation_id"],
        "trial": trial,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Frozen Task1 Real48 versus Real96 paired Eval48. "
