...
from summarize_reviewed_results import resolved_label, summarize_rows
...
def test_frozen_plan_and_research_source_hashes() -> None:
    ...
    assert evaluator.sha256_file(evaluator.SOURCE_RESEARCH_CONTRACT) == evaluator.EXPECTED_RESEARCH_HASHES["experiment_design"]
    assert evaluator.sha256_file(evaluator.SOURCE_POSE_MANIFEST) == evaluator.EXPECTED_RESEARCH_HASHES["pose_manifest"]
...
def test_exact_96_order_matches_frozen_pose_manifest() -> None:
    ...
    assert [trial["model_key"] for trial in trials[:6]] == ["real48", "real96", "real96", "real48", "real48", "real96"]
    assert {key: sum(trial["model_key"] == key for trial in trials) for key in evaluator.MODEL_IDS} == {"real48": 48, "real96": 48}


def test_each_pair_preserves_identical_frozen_pose() -> None:
    trials = load_plan()["trials"]
    fields = (
        "pose_order", "eval_pose_id", "source_order_sha256", "cell", "coverage_tier",
        "quadrant", "nominal_x_forward_m", "nominal_y_lateral_m",
    ...
    for left, right in zip(trials[::2], trials[1::2], strict=True):
        ...
        assert {left["model_key"], right["model_key"]} == {"real48", "real96"}
    source = trials[::2]
    assert {tier: sum(row["coverage_tier"] == tier for row in source) for tier in (
        "seen_by_real48", "added_by_real96", "unseen_by_both"
    )} == {"seen_by_real48": 24, "added_by_real96": 18, "unseen_by_both": 6}
    assert {yaw: sum(row["nominal_yaw_degrees_modulo_90"] == yaw for row in source) for yaw in (0, 45)} == {0: 24, 45: 24}
...
def test_fake_protocol_exercises_all_trials_without_devices() -> None:
    ...
    assert result["policy_reset_calls"] == {"real48": 48, "real96": 48}
...
def test_slow_tick_never_generates_catch_up_burst() -> None:
    ...
    records = evaluator.run_paced_ticks(0.22, tick, lambda row: None, period=0.05, now_fn=Clock.now, sleep_fn=Clock.sleep)
...
def test_checkpoint_hash_tamper_fails_closed() -> None:
    ...
    plan["models"]["real96"]["model_sha256"] = "0" * 64
...
def test_static_contract_verifies_models_processor_calibration_and_no_devices() -> None:
    ...
    assert set(static["models"]) == {"real48", "real96"}
    assert static["hardware_identity_contract"]["devices_opened"] is False
    assert static["hardware_identity_contract"]["calibration_sha256"] == load_plan()["setup"]["follower_calibration_sha256"]
...
def test_fresh_order_and_one_linked_replacement(tmp_path: Path) -> None:
    ...
    original.write_text(json.dumps({"status": "aborted_with_error", "termination": "hardware_or_runtime_error"}), encoding="utf-8")
...
def test_review_resolution_and_all_required_report_slices() -> None:
    agreed = {
        "operator_label": {"status": "labeled", "success": True},
        "canonical_video_review_label": {"status": "reviewed", "success": True},
    }
    assert resolved_label(agreed)["source"] == "operator_review_agreement"
    rows = []
    for trial in load_plan()["trials"]:
        success = trial["model_key"] == "real96"
        rows.append({
            "model_key": trial["model_key"], "pose_order": trial["pose_order"],
            "eval_pose_id": trial["eval_pose_id"], "coverage_tier": trial["coverage_tier"],
            "cell": trial["cell"], "yaw": trial["nominal_yaw_degrees_modulo_90"],
            "success": success, "failure_category": None if success else "missed_grasp",
        })
    summary = summarize_rows(rows)
    assert summary["overall"]["real48"]["trials"] == 48
    assert summary["overall"]["real96"]["trials"] == 48
    assert set(summary["by_coverage_tier"]) == {"seen_by_real48", "added_by_real96", "unseen_by_both"}
    assert len(summary["by_cell"]) == 12
    assert set(summary["by_yaw"]) == {"0", "45"}
    assert summary["paired_real96_minus_real48"]["success_count_difference"] == 48
...
def test_evidence_schema_covers_execution_review_and_reporting_contracts() -> None:
    ...
    for fragment in (
        "pre-action canonical frame", "requested/observed ready", "policy reset and tick0",
        "canonical video", "raw/requested/official sent", "operator label",
        "canonical-video review label", "adjudication", "return trajectory",
        "torque disable",
    ):
        assert fragment in per_trial
    assert reporting == {
        "overall by model", "coverage tiers 24/18/6", "cell", "yaw",
        "failure categories", "paired Real96 minus Real48 success difference",
    }
...
def test_full_software_dry_run_has_no_hardware_access() -> None:
    ...
    assert result["hardware_access"] == {"serial": False, "camera": False, "robot": False, "torque": False, "rollout": False}
