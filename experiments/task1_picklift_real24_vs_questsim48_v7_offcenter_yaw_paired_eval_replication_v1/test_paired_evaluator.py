...
def test_replication_changes_only_predeclared_first_model_order() -> None:
    plan = load_plan()
    revision = plan["protocol_revision"]
    assert revision["predecessor_evaluation_id"] == (
        "task1_picklift_real24_vs_questsim48_v7_offcenter_yaw_paired_eval24_v1"
    )
    assert revision["predecessor_plan_sha256"] == (
        "f3f6e797ce001752b46f5f278dcb94df712a29d3e1e29708228810a82074406e"
    )
    assert revision["only_intentional_change_from_predecessor"] == (
        "per-pose first-model order is reversed"
    )
    assert revision["poses_or_checkpoints_changed_after_first_run_results"] is False
    assert revision["historical_trial_evidence_reused"] is False
    assert revision["historical_results_used_for_pose_selection"] is False
    assert plan["research_contract"]["first_run_result_manifest"]["sha256"] == (
        "9244f34c2e2e141d65b0c1ad19aee51fe7176dad1703068129f347a05d84425f"
    )
