...
EXPECTED_RESEARCH_IDENTITY_VERIFICATION_SHA256 = (
    "8452975c25e38ae8e0530c5812100a583d9a4d2d90a13409a8907eb0cd848c62"
)
EXPECTED_PLAN_SHA256 = "4fa8d08216f9923cfe6b9c1cc2f3f52480d6abeed5e5b77e4f08c6e8ab6ab15a"
...
EXPECTED_RESEARCH_COMMIT = "126bf299662aa8d0bc195a0ce3bd2d8b77e9ac94"
EXPECTED_RESEARCH_HASHES = {
    "training_run_manifest": "5a897eac537ee6f438074ea162ac5846d516926b2eacf4e339a4643359a3a6ba",
    "training_decision": "3159029b7044f0f9641a28b4f9fa8276ff2d94cc56e754f6a2a73443efbce058",
    "first_run_result_manifest": "9244f34c2e2e141d65b0c1ad19aee51fe7176dad1703068129f347a05d84425f",
    ...
    "return_amendment": "c32c8f7e8040309b06b6bbb717299945f6bd06f8eb1f72b5b0860edc758e3fae",
}
...
MODEL_IDS = ("real24_only", "questsim48_v7")
...
def load_frozen_plan(path: Path = DEFAULT_PLAN) -> dict:
    ...
    trials = plan["trials"]
    if len(trials) != 24:
        raise RuntimeError("Paired Eval-v2 plan must contain exactly 24 scored trials.")
    if [trial["order"] for trial in trials] != list(range(1, 25)):
        raise RuntimeError("Paired order indices must be contiguous 1..24.")
    source_trials = trials[::2]
    actual_schedule = tuple(
        (
            trial["source_pose_id"],
            trial["cell_id"],
            trial["quadrant"],
    ...
    if actual_schedule != EXPECTED_POSE_SCHEDULE:
        raise RuntimeError("Paired pose order differs from the frozen research pose manifest.")
    actual_xy = tuple(
        (trial["nominal_x_forward_m"], trial["nominal_y_lateral_m"])
        for trial in source_trials
    )
    if actual_xy != EXPECTED_NOMINAL_XY:
        raise RuntimeError("Paired nominal coordinates differ from the frozen 15 mm grid revision.")
    if len({trial["cell_id"] for trial in trials}) != 12:
        raise RuntimeError("Every 5 cm cell must appear as exactly one paired pose.")
    if len({trial["spawn_region"] for trial in trials}) != 24:
        raise RuntimeError("Each trial needs a unique immutable artifact stem.")
    if any(
        left["source_pose_order"] != pair_index
        or right["source_pose_order"] != pair_index
        or left["source_pose_id"] != right["source_pose_id"]
    ...
    expected_pair_models = [
        ("questsim48_v7", "real24_only")
        if pair_index % 2 == 1
        else ("real24_only", "questsim48_v7")
    ...
    if actual_pair_models != expected_pair_models:
        raise RuntimeError("Alternating first-model order differs from the frozen pairing rule.")
    if {
        model_id: sum(trial["model_id"] == model_id for trial in trials)
        for model_id in MODEL_IDS
    } != {"real24_only": 12, "questsim48_v7": 12}:
    ...
    if set(quadrant_counts.values()) != {6}:
    ...
    revision = plan["protocol_revision"]
    if revision["fresh_evidence"] is not True:
        raise RuntimeError("This comparison must use a fresh evidence identity.")
    if revision["historical_trial_evidence_reused"] is not False:
        raise RuntimeError("Historical Eval-v2 evidence must not be reused as paired trials.")
    if revision["historical_results_used_for_pose_selection"] is not False:
        raise RuntimeError("Historical outcomes must not select the paired poses.")
    if revision["predecessor_evaluation_id"] != (
        "task1_picklift_real24_vs_questsim48_v7_offcenter_yaw_paired_eval24_v1"
    ):
        raise RuntimeError("Replication predecessor evaluation identity changed.")
    if revision["predecessor_plan_sha256"] != (
        "f3f6e797ce001752b46f5f278dcb94df712a29d3e1e29708228810a82074406e"
    ):
        raise RuntimeError("Replication predecessor plan identity changed.")
    if revision["only_intentional_change_from_predecessor"] != (
        "per-pose first-model order is reversed"
    ):
        raise RuntimeError("Replication change scope differs from the frozen one-change rule.")
    if revision["poses_or_checkpoints_changed_after_first_run_results"] is not False:
        raise RuntimeError("Replication may not change poses or checkpoints after first-run results.")
    if revision["source_pose_count"] != 12 or revision["paired_trial_count"] != 24:
        raise RuntimeError("Paired pose/trial counts changed.")
    if revision["adaptive_order_or_checkpoint_change_allowed"] is not False:
        raise RuntimeError("Adaptive checkpoint or order changes must remain prohibited.")
    if revision["scored_window_changed"] is not False:
        raise RuntimeError("Ready/return behavior must remain outside the scored window.")
...
def software_dry_run(plan: dict) -> dict:
    ...
    if fake["trials_exercised"] != 24:
        raise RuntimeError("Fake protocol did not exercise all 24 paired trials.")
    if fake["policy_reset_calls"] != {"real24_only": 12, "questsim48_v7": 12}:
    ...
    if not all(checks):
        raise RuntimeError("Fake paired Eval-v2 protocol verification failed.")
    return {
        "schema_version": 1,
    ...
    *,
    ...
    if not replacement:
        if first_missing is None:
            raise RuntimeError("All 24 frozen original paired trials already have evidence.")
...
def write_evalv2_sidecar(
    ...
    if sidecar_path.exists():
        raise RuntimeError(f"Refusing to overwrite paired Eval-v2 sidecar: {sidecar_path}")
...
def execute_hardware(args: argparse.Namespace, plan: dict) -> None:
    ...
    preflight.update(
        {
            "paired_evalv2_evaluation_id": plan["evaluation_id"],
            "paired_evalv2_plan_sha256": EXPECTED_PLAN_SHA256,
            "paired_evalv2_trial": trial,
            "replacement_for": replacement_for,
            "success_contract": plan["success_contract"],
            "operator_label": {"status": "pending"},
            "canonical_video_review_label": {"status": "pending"},
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Frozen Task1 Real24-only vs QuestSim48-v7 off-center/yaw paired replication. "
