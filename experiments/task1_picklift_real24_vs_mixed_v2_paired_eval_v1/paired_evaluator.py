...
EXPECTED_PREDECESSOR_PLAN_SHA256 = "1eb82cc573a661f85d7c21ced5651a9cc64c8809873a3299d13e86d59fd19ada"
...
EXPECTED_EVALUATION_ID = "task1_picklift_real24_vs_mixed_v2_paired_real24_eval24_v1"
...
MODEL_IDS = ("real24_only", "mixed_v2")
EXPECTED_SCHEDULE = (
    ("r1c1", "real24_only"),
    ("r1c1", "mixed_v2"),
    ("r1c2", "mixed_v2"),
...
def load_frozen_plan(path: Path = DEFAULT_PLAN) -> dict:
    ...
    if plan["research_contract"]["predecessor_plan_sha256"] != EXPECTED_PREDECESSOR_PLAN_SHA256:
        raise RuntimeError("Predecessor paired-contract hash reference changed.")
    ...
    actual_schedule = tuple((trial["cell_id"], trial["model_id"]) for trial in trials)
    ...
    if [trial["trial_id"] for trial in trials] != [f"t{index:02d}" for index in range(1, 25)]:
    ...
    for cell in sorted({trial["cell_id"] for trial in trials}):
        models = [trial["model_id"] for trial in trials if trial["cell_id"] == cell]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Frozen Task1 paired Real24-only versus Mixed v2 ACT evaluator. "
