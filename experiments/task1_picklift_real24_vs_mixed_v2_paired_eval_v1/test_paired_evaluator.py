...
def test_frozen_plan_has_exact_24_trial_paired_order() -> None:
    ...
    assert [(row["cell_id"], row["model_id"]) for row in plan["trials"]] == list(evaluator.EXPECTED_SCHEDULE)
    assert {
        model_id: sum(row["model_id"] == model_id for row in plan["trials"])
        for model_id in evaluator.MODEL_IDS
    } == {"real24_only": 12, "mixed_v2": 12}
...
def test_fake_robot_camera_bus_exercises_both_models_and_same_ready_pose() -> None:
    ...
    assert result["models_in_frozen_order"] == [trial["model_id"] for trial in plan["trials"]]
...
def test_both_checkpoint_owned_processors_use_saved_imagenet_stats() -> None:
    result = evaluator.verify_static_files(evaluator.load_frozen_plan())
    assert set(result["models"]) == {"real24_only", "mixed_v2"}
