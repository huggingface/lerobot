...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify frozen Combined72 Sim48-v7 Dataset v3")
    parser.add_argument("--dataset-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument("--evidence-root", type=Path, default=DEFAULT_EVIDENCE_ROOT)
    args = parser.parse_args()

    source, real_dataset, sim_dataset = source_audit()
    freeze = json.loads((args.evidence_root / "freeze_manifest.json").read_text())
    tree_sha, file_count, total_bytes, inventory = derived_tree_identity(args.dataset_root)
    ...
    if loader_dataset.meta.total_episodes != 72 or len(loader_dataset) != 10147:
    ...
    if sorted(frame_counts) != list(range(72)):
        raise RuntimeError("Derived episode indices are not exactly 0..71")
    if sorted(global_indices) != list(range(10147)):
        raise RuntimeError("Derived global indices are not exactly 0..10146")

    derived_state = np.stack(loader_dataset.hf_dataset["observation.state"])
    derived_action = np.stack(loader_dataset.hf_dataset["action"])
    ...
    if not (
        np.array_equal(derived_state[:3790], real_state)
        and np.array_equal(derived_action[:3790], real_action)
        and np.array_equal(derived_state[3790:], sim_state)
        and np.array_equal(derived_action[3790:], sim_action)
    ):
        raise RuntimeError("Identity canonicalization did not preserve source state/action values")

    rows = [
        json.loads(line)
        for line in (args.dataset_root / "provenance/source_episodes.jsonl").read_text().splitlines()
        if line
    ]
    if len(rows) != 72:
        raise RuntimeError("Provenance map does not contain exactly 72 rows")
    if [row["derived_episode_index"] for row in rows] != list(range(72)):
        raise RuntimeError("Provenance derived episode indices are not exactly 0..71")
    domains = Counter(row["source_domain"] for row in rows)
    if domains != {"real": 24, "quest_remote_mujoco_sim48_v7": 48}:
    ...
    provenance_manifest = json.loads((args.dataset_root / "provenance/manifest.json").read_text())
    canonicalization = provenance_manifest["gripper_canonicalization"]
    if (
        canonicalization["canonicalization_id"] != GRIPPER_CANONICALIZATION_ID
        or canonicalization["real_transform"] != "identity"
        or canonicalization["simulation_transform"] != "identity"
    ):
        raise RuntimeError("Unexpected gripper canonicalization contract")

    stats = json.loads((args.dataset_root / "meta/stats.json").read_text())
    policy_stats = {key: stats[key] for key in ("observation.state", "observation.images.front", "action")}
    if not numeric_tree(policy_stats):
        raise RuntimeError("Policy normalization stats contain a non-numeric value")
    for key in ("observation.state", "action"):
        if stats[key]["count"] != [10147]:
            raise RuntimeError(f"Unexpected {key} stats count")
    image_count = 10147 * 480 * 640
    ...
    result = {
        "schema": "task1_picklift_combined72_sim48_v7_verification_v1",
        "status": "pass",
        ...
        "derived": {
            ...
            "episodes": 72,
            "frames": 10147,
            "episode_indices": "0..71 contiguous",
            "global_indices": "0..10146 contiguous",
            "domain_episode_counts": dict(sorted(domains.items())),
            "domain_frame_counts": {
                domain: sum(row["frame_count"] for row in rows if row["source_domain"] == domain)
                for domain in sorted(domains)
            },
            "high_resolution_sidecar_paths": 0,
            "state_action_identity_against_sources": True,
            "gripper_canonicalization": canonicalization,
        },
        "official_loader": loader,
        "policy_normalization_stats": {
            "all_numeric_and_finite": True,
            "state_action_frame_count": 10147,
            "image_decoded_frame_count": 10147,
