...
def verify(output_dir: Path, phase: str) -> dict[str, Any]:
    ...
    for container_name, container in (
        ...
        processor = container.get("checkpoint_owned_processor")
        if not isinstance(processor, dict):
            raise RuntimeError(
                f"{container_name} checkpoint processor evidence is missing."
            )
        if processor.get("use_imagenet_stats") is not True:
            raise RuntimeError(
                f"{container_name} does not verify ImageNet processor stats."
            )
        if (
            processor.get("policy_preprocessor_sha256")
            != FIXED_PREPROCESSOR_SHA256
        ):
            raise RuntimeError(f"{container_name} preprocessor hash mismatch.")
        if (
            processor.get("normalizer_stats_sha256")
            != FIXED_NORMALIZER_STATS_SHA256
        ):
            raise RuntimeError(f"{container_name} normalizer stats hash mismatch.")
        if not np.allclose(
            processor.get("visual_mean"),
            IMAGENET_MEAN,
            rtol=0.0,
            atol=1.0e-7,
        ):
            raise RuntimeError(f"{container_name} ImageNet mean mismatch.")
        if not np.allclose(
            processor.get("visual_std"),
            IMAGENET_STD,
            rtol=0.0,
            atol=1.0e-7,
        ):
            raise RuntimeError(f"{container_name} ImageNet std mismatch.")
    ...
    return {
        ...
        "checkpoint_owned_processor_verified": True,
