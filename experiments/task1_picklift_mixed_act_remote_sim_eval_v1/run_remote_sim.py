"""Run the frozen mixed-ACT Task1 diagnostic through the formal Nexus adapter.
...
EXPECTED_OWNER_PLAN_SHA256 = (
    "231063547b30d42e77ec5432ff6e95efa7f0babeffe71de75f1c6813bd22feb9"
)
...
COMPARISON_BOUNDARY = (
    "Historical Real-only 1/120 used a different action-processing version; "
    "the results cannot be directly attributed to the mixed dataset or model."
)
...
def load_owner_plan() -> dict[str, Any]:
    if sha256_file(PLAN_PATH) != EXPECTED_OWNER_PLAN_SHA256:
        raise RuntimeError("Mixed-sim owner plan hash mismatch.")
    plan = json.loads(PLAN_PATH.read_text(encoding="utf-8"))
    if plan["model"]["model_sha256"] != EXPECTED_MODEL_SHA256:
        raise RuntimeError("Mixed checkpoint binding drifted.")
...
def validate_deployment(phase: str) -> dict[str, Any]:
    ...
    if model_hash != EXPECTED_MODEL_SHA256:
        raise RuntimeError("Mixed checkpoint file hash mismatch.")
    return {
        "status": "pass",
        "phase": phase,
...
def run_phase(phase: str, output_dir: Path) -> dict[str, Any]:
    ...
    from sim_policy_inference import Task1MixedActSimInference

    policy = Task1MixedActSimInference()
    if policy.model_hash != EXPECTED_MODEL_SHA256:
        raise RuntimeError("Loaded mixed ACT model SHA mismatch.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the frozen mixed ACT formal Remote Nexus diagnostic."
    )
