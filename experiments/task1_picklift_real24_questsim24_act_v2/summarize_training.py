...
def main() -> None:
    parser = argparse.ArgumentParser(description="Summarize controlled Mixed v2 ACT training")
    parser.add_argument(
        "--offline-validation",
        type=Path,
    ...
    result = {
        "schema": "task1_picklift_real24_questsim24_act_mixed_v2_run_result_v1",
        ...
        "smoke": {
            ...
            **log_times(smoke_log),
            ...
            "attempt0": {
                "status": "training_completed_but_outer_log_capture_failed",
                "reason": "tee opened its target before the artifact parent existed",
                "preserved_output": str(ARTIFACT_ROOT / "smoke_500_attempt0_tee_parent_race"),
                "training_steps_completed": 500,
                "model_sha256": sha256_file(
                    ARTIFACT_ROOT / "smoke_500_attempt0_tee_parent_race/checkpoints/000500/"
                    "pretrained_model/model.safetensors"
                ),
                "domain_sampling_sha256": sha256_file(
                    ARTIFACT_ROOT / "smoke_500_attempt0_tee_parent_race/domain_sampling_counts.json"
                ),
            },
        },
        "full_training": {
        ...
        "interpretation": (
            "Offline bundled engineering correction only. No causal attribution "
            "between corrections and no simulation or real-robot performance "
            "conclusion is made by this run."
        ),
    }
