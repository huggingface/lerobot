...
from evalv2_pilot import find_trial, load_frozen_plan, sha256_file
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Write one immutable operator or canonical-video review label without modifying trial evidence."
        )
    )
    parser.add_argument("--trial-id", required=True)
    parser.add_argument("--artifact-stem")
    parser.add_argument("--source", choices=("operator", "review"), required=True)
...
def main() -> None:
    ...
    if not success and args.failure_category not in FAILURE_CATEGORIES:
        raise RuntimeError(f"Failed labels require one frozen failure category: {sorted(FAILURE_CATEGORIES)}")
    if (
    ...
    if args.source == "review":
        ...
        if not video_path.exists():
            raise RuntimeError("Canonical-video review requires the recorded video.")
        if sha256_file(video_path) != video["sha256"]:
            raise RuntimeError("Canonical video hash differs from trial evidence.")
    labels_root = Path(plan["evidence_root"]) / "labels"
    labels_root.mkdir(parents=True, exist_ok=True)
    output_path = labels_root / f"{artifact_stem}.{args.source}.json"
    ...
    label = {
        ...
        "canonical_video": (evidence["video"] if args.source == "review" else {"status": "not_reviewed"}),
    }
    output_path.write_text(
        json.dumps(label, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    print(f"Saved immutable {args.source} label to {output_path}")
