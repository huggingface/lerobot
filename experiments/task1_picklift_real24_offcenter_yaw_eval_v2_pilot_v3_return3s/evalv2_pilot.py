...
EXPECTED_RESEARCH_IDENTITY_VERIFICATION_SHA256 = (
    "1bc2ac7740d814d57c48c548115b6b4a9def084f1d8395a0700fff8190653e7d"
)
...
EXPECTED_EVALUATION_ID = "task1_picklift_real24_offcenter_yaw_eval_v2_difficulty_pilot_v3_return3s"
EXPECTED_RESEARCH_COMMIT = "e7b628192c9b4d0d1f9ef7f8e9fa9e6417a01874"
...
def load_frozen_plan(path: Path = DEFAULT_PLAN) -> dict:
    ...
    if revision["predecessor_plan_sha256"] != (
        "48a159bf74ea61b9c444903f048f4c6315de0417248b37b500e9a1cb04b27cb5"
    ):
        raise RuntimeError("Predecessor plan reference changed.")
    if revision["immutable_completed_orders"] != [1, 2, 3, 4]:
        raise RuntimeError("Immutable predecessor trial boundary changed.")
    if revision["continuation_orders"] != list(range(5, 13)):
        raise RuntimeError("Continuation trial boundary changed.")
    if revision["scored_window_changed"] is not False:
        raise RuntimeError("Ready/return revision must remain outside the scored window.")
    return plan
