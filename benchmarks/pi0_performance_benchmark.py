...
def old_approach_embed_prefix(images: Dict[str, torch.Tensor], 
                             ...
                             hidden_size: int = 1024) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def old_approach_embed_suffix(state: torch.Tensor,
                             noisy_actions: torch.Tensor,
                             timestep: torch.Tensor,
                             proj_width: int = 1024) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def new_approach_embed_suffix(state: torch.Tensor,
                             noisy_actions: torch.Tensor,
                             timestep: torch.Tensor,
                             proj_width: int = 1024) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
def main():
    ...
    for i, config in enumerate(configs, 1):
        ...
        old_suffix_stats = benchmark_function(
            old_approach_embed_suffix, state, noisy_actions, timestep
        )
        new_suffix_stats = benchmark_function(
            new_approach_embed_suffix, state, noisy_actions, timestep
        )
        
        suffix_improvement = ((old_suffix_stats['mean'] - new_suffix_stats['mean']) / old_suffix_stats['mean']) * 100
