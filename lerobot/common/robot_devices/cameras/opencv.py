...
class OpenCVCamera():
    ...
    def __init__(self, camera_index: int, config: OpenCVCameraConfig | None = None):
        ...
        if camera_index is None:
            ...
            raise ValueError(f"`camera_index` is expected to be one of these available cameras {available_camera_indices}, but {camera_index} is provided instead.")
        ...
        if not is_camera_open:
            ...
            raise OSError(f"Can't access camera {camera_index}.")
        ...
        if self.fps and self.fps != actual_fps:
            raise OSError(f"Can't set {self.fps=} for camera {camera_index}. Actual value is {actual_fps}.")
        if self.width and self.width != actual_width:
            raise OSError(f"Can't set {self.width=} for camera {camera_index}. Actual value is {actual_width}.")
        if self.height and self.height != actual_height:
            raise OSError(f"Can't set {self.height=} for camera {camera_index}. Actual value is {actual_height}.")
...
if __name__ == "__main__":
    ...
    # config = OpenCVCameraConfig()
    config = OpenCVCameraConfig(30, 640, 480)
    ...
    if save_images:
        ...
        out_dir = "outputs/benchmark_cameras/opencv/2024_06_21_1917_"
        out_dir += f"{config.width}x{config.height}_{config.fps}"
        benchmark_cameras(cameras, out_dir, save_images=True)
    else:
        camera_ids = [5, 11, 18, 26]
