...
def check_format(raw_dir):
    ...
    zarr_paths = list(raw_dir.glob("*_push_*.zarr"))
    assert len(zarr_paths) > 0, "No zarr files found in raw_dir"
...
def load_from_raw(
    ...
    try:
        ...
        from gym_pushany.envs.pushany_env import PushAnyEnv, pymunk_to_shapely

        from lerobot.common.datasets.push_dataset_to_hub._diffusion_policy_replay_buffer import (
            ReplayBuffer as DiffusionPolicyReplayBuffer,
        )
    except ModuleNotFoundError as e:
        print("`gym_pushany` is not installed. Please install it with `pip install 'lerobot[pushany]'`")
        raise e
    ...
    # zarr_path = raw_dir / "s_push_0.zarr"
    # in raw_dirk, there are multiple zarr files, one for each object with each demonstrator.
    # for example, s_push_0.zarr, l_push_0.zarr, m_push_0.zarr, etc.
    # we need to concatenate them all together.
    zarr_paths = list(raw_dir.glob("*_push_*.zarr"))
    assert len(zarr_paths) > 0, "No zarr files found in raw_dir"

    print(f"Found {len(zarr_paths)} zarr files in {raw_dir}")
    total_ep_idx = 0       
    ep_dicts = []
    for zarr_path in zarr_paths:
        ...
        demonstrator = DEMONSTRATOR_LIST.index(demonstrator)
        object_name = OBJECT_NAME_LIST.index(object_name)
        ...
        # goal_body = PushAnyEnv._get_goal_pose_body(goal_pos_angle)
        ...
        for ep_idx, selected_ep_idx in tqdm.tqdm(enumerate(ep_ids)):
            ...
            #     # Add walls.
            #     walls = [
            #         PushAnyEnv.add_segment(space, (5, 506), (5, 5), 2),
            #         PushAnyEnv.add_segment(space, (5, 5), (506, 5), 2),
            #         PushAnyEnv.add_segment(space, (506, 5), (506, 506), 2),
            #         PushAnyEnv.add_segment(space, (5, 506), (506, 506), 2),
            #     ]
            #     space.add(*walls)

            #     block_body, block_shapes = PushAnyEnv.add_tee(space, block_pos[i].tolist(), block_angle[i].item())
            ...
            #     if keypoints_instead_of_image:
            #         keypoints[i] = torch.from_numpy(PushAnyEnv.get_keypoints(block_shapes).flatten())
            ...
            ep_dict["demonstrator"] = [demonstrator] * num_frames
            ep_dict["object_name"] = [object_name] * num_frames
            ep_dicts.append(ep_dict)
        total_ep_idx += num_episodes
...
def to_hf_dataset(data_dict, video, keypoints_instead_of_image: bool = False):
    ...
    features["demonstrator"] = Value(dtype="int64", id=None)
    features["object_name"] = Value(dtype="int64", id=None)
