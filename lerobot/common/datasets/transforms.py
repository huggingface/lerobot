...
def make_transforms(cfg):
    image_transforms = [
        v2.ColorJitter(
            brightness=cfg.colorjitter_factor,
            contrast=cfg.colorjitter_factor,
        ),
        v2.RandomAdjustSharpness(cfg.sharpness_factor, p=cfg.sharpness_p),
        v2.RandomAdjustSharpness(cfg.blur_factor, p=cfg.blur_p),
        v2.ToDtype(torch.float32, scale=True),
    ]
    return RandomSubsetApply(image_transforms, n_subset=cfg.n_subset)
