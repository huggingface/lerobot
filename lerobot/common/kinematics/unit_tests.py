...
class TestKinematics(unittest.TestCase):
    def setUp(self):
        ...
        self.q_init = np.array([0.0, 0, 0.0, -np.pi / 2, 0.0])
        # since everyone may set different out of bound values, I do unit testing with these fixed values
        self.mech_joint_limits_low = np.deg2rad([-90.0, -90.0, -90.0, -90.0, -90.0])
        self.mech_joint_limits_up = np.deg2rad([90.0, 90.0, 90.0, 90.0, 90.0])
        # same here for dh2mech and mech2dh methods
        self.q_init_dh = self.q_init[:]  # self.robot.from_mech_to_dh(self.q_init)
    ...
    def test_basic_usage(self):
        ...
        q_final = self.kin.inverse_kinematics(self.robot, self.q_init_dh, T_goal, use_orientation=False)
        q_final_mech = q_final[:]  # self.robot.from_dh_to_mech(q_final)
