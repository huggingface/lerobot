...
class TrossenArmDriver:
    ...
    def connect(self):
        ...
        if self.is_connected:
            print(f"TrossenArmDriver({self.ip}) is already connected. Do not call `motors_bus.connect()` twice.")
            return
    ...
    def read(self, data_name, motor_names: str | list[str] | None = None):
        if not self.is_connected:
            print(f"TrossenArmDriver({self.ip}) is not connected. You need to run `motors_bus.connect()`.")
            return
    ...
    def disconnect(self):
        if not self.is_connected:
            print(f"TrossenArmDriver ({self.ip}) is not connected. Try running `motors_bus.connect()` first.")
            return
