"""Frame-stride split sim_3stage_v3_success (60 eps) → train_fs / val_fs.

Output: domrachev03/sim_3stage_v3_success_train_fs (success-only, no partials).

After: rebuild CLIP cache + temporal_proportions.
"""
...
SRC = "domrachev03/sim_3stage_v3_success"
N_STRIDE = 10
OUT_TRAIN = "domrachev03/sim_3stage_v3_success_train_fs"
OUT_VAL = "domrachev03/sim_3stage_v3_success_val_fs"
...
def main() -> None:
    ...
    for global_idx in range(len(src)):
        ...
        if ep_idx != cur_ep:
            if cur_ep >= 0:
                _flush_ep(cur_ep)
            cur_ep = ep_idx
            train_in_ep = 0; val_in_ep = 0
            train_orig_to_new = []; val_orig_to_new = []
    ...
    for split_name, ds in (("train", train_ds), ("val", val_ds)):
        ...
        print(f"patched subtasks at {path}")
