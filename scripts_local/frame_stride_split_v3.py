"""Frame-stride train/val split for merged_v3 (108 eps; no broken eps to drop).

Per-ep stride: every N-th frame (frame_in_ep % N == N-1) → val.
Re-encodes videos. Subtask annotations regenerated per-ep based on counting
train/val frames within each subtask range.

Source: local/sim_assemble_sarm_merged_v3_full
Outputs:
  - local/sim_assemble_sarm_merged_v3_train_fs
  - local/sim_assemble_sarm_merged_v3_val_fs
  - outputs/iter7_split_meta.json (extended with split + new-val info)
"""
...
SRC = "local/sim_assemble_sarm_merged_v3_full"
N_STRIDE = 10
OUT_TRAIN = "local/sim_assemble_sarm_merged_v3_train_fs"
OUT_VAL = "local/sim_assemble_sarm_merged_v3_val_fs"
SOURCE_OFFSETS = {"filtered": 0, "_2": 51, "_3_3of4": 103}  # 51+52=103; _3 5 eps at 103-107
...
def _which_source(merged_idx: int) -> str:
    if merged_idx < SOURCE_OFFSETS["_2"]:
        return "filtered"
    if merged_idx < SOURCE_OFFSETS["_3_3of4"]:
        return "_2"
    return "_3_3of4"
...
def main() -> None:
    ...
    def _flush_ep(ep_idx: int) -> None:
        ...
        if train_buf:
            ...
            new_subtasks["train"].append({
                "src_merged_idx": ep_idx, "src": _which_source(ep_idx),
        ...
        if val_buf:
            ...
            if _which_source(ep_idx) == "_3_3of4":
    ...
    print(f"  new_val (src=_3_3of4): {len(new_val_eps_from_3)} eps in val ds: {new_val_eps_from_3}")
    ...
    existing = json.load(open(META_OUT)) if META_OUT.exists() else {}
    existing.update({
        "stride_n": N_STRIDE,
    ...
    META_OUT.write_text(json.dumps(existing, indent=2))
