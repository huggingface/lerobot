"""Frame-stride train/val split with subtask annotation regeneration.

Source ds: local/sim_assemble_sarm_merged_v2_full
Drops 4 broken _3 eps (merged ids 105, 111, 114, 115).
Per-ep stride: every N-th frame (frame_in_ep % N == N-1) → val, rest → train.

Re-encodes train + val into new datasets. Subtask annotations regenerated
per-ep based on counting train/val frames within each subtask range.

Outputs:
  - local/sim_assemble_sarm_merged_v2_train_fs   (train; ~90% frames)
  - local/sim_assemble_sarm_merged_v2_val_fs     (val; ~10% frames, every Nth)
  - outputs/iter6_split_meta.json                (meta + new-val ep_idx in val ds)
"""
...
SRC = "local/sim_assemble_sarm_merged_v2_full"
DROP_MERGED = {105, 111, 114, 115}  # _3 ep 2/8/11/12 (0-based local; see build_sarm_merged_v2)
SOURCE_OFFSETS = {"filtered": 0, "_2": 51, "_3": 103}  # ep_idx ranges for each src
N_STRIDE = 10  # every 10th frame → val (~10% val, 90% train)

OUT_TRAIN = "local/sim_assemble_sarm_merged_v2_train_fs"
OUT_VAL = "local/sim_assemble_sarm_merged_v2_val_fs"
...
def _which_source(merged_idx: int) -> str:
    if merged_idx < SOURCE_OFFSETS["_2"]:
        return "filtered"
    if merged_idx < SOURCE_OFFSETS["_3"]:
        return "_2"
    return "_3"
...
def main() -> None:
    ...
    src_meta_rows = pq.read_table(
        src.root / "meta" / "episodes" / "chunk-000" / "file-000.parquet"
    ).to_pandas()
    ...
    # subtask annotations to write into each split's meta after build:
    #   new_subtasks[split][new_ep_idx] = {"sparse_subtask_names": [...], "sparse_subtask_start_frames":[...], "sparse_subtask_end_frames":[...], (dense same)}
    new_subtasks = {"train": [], "val": []}
    new_val_eps_from_3: list[int] = []  # new_ep_idx in val ds whose source is _3
    n_train_eps = 0
    n_val_eps = 0
    ...
    train_in_ep = 0
    val_in_ep = 0
    train_orig_to_new: list[int] = []  # train_orig_to_new[orig_frame_in_ep] = new_train_idx_in_ep (or -1 if val)
    ...
    def _flush_ep(ep_idx: int) -> None:
        ...
        if not train_buf and not val_buf:
            return
        # Compute new subtask boundaries from src
        ...
        def _adjust(orig_frame_in_ep_list: list[int], mapping: list[int]) -> list[int]:
            """Find largest new_idx <= orig boundary."""
            adj = []
            for f in orig_frame_in_ep_list:
                # mapping[i] = new_idx_or_-1 for orig frame i in ep
                # we want max new_idx among entries 0..f with mapping != -1
                best = -1
        ...
        if train_buf:
            ...
            new_subtasks["train"].append({
                "src_merged_idx": ep_idx,
                "src": _which_source(ep_idx),
                "sparse_subtask_names": names,
                "sparse_subtask_start_frames": tr_starts,
                "sparse_subtask_end_frames": tr_ends,
                "dense_subtask_names": names,
                "dense_subtask_start_frames": tr_starts,
                "dense_subtask_end_frames": tr_ends,
            })
        ...
        if val_buf:
            ...
            new_subtasks["val"].append({
                ...
                "src": _which_source(ep_idx),
                "sparse_subtask_names": names,
                "sparse_subtask_start_frames": vl_starts,
                "sparse_subtask_end_frames": vl_ends,
                "dense_subtask_names": names,
                "dense_subtask_start_frames": vl_starts,
                "dense_subtask_end_frames": vl_ends,
            })
            if _which_source(ep_idx) == "_3":
                new_val_eps_from_3.append(n_val_eps)
            val_ds.save_episode()
            val_buf = False
            n_val_eps += 1
    ...
    for global_idx in range(len(src)):
        ...
        frame_in_ep = len(train_orig_to_new)  # equals current orig position in ep
    ...
    print(f"  dropped frames (4 broken _3 eps): {n_dropped_frames}")
    print(f"  train: {n_train_eps} eps, {train_ds.num_frames} frames")
    print(f"  val:   {n_val_eps} eps, {val_ds.num_frames} frames")
    print(f"  new_val (src=_3): {len(new_val_eps_from_3)} eps")

    # Write subtask cols into each split's meta parquet
    for split_name, ds in (("train", train_ds), ("val", val_ds)):
        ...
        df["src_merged_idx"] = [r["src_merged_idx"] for r in rows]
        df["src"] = [r["src"] for r in rows]
    ...
    META_OUT.write_text(json.dumps({
        ...
        "new_val_eps_in_val_ds": new_val_eps_from_3,
