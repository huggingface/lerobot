...
- **Lazy re-exports**: packages that re-export torch-heavy names (`lerobot.policies`, `lerobot.utils`, `lerobot.lerobot_types`, `lerobot.rollout`, ...) import them under `if LAZY_IMPORTS:` with `else: __getattr__ = lazy_getattr(__name__)` (see `utils/import_utils.py`), so importing the package stays light. A module's own code imports such names where it uses them. `tests/test_torch_free_imports.py` fails if torch comes back on `lerobot-calibrate`, the find and setup commands, `lerobot.configs`, `lerobot.policies` or `lerobot-rollout`.
