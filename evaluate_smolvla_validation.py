...
def calculate_validation_metrics(
    policy, val_dataloader, device: torch.device, num_action_dims: int = 6
) -> dict:
...
def main(cfg: TrainPipelineConfig):
    ...
    if cfg.dataset.repo_id == "all/datasets":
        ...
        dataset_repo_ids = (
            [cfg.dataset.repo_id] if isinstance(cfg.dataset.repo_id, str) else cfg.dataset.repo_id
        )
    ...
    for repo_id in dataset_repo_ids:
        ...
        if total_episodes < 2:
            logging.warning(
                f"Dataset {repo_id} has only {total_episodes} episodes, skipping validation split"
            )
            continue
        ...
        val_dataset = LeRobotDataset(
            repo_id=repo_id, episodes=val_episodes, video_backend=cfg.dataset.video_backend
        )
    ...
    if all_val_metrics:
        ...
        avg_loss = (
            sum(m["validation_loss"] * m["n_examples"] for m in all_val_metrics.values()) / total_examples
        )
