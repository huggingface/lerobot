...
Example:
```shell
python -m lerobot.rtc_inference.policy_server \
     --host=127.0.0.1 \
     --port=8080 \
     --fps=30 \
...
from lerobot.policies.rtc import ActionQueue, LatencyTracker, RTCConfig
...
class PolicyServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def __init__(self, config: PolicyServerConfig):
        ...
        self.rtc_latency_tracker: LatencyTracker | None = None
    ...
    def SendPolicyInstructions(self, request, context):  # noqa: N802
        ...
        if self.rtc_enabled:
            if not hasattr(self.policy.config, "rtc_config"):
                self.logger.warning(
                    "RTC requested but policy '%s' has no rtc_config. Falling back to non-RTC.",
                    self.policy_type,
                )
                self.rtc_enabled = False
            else:
                self.policy.config.rtc_config = RTCConfig(
                    ...
                    prefix_attention_schedule=policy_specs.rtc_prefix_attention_schedule,
                    debug=policy_specs.rtc_debug,
                    debug_maxlen=policy_specs.rtc_debug_maxlen,
                )
                if hasattr(self.policy, "init_rtc_processor"):
                    self.policy.init_rtc_processor()

                self.rtc_action_queue = ActionQueue(self.policy.config.rtc_config)
                self.rtc_latency_tracker = LatencyTracker()
                self.rtc_last_queue_update_time = time.perf_counter()
        ...
        if self.rtc_enabled:
            self.logger.info(
                "RTC enabled on server | horizon=%s | max_weight=%s | schedule=%s | inference_delay_steps=%s",
                policy_specs.rtc_execution_horizon,
                policy_specs.rtc_max_guidance_weight,
    ...
    def GetActions(self, request, context):  # noqa: N802
        ...
        try:
            ...
            if len(action_chunk) == 0:
                self.logger.warning(
                    "[SERVER] Generated empty action chunk for obs #%s; returning Empty",
                    obs.get_timestep(),
                )
                return services_pb2.Empty()
    ...
    def _rtc_consume_elapsed_actions(self) -> None:
        """Advance RTC action queue index based on wall-clock elapsed time."""
        if not self.rtc_enabled or self.rtc_action_queue is None or self.rtc_last_queue_update_time is None:
            return

        now = time.perf_counter()
        elapsed = now - self.rtc_last_queue_update_time
        steps = int(elapsed / self.config.environment_dt)
        if steps <= 0:
            return
        ...
        self.rtc_last_queue_update_time += steps * self.config.environment_dt

    def _get_rtc_inference_delay(self) -> int:
        """Return current inference delay in steps for RTC."""
        if self.rtc_inference_delay_steps is not None:
            return int(self.rtc_inference_delay_steps)

        if self.rtc_latency_tracker is None:
            return 0
    ...
    def _get_action_chunk(self, observation: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if not self.rtc_enabled or self.rtc_action_queue is None:
            ...
            self._rtc_consume_elapsed_actions()
            self._rtc_action_index_before_inference = self.rtc_action_queue.get_action_index()
            prev_actions = self.rtc_action_queue.get_left_over()
            current_delay = self._get_rtc_inference_delay()
    ...
    def _predict_action_chunk(self, observation_t: TimedObservation) -> list[TimedAction]:
        ...
        if self.rtc_enabled and self.rtc_latency_tracker is not None:
        ...
        for i in range(chunk_size):
            ...
            single_action = raw_action_tensor[:, i, :]
        ...
        if self.rtc_enabled and self.rtc_action_queue is not None:
            ...
            self.rtc_action_queue.merge(
                ...
                action_index_before_inference=self._rtc_action_index_before_inference,
            )
            self.rtc_last_queue_update_time = time.perf_counter()
            action_tensor = self.rtc_action_queue.get_processed_left_over()
