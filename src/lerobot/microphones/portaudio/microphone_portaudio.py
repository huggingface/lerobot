...
class PortAudioMicrophone(Microphone):
    ...
    def _validate_sample_rate(self) -> None:
        ...
        actual_sample_rate = PortAudioMicrophone.find_microphones(self.microphone_index, self.sounddevice_sdk)["sample_rate"]
    ...
    def _validate_channels(self) -> None:
        ...
        actual_channels = PortAudioMicrophone.find_microphones(self.microphone_index, self.sounddevice_sdk)["channels"]

        if self.channels is not None and len(self.channels) > 0:
            if any(all(c > actual_channels) or c <= 0 or not isinstance(c, np.integer) for c in self.channels):
                raise RuntimeError(
