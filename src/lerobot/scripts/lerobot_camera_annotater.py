...
def make_mouse_callback(state, window_name):
    ...
    def on_mouse(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            state.hover_idx = (
                find_box_at_center(state, x, y) if state.moving_idx is None else state.moving_idx
            )
...
def draw_overlay(frame, state):
    ...
    for i, box in enumerate(state.boxes):
        ...
        cv2.rectangle(
            out, (cx - tw // 2 - 4, cy - th // 2 - 4), (cx + tw // 2 + 4, cy + th // 2 + 4), bg_color, -1
        )
        cv2.putText(
            out,
            text,
...
def parse_cam_id(cam_str):
    ...
    if cam_str.startswith("/dev/"):
        return cam_str
    try:
        # If it's just a number, return it as an integer for cross-platform support
        # (Linux uses /dev/videoN, macOS uses integer indices)
        return int(cam_str)
    except ValueError:
        return cam_str
...
def load_annotations(path: Path, states: dict):
    ...
    for json_path in files_to_load:
        try:
            ...
            # Find matching state by camera ID (try direct match first, then int conversion)
            matched_state = None
            matched_key = None
            if cam_id in states:
                matched_state = states[cam_id]
                matched_key = cam_id
            else:
                # Try converting string "0" to int 0 for matching
                try:
                    cam_id_int = int(cam_id)
                    if cam_id_int in states:
                        matched_state = states[cam_id_int]
                        matched_key = cam_id_int
                except (ValueError, TypeError):
                    pass

            if matched_state:
                matched_state.boxes = [[b["x0"], b["y0"], b["x1"], b["y1"], b["id"]] for b in boxes]
                matched_state.next_id = max((b["id"] for b in boxes), default=0) + 1
                print(f"Loaded {len(boxes)} boxes for {matched_key} from {json_path}")
                loaded += 1
            else:
                # Try to match by normalized camera ID (handles int vs string, path differences)
                cam_id_norm = str(cam_id).replace("/", "_")
                for state_cam_id, state in states.items():
                    state_cam_id_norm = str(state_cam_id).replace("/", "_")
                    if (
                        cam_id_norm == state_cam_id_norm
                        or cam_id_norm in state_cam_id_norm
                        or state_cam_id_norm in cam_id_norm
                    ):
...
def main():
    ...
    try:
            ...
            for cam_id in list(cameras.keys()):
                ...
                try:
                    ...
                    if state.last_frame is not None:
                        ...
                        cv2.putText(
                            annotated,
                            f"Camera read error ({state.failed_reads})",
                            (10, 60),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.6,
                            (0, 0, 255),
            ...
            if key == ord("q") or key == 27:
                ...
                for cam_id, s in states.items():
                    ...
                    if s.recording and s.writer is None:
                        safe = str(cam_id).replace("/", "_")
                        path = args.record_output.replace("{cam}", safe)
                        Path(path).parent.mkdir(parents=True, exist_ok=True)
                        s.writer = cv2.VideoWriter(
                            path, cv2.VideoWriter_fourcc(*"MJPG"), args.fps, (args.width, args.height)
                        )
                ...
                for cam_id, state in states.items():
                    ...
                    if frame_bgr is not None:
                        annotated = draw_overlay(frame_bgr, state)
                        safe_cam_id = str(cam_id).replace("/", "_")
                        # Save timestamped snapshot
                        path = f"outputs/snapshots/snapshot_{safe_cam_id}_{ts}.png"
                        cv2.imwrite(path, annotated)
                        # Save as "latest" for easy access
                        latest_path = f"outputs/snapshots/snapshot_latest_{safe_cam_id}.png"
                        cv2.imwrite(latest_path, annotated)
                        print(f"Saved: {path} (also: {latest_path})")
        ...
        # Save annotations (always save to persist cleared boxes too)
        out_dir = Path("outputs/annotations")
        out_dir.mkdir(parents=True, exist_ok=True)
        for cam_id, s in states.items():
            safe_cam_id = str(cam_id).replace("/", "_")
            data = {
                "camera": str(cam_id),  # Store as string for consistent matching
                "frame_width": args.width,
                "frame_height": args.height,
                "boxes": [{"id": i, "x0": x0, "y0": y0, "x1": x1, "y1": y1} for x0, y0, x1, y1, i in s.boxes],
            }
            path = out_dir / f"boxes_{safe_cam_id}.json"
            with open(path, "w") as f:
                json.dump(data, f, indent=2)
            print(f"Saved: {path} ({len(s.boxes)} boxes)")
