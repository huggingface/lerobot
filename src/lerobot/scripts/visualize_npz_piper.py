...
def main():
    ...
    parser.add_argument(
        "--key", type=str, default=None, help="Key in the NPZ file to use. If None, uses the first available."
    )
    ...
    def key_callback(keycode):
        if keycode == 32:
            state["paused"] = not state["paused"]  # Space
        elif keycode == 263:
            state["paused"] = True
            state["step"] = -1  # Left Arrow
        elif keycode == 262:
            state["paused"] = True
            state["step"] = 1  # Right Arrow
        elif keycode == 256:
            sys.exit(0)  # ESC
