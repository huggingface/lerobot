...
class ACTPolicyWithReward:
    ...
    def select_action(self, observation: dict[str, torch.Tensor]) -> tuple[torch.Tensor, float]:
        ...
        if self.recording_enabled:
            self.recorded_rewards.append({"step": self.step_count, "reward": reward_value})
    ...
    def get_current_reward(self) -> float | None:
...
def create_reward_visualization_video(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Generate frames
        for step_idx, (images, reward_info) in enumerate(zip(images_list, reward_data, strict=False)):
...
def create_reward_frame(
    ...
    # Graph background
    cv2.rectangle(
        canvas, (margin, graph_y), (canvas_width - margin, graph_y + graph_height), (60, 60, 60), -1
    )
    ...
    cv2.putText(
        canvas, reward_text, (text_x, text_y), font, font_scale * 1.2, (255, 255, 255), font_thickness + 1
    )
