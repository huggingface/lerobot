...
def main():
    ...
    if dataset.meta.episodes is None or len(dataset.meta.episodes) == 0:
         print(f"[visualize_piper] Error: No episodes found.")
         return
    ...
    def key_callback(keycode):
        if keycode == 32: state["paused"] = not state["paused"]
        elif keycode == 263: state["paused"] = True; state["step"] = -1
        elif keycode == 262: state["paused"] = True; state["step"] = 1
        elif keycode == 256: sys.exit(0)
    ...
    with mujoco.viewer.launch_passive(model, data, key_callback=key_callback) as viewer:
        ...
        while viewer.is_running():
            ...
            if state["offset"] >= length: state["offset"] = 0
            ...
            for k in camera_keys:
                if k in item:
                    ...
                    cv2.putText(img, k.replace("observation.images.", ""), (10, 30), 
