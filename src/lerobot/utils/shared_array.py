...
class SharedArray:
    ...
    SharedArray offers quasi-instantaneous array-wide read and flush capabilities in comparaison to Queues, but has a limited size defined at initialization.
    ...
    def __init__(self, shape: tuple[int], dtype: np.dtype | str):
        ...
        self.shared_memory = shared_memory.SharedMemory(create=True, size=np.prod(shape) * np.dtype(dtype).itemsize)
