...
class InferenceLogger:
    ...
    def log_robot_state(self, robot, observation: Dict[str, Any]):
        ...
        try:
            ...
            for key, value in observation.items():
                if '.pos' in key and not 'image' in key:
                    motor_positions[key] = float(value) if isinstance(value, (int, float, np.ndarray, torch.Tensor)) else str(value)
    ...
    def log_policy_inference(self, observation: Dict[str, Any], action: Dict[str, Any], 
                ...
                if 'image' not in key.lower():
                    if isinstance(value, (torch.Tensor, np.ndarray)):
                        if value.numel() == 1:  # Single value
                            ...
                            inference_data[f'obs_{key}'] = str(value.tolist() if hasattr(value, 'tolist') else value)
    ...
    def log_trajectory_point(self, robot_pose: Optional[Dict[str, float]] = None,
    ...
    def log_step_summary(self, observation: Dict[str, Any], action: Dict[str, Any], 
                        robot, policy_output: Optional[torch.Tensor] = None,
                        inference_time: float = 0.0, task: Optional[str] = None):
        """
