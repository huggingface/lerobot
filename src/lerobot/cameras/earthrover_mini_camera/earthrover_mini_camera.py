...
class VirtualCamera(Camera):
    ...
    Example:
        ```python
        from lerobot.cameras.earthrover_mini_camera.virtual_camera import VirtualCamera
        
        # Inside robot class
        self.cameras = {
            "front": VirtualCamera("front", self),
            "rear": VirtualCamera("rear", self)
        }
        
        # Later, after robot.get_observation() has been called:
        frame = robot.cameras["front"].read()  # Gets frame from cache
        ```
    """

    def __init__(self, name: str, robot, fps: int = 30, width: int = 640, height: int = 480):
        """
        Initialize the virtual camera adapter.
        
        Args:
            name: Camera name (e.g., "front", "rear")
            robot: Reference to the parent robot instance that provides observations
            fps: Frames per second (for compatibility, not enforced)
            width: Frame width in pixels
            height: Frame height in pixels
        """
        # Create a minimal config-like object for the parent Camera class
        class MinimalConfig:
            def __init__(self):
                self.fps = fps
                self.width = width
    ...
    def read(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray: Camera frame as (height, width, 3) array in RGB/BGR format,
        ...
        Note:
            ...
            is populated by the robot's get_observation() method. Make sure
            get_observation() has been called at least once before reading frames.
        """
        if hasattr(self.robot, '_last_observation'):
            if self.name in self.robot._last_observation:
                return self.robot._last_observation[self.name]
