...
def control_loop(
    ...
    try:
        while episode_idx < cfg.dataset.num_episodes_to_record:
            ...
            if use_gripper:
                neutral_action = torch.cat(
                    [neutral_action, torch.tensor([1.0])]
                )  # Gripper stay
            ...
            if cfg.mode == "record":
                ...
                frame = {
                    **observation,
                    ...
                    REWARD: np.array(
                        [transition[TransitionKey.REWARD]], dtype=np.float32
                    ),
            ...
            if terminated or truncated:
                ...
                transition = reset_and_build_transition(
                    env, env_processor, action_processor
                )
        ...
        if (
            dataset is not None
            and dataset.writer is not None
            and dataset.writer.image_writer is not None
        ):
