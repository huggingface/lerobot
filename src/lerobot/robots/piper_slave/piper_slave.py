...
from .config_piper_slave import PIPERSlaveConfig
...
class PIPERSlave(Robot):
    config_class = PIPERSlaveConfig
    name = "piper_slave"

    def __init__(self, config: PIPERSlaveConfig):
        super().__init__(config)
        self.config = config
        self.bus = PiperMotorsBus(
            PiperMotorsBusConfig(
                can_name=config.port,
                motors={
    ...
    def connect(self) -> None:
        ...
        if not self.config.read_only:
            ...
            print("piper slave connected (ACTIVE MODE)")
        else:
            # In read-only mode, we skip enabling the motors (EnablePiper)
            # The CAN port was already opened in __init__ via PiperMotorsBus -> C_PiperInterface_V2
            print("piper slave connected (PASSIVE/READ-ONLY MODE)")
    ...
    def disconnect(self) -> None:
        ...
        if not self.config.read_only:
            ...
            print("piper slave disconnecting (PASSIVE MODE)")
            # In passive mode, we might just want to close the CAN port if the SDK supports it, or just do nothing.
            # Assuming SDK handles cleanup or is fine.
