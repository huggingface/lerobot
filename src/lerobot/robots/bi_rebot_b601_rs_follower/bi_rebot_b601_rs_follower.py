...
from ..rebot_b601_rs_follower import RebotB601RSFollower, RebotB601RSFollowerRobotConfig
from ..robot import Robot
from .config_bi_rebot_b601_rs_follower import BiRebotB601RSFollowerConfig

logger = logging.getLogger(__name__)


class BiRebotB601RSFollower(BimanualMixin, Robot):
    """Bimanual Seeed Studio reBot B601-RS follower.

    Composes two single-arm :class:`RebotB601RSFollower` instances. Observation and
    action keys of each arm are namespaced with a ``left_`` / ``right_`` prefix.
    """

    config_class = BiRebotB601RSFollowerConfig
    name = "bi_rebot_b601_rs_follower"

    def __init__(self, config: BiRebotB601RSFollowerConfig):
        ...
        left_arm_config = RebotB601RSFollowerRobotConfig(
            ...
            gripper_mit_hold_torque_limit=config.left_arm_config.gripper_mit_hold_torque_limit,
            joint_limits=config.left_arm_config.joint_limits,
            joint_directions=config.left_arm_config.joint_directions,
        )

        right_arm_config = RebotB601RSFollowerRobotConfig(
            ...
            gripper_mit_hold_torque_limit=config.right_arm_config.gripper_mit_hold_torque_limit,
            joint_limits=config.right_arm_config.joint_limits,
            joint_directions=config.right_arm_config.joint_directions,
        )

        self.left_arm = RebotB601RSFollower(left_arm_config)
        self.right_arm = RebotB601RSFollower(right_arm_config)
