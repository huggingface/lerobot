...
class DynamixelMotorsBus(MotorsBus):
    ...
    def disable_torque(self, motors: str | list[str] | None = None, num_retry: int = NUM_WRITE_RETRY) -> None:
        for motor in self._get_motors_list(motors):
            self.write("Torque_Enable", motor, TorqueMode.DISABLED.value, num_retry=num_retry)

    def _disable_torque(self, motor_id: int, model: str, num_retry: int = NUM_WRITE_RETRY) -> None:
        addr, length = get_address(self.model_ctrl_table, model, "Torque_Enable")
        self._write(addr, length, motor_id, TorqueMode.DISABLED.value, num_retry=num_retry)

    def enable_torque(self, motors: str | list[str] | None = None, num_retry: int = NUM_WRITE_RETRY) -> None:
        for motor in self._get_motors_list(motors):
