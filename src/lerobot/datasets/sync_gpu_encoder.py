...
class SyncGPUVideoEncoder:
    ...
    def __init__(
        ...
        if self.gpu_encoding:
            ...
            if self.enable_logging:
                ...
                logger.info(f"Sync GPU encoder initialized: {encoder_info['selected_encoder']} {encoder_info['selected_codec']}")
        else:
            if self.enable_logging:
    ...
    def encode_episode_videos(
        ...
        """
        if self.enable_logging:
            logger.info(f"Encoding videos for episode {episode_index} using {'GPU' if self.gpu_encoding else 'CPU'} acceleration")
        
        start_time = time.time()
        
        for video_key in video_keys:
            # Construct paths
            video_path = root_path / "videos" / f"chunk-{episode_index//1000:03d}" / video_key / f"episode_{episode_index:06d}.mp4"
        ...
        if self.enable_logging:
            logger.info(f"Episode {episode_index} encoding completed in {encoding_time:.2f}s using {'GPU' if self.gpu_encoding else 'CPU'} acceleration")
    ...
    def batch_encode_videos(
        ...
        """
        if self.enable_logging:
            logger.info(f"Starting batch video encoding for episodes {start_episode} to {end_episode - 1} using {'GPU' if self.gpu_encoding else 'CPU'} acceleration")
        ...
        if self.enable_logging:
            logger.info(f"Batch video encoding completed in {total_time:.2f}s using {'GPU' if self.gpu_encoding else 'CPU'} acceleration")
...
def create_sync_gpu_encoder(
    ...
    return SyncGPUVideoEncoder(
        ...
        gpu_encoder_config=gpu_encoder_config,
        enable_logging=enable_logging
    )
