...
class RecordingBenchmark:
    ...
    def log_stats(self, force: bool = False) -> None:
        ...
        if force or frame_count % self.log_interval == 0:
            ...
            logging.info(f"  Frame capture: {self.frame_capture.mean_time*1000:.2f}ms ± {self.frame_capture.std_time*1000:.2f}ms")
            logging.info(f"  Frame processing: {self.frame_processing.mean_time*1000:.2f}ms ± {self.frame_processing.std_time*1000:.2f}ms")
            logging.info(f"  Image writing: {self.image_writing.mean_time*1000:.2f}ms ± {self.image_writing.std_time*1000:.2f}ms")
