...
class GR00TN17ActionHead(nn.Module):
    ...
    def get_action_with_features(
        ...
        def denoise_step(actions: torch.Tensor, time_tensor: torch.Tensor) -> torch.Tensor:
            # The solver hands us the float32 continuous time; recover the integer step from it so
            # the timestep buckets keep being derived from the exact `t_step / num_inference_timesteps`.
            t_step = round(float(time_tensor[0]) * self.num_inference_timesteps)
