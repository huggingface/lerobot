...
"""Wall-OSS-0.5 bridge for the generic language runtime."""
...
class WallOSS05PolicyAdapter(BaseLanguageAdapter):
    """Expose Wall's native subtask, VQA, and action paths to the runtime."""
    ...
    def generate_text(
        ...
        if len(output) != 1:
            raise ValueError(f"The interactive runtime expected one Wall text output, got {len(output)}.")
        return output[0]
