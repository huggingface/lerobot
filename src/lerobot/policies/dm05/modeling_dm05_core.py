...
def make_suffix_attn_mask(
    ...
    suffix_key_padding_mask: torch.Tensor | None = None,
) -> torch.Tensor:
    """Build the suffix-to-prefix-plus-suffix attention mask.

    Each suffix token can attend to non-action prefix tokens and valid suffix tokens.

    Prefix action-label tokens are masked to avoid leaking discrete actions.
    Padded suffix keys are masked so repeated episode-tail targets cannot
    influence valid action timesteps.
    ...
    if suffix_key_padding_mask is not None:
        expected_shape = (batch_size, suffix_len)
        if suffix_key_padding_mask.shape != expected_shape:
            raise ValueError(f"suffix_key_padding_mask must have shape {expected_shape}.")
        suffix_mask = suffix_mask.masked_fill(
            suffix_key_padding_mask.to(device=device, dtype=torch.bool).unsqueeze(1),
            neg_inf,
        )
...
def masked_flow_matching_loss(
    ...
    *,
    ...
    has_actions: torch.Tensor | None = None,
) -> torch.Tensor:
    """Mean squared error over real action elements only."""
    if prediction.shape != target.shape or prediction.ndim != 3:
        raise ValueError(
            f"DM05 flow matching expects matching [B,T,D] tensors, got "
    ...
    if action_dim_mask is not None:
        if action_dim_mask.shape != (batch_size, action_dim):
            raise ValueError(f"action_dim_mask must have shape {(batch_size, action_dim)}.")
        valid &= action_dim_mask.to(device=prediction.device, dtype=torch.bool).unsqueeze(1)
    if action_is_pad is not None:
        if action_is_pad.shape != (batch_size, horizon):
            raise ValueError(f"action_is_pad must have shape {(batch_size, horizon)}.")
        valid &= ~action_is_pad.to(device=prediction.device, dtype=torch.bool).unsqueeze(-1)
    if action_prefix_mask is not None:
        if action_prefix_mask.shape != (batch_size, horizon):
            raise ValueError(f"action_prefix_mask must have shape {(batch_size, horizon)}.")
        valid &= ~action_prefix_mask.to(device=prediction.device, dtype=torch.bool).unsqueeze(-1)
    elementwise = torch_nn_functional.mse_loss(prediction, target, reduction="none")
    valid = valid.to(dtype=elementwise.dtype)
    valid_per_sample = valid.sum(dim=(1, 2)).clamp(min=1)
    loss_per_sample = (elementwise * valid).sum(dim=(1, 2)) / valid_per_sample
    if has_actions is None:
        has_actions = torch.ones(batch_size, device=prediction.device, dtype=torch.bool)
    elif has_actions.shape != (batch_size,):
        raise ValueError(f"has_actions must have shape {(batch_size,)}.")
    has_actions = has_actions.to(device=prediction.device, dtype=elementwise.dtype)
    return (loss_per_sample * has_actions).sum() / has_actions.sum().clamp(min=1)


def _expand_action_dim_mask(
    action_dim_mask: torch.Tensor | None,
    reference: torch.Tensor,
) -> torch.Tensor | None:
    if action_dim_mask is None:
        return None
    expected_shape = (reference.shape[0], reference.shape[-1])
    if action_dim_mask.shape != expected_shape:
        raise ValueError(f"action_dim_mask must have shape {expected_shape}.")
    return action_dim_mask.to(device=reference.device, dtype=reference.dtype).unsqueeze(1)
...
def apply_dm05_gradient_checkpointing(
    *,
    ...
    if vision_tower is not None:
        method_name = "gradient_checkpointing_enable" if vlm_enable else "gradient_checkpointing_disable"
        method = getattr(vision_tower, method_name, None)
        if callable(method):
            kwargs = {"gradient_checkpointing_kwargs": {"use_reentrant": False}} if vlm_enable else {}
            method(**kwargs)
        elif hasattr(vision_tower, "gradient_checkpointing"):
            vision_tower.gradient_checkpointing = vlm_enable
...
class DM05ForCausalLM(DM05CorePreTrainedModel):
    ...
    def __init__(self, config: DM05CoreModelConfig):
        ...
        if vlm_n_layers != ae_n_layers:
            raise ValueError(
                f"VLM has {vlm_n_layers} layers but Action Expert has {ae_n_layers}. "
                "They must match for merged attention. "
    ...
    def _apply_liger_kernel(self):
        ...
        logger.info("Applied Liger Kernel: VLM (RMSNorm+GeGLU+RoPE+LayerNorm), AE (RMSNorm+GeGLU)")
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Build flow-matching inputs. Invalid dimensions and padded timesteps
        # must not enter the shared action projection or suffix attention.
        expanded_action_dim_mask = _expand_action_dim_mask(action_dim_mask, actions)
        if expanded_action_dim_mask is not None:
            actions = actions * expanded_action_dim_mask
        valid_action_timesteps = None
        if action_is_pad is not None:
            expected_shape = (batch_size, actions.shape[1])
            if action_is_pad.shape != expected_shape:
                raise ValueError(f"action_is_pad must have shape {expected_shape}.")
            valid_action_timesteps = ~action_is_pad.to(device=actions.device, dtype=torch.bool)
            actions = actions * valid_action_timesteps[..., None]

        noise = torch.randn_like(actions)
        if expanded_action_dim_mask is not None:
            noise = noise * expanded_action_dim_mask
        if valid_action_timesteps is not None:
            noise = noise * valid_action_timesteps[..., None]
        time = (
        ...
        if expanded_action_dim_mask is not None:
            x_t = x_t * expanded_action_dim_mask
        if valid_action_timesteps is not None:
            x_t = x_t * valid_action_timesteps[..., None]
        u_t = noise - actions
        ...
        suffix_attn_mask = make_suffix_attn_mask(
            ...
            suffix_key_padding_mask=action_is_pad,
        )
        ...
        fm_loss = masked_flow_matching_loss(
            v_t,
            u_t,
    ...
    def inference_action(
        ...
        **kwargs,
        ...
        if action_prefix_mask is not None:
            raise ValueError("action_prefix_mask is only supported with use_compiled_suffix=True.")
        validate_action_prefill_pair(prefill_actions, action_prefill_len)
        action_prefix_mask = build_action_prefix_mask(
            action_prefill_len,
        ...
        expected_shape = (batch_size, self.model.config.chunk_size, self.model.config.action_dim)
        if initial_noise is None:
            x_t = torch.randn(*expected_shape, device=device, dtype=dtype)
        else:
    ...
    def _inference_action_compiled_suffix(
        ...
        *,
        ...
        action_dim_mask: torch.BoolTensor | None,
        dtype: torch.dtype,
    ) -> torch.Tensor:
        ...
        prefill_actions = prefill_actions.to(device=device, dtype=dtype)
        expanded_action_dim_mask = _expand_action_dim_mask(action_dim_mask, initial_noise)
