...
class LatentLAMModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.vq_type == "nsvq":
            self.vq = NSVQ(codebook_size=codebook_size, code_dim=code_dim, use_diveq=False, **vq_kwargs)
        elif self.vq_type in ("ema", "ema_vq"):
            self.vq = EMAVQ(codebook_size=codebook_size, code_dim=code_dim, **vq_kwargs)
        elif self.vq_type == "vq":
            self.vq = VQ(codebook_size=codebook_size, code_dim=code_dim, **vq_kwargs)
            ...
            self.vq = NSVQ(codebook_size=codebook_size, code_dim=code_dim, **vq_kwargs)
    ...
    def _run(
        ...
        states: torch.Tensor | None,  # [B,T,D]
        dec_videos: torch.Tensor,  # [B,T,C,H,W]
        state_mask: torch.Tensor | None = None,
        embodiment_ids: torch.Tensor | None = None,
        user_specific: int | None = None,
        ...
        if self.train_in_latent:
            if self._uses_vjepa_split_features():
                if T != 2:
                    raise ValueError(f"V-JEPA LAM expects endpoint clips with T=2, got T={T}.")
    ...
    @torch.no_grad()
    def extract_vision_features(self, videos: torch.Tensor, *, n: Any | None = -2) -> torch.Tensor:
