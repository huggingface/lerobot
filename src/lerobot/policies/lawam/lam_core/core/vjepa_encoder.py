...
def _get_existing_path(model_id: str) -> Optional[Path]:
    path = Path(str(model_id)).expanduser()
    return path if path.exists() else None


def _find_local_vjepa_repo() -> Optional[Path]:
...
def _infer_vjepa_hub_name(model_id: str) -> Optional[str]:
...
def _is_cosmos_tokenizer_dir(model_id: Union[str, Path]) -> bool:
    path = Path(str(model_id)).expanduser()
...
def _looks_like_cosmos_model_id(model_id: Union[str, Path]) -> bool:
    key = str(model_id).lower()
    return "cosmos" in key or _is_cosmos_tokenizer_dir(model_id)


def _infer_cosmos_variant(model_id: Union[str, Path]) -> Tuple[Optional[str], Optional[int]]:
...
def _safe_torch_load(path: Union[str, Path]) -> dict:
    try:
...
class VJEPAEncoder(nn.Module):
    ...
    def __init__(
        ...
        if existing_path is not None and existing_path.is_dir():
            ...
            checkpoint_path = str(existing_path) if existing_path is not None and existing_path.is_file() else None
    ...
    def _load_torch_hub_model(self, hub_name: str, checkpoint_path: Optional[str]) -> None:
    ...
    def encode(
        self,
        images: torch.Tensor,
        norm_latents: Optional[bool] = None,
        n: int=-1
    ) -> torch.Tensor:
        if images.dim() != 4:
            ...
            T=1
        assert images.dim() == 4, f"Expected a 4D tensor [B*T, C, H, W], got: {images.shape}; invalid image dimensions"
            ...
            if self.enable_norm:
                if self.norm_layer_type == "bn":
                    if self.latent_norms is None:
                        raise ValueError("DINOv3Encoder has no BN layer initialized; set norm_layer_type to 'bn'.")
                    ...
                    if self.latent_norms is None:
                        raise ValueError("DINOv3Encoder has no LN layer initialized; set norm_layer_type to 'ln'.")
...
class CosmosAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        if not (model_path.exists() and model_path.is_dir()):
            raise ValueError("CosmosAutoencoder expects model_id to be a local directory containing weight files.")
        ...
        if enc.exists() and dec.exists():
            self.encoder_model = torch.jit.load(str(enc), map_location="cpu").eval()
            self.decoder_model = torch.jit.load(str(dec), map_location="cpu").eval()
            self.autoencoder_model = torch.jit.load(str(ae), map_location="cpu").eval() if ae.exists() else None
        elif ae.exists():
            self.encoder_model = None
            self.decoder_model = None
            self.autoencoder_model = torch.jit.load(str(ae), map_location="cpu").eval()
        else:
    ...
    def _apply_latent_norm(self, encoded_features: torch.Tensor, use_norm: bool) -> torch.Tensor:
        ...
        if self.norm_layer_type == "bn":
            if self.latent_norm is None:
                raise ValueError("CosmosAutoencoder has no BN layer initialized; set norm_layer_type to 'bn'.")
            flat_features = encoded_features.reshape(-1, self.feature_dim)
            flat_features = self.latent_norm(flat_features)
            return flat_features.view_as(encoded_features)
        if self.norm_layer_type == "ln":
            if self.latent_norm is None:
                raise ValueError("CosmosAutoencoder has no LN layer initialized; set norm_layer_type to 'ln'.")
            return self.latent_norm(encoded_features)
        if self.norm_layer_type == "l2":
            return F.normalize(encoded_features, p=2, dim=-1)
        raise ValueError(
    ...
    def _module_param_dtype(self, module: Optional[nn.Module], default: torch.dtype = torch.float32) -> torch.dtype:
        if module is None:
            return default
        try:
    ...
    def autoencode(self, images: torch.Tensor) -> torch.Tensor:
        ...
        if self.encoder_model is None or self.decoder_model is None:
            raise RuntimeError("The current Cosmos weight directory is missing model files required for autoencode.")
