...
class FeatureFusionBlock(nn.Module):
    def __init__(
        self, features, activation, deconv=False, bn=False, expand=False, align_corners=True, size=None
    ):
        super().__init__()
...
def _make_fusion_block(features, use_bn, size=None):
    return FeatureFusionBlock(
        features, nn.ReLU(inplace=True), deconv=False, bn=use_bn, expand=False, size=size
    )
...
class DPTHead(nn.Module):
    ...
    def __init__(
        ...
        self.projects = nn.ModuleList(
            [nn.Conv2d(in_ch_list[i], out_channels[i], kernel_size=1) for i in range(len(out_channels))]
        )
        ...
        self.refine = nn.ModuleList(
            [_make_fusion_block(features, use_bn) for _ in range(len(out_channels) - 1)]
        )
        ...
        for i, x in enumerate(out_features):
            ...
            if g * g != N:
                ...
                if self.grid_size is not None and self.grid_size * self.grid_size == N:
                    ...
                    raise ValueError(
                        f"Input #{i} token length N={N} not a perfect square; can't reshape to map. "
            ...
            if C != expected_in_ch:
                raise RuntimeError(
                    f"Layer {i}: token channel C={C} doesn't match DPTHead configured in_channels={expected_in_ch}. "
