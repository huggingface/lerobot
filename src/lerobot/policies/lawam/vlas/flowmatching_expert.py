...
class ConditionalFlowMatchingHead(nn.Module):
    def __init__(self, config: ConditionalFlowMatchingConfig | None = None):
        super().__init__()
        ...
        if int(self.config.num_target_vision_tokens) > 0:
            self.future_tokens: nn.Embedding | None = nn.Embedding(
    ...
    def _expand_future_tokens(
        ...
        *,
        ...
        future_token_valid = torch.ones((batch_size, future_tokens.shape[1]), dtype=torch.bool, device=device)
    ...
    def _build_hidden_timesteps(
        *,
        ...
        prefix_timestep = (
            ...
            .round()
            .to(dtype=action_timesteps.dtype)
        )
    ...
    def forward(
        ...
        if self.config.token_independent_noise:
            ...
            noise = self.sample_noise((flat_token_count, actions.shape[-1]), device, actions.dtype).view_as(
                actions
            )
        ...
        noisy_trajectory_emb = noisy_trajectory_emb * token_valid.unsqueeze(-1).to(
        ...
        if self.config.use_state:
            ...
            if future_tokens is not None and future_token_valid is not None:
                ...
                hidden_attention_mask = torch.cat([state_token_valid, future_token_valid, token_valid], dim=1)
            else:
        ...
        if attention_mask is not None:
            ...
            encoder_attention_mask = torch.cat(
                [vision_mask_bool, vlm_mask_bool], dim=1
            )  # [B, 512 + vlm_seq_len]
        else:
