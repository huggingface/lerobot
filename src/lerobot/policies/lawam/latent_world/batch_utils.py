...
def _cap_source_aspect_video_uint8(
    ...
    if videos_uint8.ndim == 4:
        ...
        raise ValueError(
            f"Expected tensor with shape [T, 3, H, W] or [B, T, 3, H, W], got {tuple(videos_uint8.shape)}."
        )
...
def _apply_default_spatial_preprocess_chw_uint8(
    ...
    if frames_uint8.ndim != 4 or int(frames_uint8.shape[1]) != 3:
        raise ValueError(
            f"Expected frames tensor with shape [T, 3, H, W] or [3, H, W], got {tuple(frames_uint8.shape)}."
        )
