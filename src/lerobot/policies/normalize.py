...
def create_stats_buffers(
    ...
    for key, ft in features.items():
        ...
        if stats:
            if (
                norm_mode is NormalizationMode.MEAN_STD
                and "mean" in stats[key]
                and isinstance(stats[key]["mean"], np.ndarray)
            ) or (
                norm_mode is NormalizationMode.MIN_MAX
                and "min" in stats[key]
                and isinstance(stats[key]["min"], np.ndarray)
            ):
                if norm_mode is NormalizationMode.MEAN_STD:
                ...
                norm_mode is NormalizationMode.MEAN_STD
                and "mean" in stats[key]
                and isinstance(stats[key]["mean"], torch.Tensor)
            ) or (
                norm_mode is NormalizationMode.MIN_MAX
                and "min" in stats[key]
                and isinstance(stats[key]["min"], torch.Tensor)
            ):
