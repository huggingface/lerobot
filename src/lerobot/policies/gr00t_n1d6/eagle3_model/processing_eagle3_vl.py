...
def get_video_reader_backend() -> str:
    return "pyav" if is_pyav_available() else "torchvision"
...
class Eagle3_VLProcessorKwargs(ProcessingKwargs, total=False):  # noqa: N801
...
class Eagle3_VLProcessor(ProcessorMixin):  # noqa: N801
    r"""
            ...
            def repl(match):
                ...
                if media_type == "image":
                    ...
                    if frame_timestamps is not None:
                        assert len(frame_timestamps) == len(num_of_tokens_list), (
    ...
    def from_args_and_dict(cls, args, processor_dict: dict[str, Any], **kwargs):
        ...
        # Ensure unused_kwargs is a dict before updating
        if isinstance(unused_kwargs, dict):
            kwargs.update(unused_kwargs)
        elif unused_kwargs:
            # If it's not a dict but not empty, log a warning and skip
            import warnings

            warnings.warn(
                f"validate_init_kwargs returned non-dict: {type(unused_kwargs)}, skipping update",
                stacklevel=2,
            )
