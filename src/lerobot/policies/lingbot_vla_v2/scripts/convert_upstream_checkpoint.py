...
def main():
    ...
    parser.add_argument(
        "--robot-config-path",
        default=None,
        help="Per-embodiment robot config YAML. Optional: omit to convert the format only "
        "(no embodiment baked in); provide it at fine-tuning time instead.",
    )
    ...
    # When --robot-config-path is omitted (format-only conversion), fall back to the
    # canonical camera list — the embodiment is decided at fine-tuning time.
    camera_keys = []
    ...
    if not camera_keys:
        camera_keys = list(LingbotVLAV2Config().canonical_cameras)

    # Convert robot_config YAML to typed slot-mapping fields when provided.
    state_slots = None
    action_slots = None
    camera_mapping = None
    if args.robot_config_path:
        from ..configuration_lingbot_vla_v2 import SlotMapping

        state_slots = {}
        for entry in _robot_config.get("states", []):
            for slot_key, slot_cfg in entry.items():
                state_slots[slot_key] = SlotMapping(origin_keys=slot_cfg.get("origin_keys", []))
        action_slots = {}
        for entry in _robot_config.get("actions", []):
            for slot_key, slot_cfg in entry.items():
                action_slots[slot_key] = SlotMapping(
                    origin_keys=slot_cfg.get("origin_keys", []),
                    subtract_state=slot_cfg.get("subtract_state", False),
                )
        camera_mapping = {}
        for entry in _robot_config.get("images", []):
            if isinstance(entry, str):
                continue
            for canonical, convert_info in entry.items():
                if isinstance(convert_info, dict):
                    raw_key = convert_info.get("origin_keys")
                    if isinstance(raw_key, str):
                        camera_mapping[canonical] = raw_key
    ...
    config = LingbotVLAV2Config(
        ...
        camera_mapping=camera_mapping or None,
    ...
    # When no slot mappings are provided (format-only conversion), the processor uses
    # identity passthrough (raw features assumed canonical) so users can evaluate
    # without training first; fine-tuning overrides with the user's own slot mappings.
