...
class DepthTeacherBundle:
    ...
    def depth_targets(self, pil_images: torch.Tensor) -> torch.Tensor:
        """Port of upstream ``get_depth_target``; returns [B, 256, 1024] bf16."""
        ...
        return target.permute(0, 2, 3, 1).reshape(batch_size, -1, target.shape[1]).to(dtype=torch.bfloat16)
    ...
    def video_targets(
        ...
        if effective_fps is not None:
            # Preserve per-sample spacing (episode-tail clipping can differ in
            # the same batch); the teacher RoPE supports a [B] FPS tensor.
