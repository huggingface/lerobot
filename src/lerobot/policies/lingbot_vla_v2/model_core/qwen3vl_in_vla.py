...
def preprcess_grid_thw(self, grid_thw: torch.Tensor):
    ...
    max_seqlen = int((cu_seqlens[1:] - cu_seqlens[:-1]).max().item())
    # Rotary/grid metadata is constant, but interpolated pos_embed values depend
    # on a trainable parameter. Caching those values also caches their autograd
    # graph, which is freed after the first backward and stale after an update.
    # Inference can still materialize the complete cache for CUDA graph capture.
    pos_embeds = None
    if not (torch.is_grad_enabled() and self.pos_embed.weight.requires_grad):
...
def forward_without_grid_thw(
    ...
    **kwargs,
    ...
    if position_embeddings is None or cu_seqlens is None or max_seqlen is None:
        ...
        if position_embeddings[0].shape[0] != hidden_states.shape[0]:
            pos_embeds, position_embeddings, cu_seqlens, _, max_seqlen = self.preprcess_grid_thw(grid_thw)

    if (
        pos_embeds is None
        or pos_embeds.shape[0] != hidden_states.shape[0]
        or (torch.is_grad_enabled() and self.pos_embed.weight.requires_grad)
    ):
        # Build a fresh parameter-dependent graph every training forward. Do not
        # detach: the learned positional embedding must keep receiving gradients.
        pos_embeds = self.fast_pos_embed_interpolate(grid_thw)
