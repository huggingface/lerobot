...
class PreTrainedPolicy(nn.Module, HubMixin, abc.ABC):
    ...
    def predict_action_chunk_with_text(
        ...
        """Predict an action chunk together with the text belonging to it.

        The text is whichever string this policy produced that explains this chunk,
        at no cost beyond the action prediction itself:

        - Policies that emit text and actions from one inference pass (G0.5's System 2
          chain-of-thought) return the text generated in that pass, which the action is
          conditioned on.
        - Policies whose text head runs as its own pass (EO-1, WALL-OSS, Wall-OSS-0.5)
          return the subtask this chunk was conditioned on, generated by the same text
          head on an earlier cycle and supplied in `batch["subtask"]`.

        Never runs an extra generation pass: a caller wanting fresh text on demand calls
        `generate_text` directly. Policies without a text head inherit this default and
        return `text=None`.
