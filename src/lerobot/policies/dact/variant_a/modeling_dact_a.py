...
class DACTPolicyA(PreTrainedPolicy):
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # Directly pass the current (single-step) normalized observation batch to the model.
        # Unlike DiffusionPolicy, DACT's `generate_actions` expects a single observation, not a
        # history of `n_obs_steps` stacked along the sequence dimension.
...
class DACT(nn.Module):
    ...
    def generate_actions(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        """
        # Call forward in inference mode to get actions
        self.eval()
        with torch.no_grad():
            actions, _ = self.forward(batch)
        return actions
    ...
    def forward(self, batch: dict[str, Tensor]) -> tuple[Tensor, tuple[Tensor, Tensor] | tuple[None, None]]:
        ...
        global_cond_feats = [obs_summary] # Start with transformer encoder summary from attention pooling

        # Add direct state features
