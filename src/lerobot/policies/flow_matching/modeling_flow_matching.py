...
class ActionFlowNet(nn.Module):
    ...
    def forward(self, x_t: torch.Tensor, t: torch.Tensor, conditions: torch.Tensor) -> torch.Tensor:
        ...
        # 2. Process Sequence
        seq_idx = torch.arange(horizon, device=device).unsqueeze(0).expand(batch_size, -1)
        ...
        time_token = t_feat.unsqueeze(1).expand(-1, 1 + horizon, -1)

        # Pad Position feature to match the condition token -> [B, 1+T, H]
        # Condition token gets a zeroed positional embedding
        cond_pos = torch.zeros((batch_size, 1, self.hidden_dim), device=device)
...
class FlowMatchingEulerSolver:
    ...
    @torch.no_grad()
    def solve(
        self, vector_field_fn, x_0: torch.Tensor, conditions: dict = None, omega: float = 1.0
    ) -> torch.Tensor:
        device = x_0.device
        x_t = x_0.clone()
        ...
        for step in range(self.num_sampling_steps):
            ...
            t_tensor = torch.full((batch_size,), t_val, device=device, dtype=x_t.dtype)

            if omega == 1.0:
                v_pred = vector_field_fn(x_t, t_tensor, conditions)
            else:
                v_cond = vector_field_fn(x_t, t_tensor, conditions)
                uncond_conditions = dict.fromkeys(conditions)
...
def get_optimal_transport_target(
    x_1: torch.Tensor, x_0: torch.Tensor
    ...
    t = torch.rand((batch_size,), device=device, dtype=x_1.dtype)
    t_broadcast = t.view(batch_size, *([1] * (x_1.ndim - 1)))
...
class FlowMatchingPolicy(PreTrainedPolicy):
    ...
    def __init__(self, config: FlowMatchingConfig | None = None, **kwargs):
        ...
        self.qpos_force_proj = nn.Sequential(
            nn.Linear(6, self.hidden_dim), nn.Mish(), nn.Linear(self.hidden_dim, self.hidden_dim)
        )
    ...
    def _extract_inputs_from_batch(self, batch: dict[str, torch.Tensor]):
        ...
        images = torch.stack(image_list, dim=1) if len(image_list) > 0 else None
    ...
    def extract_condition(
        ...
        if qpos is not None:
            ...
            qpos_kin_feat = torch.zeros((batch_size, self.hidden_dim), device=device)
            qpos_force_feat = torch.zeros((batch_size, self.hidden_dim), device=device)

        if images is not None:
            flat_imgs = images.view(-1, 3, images.shape[-2], images.shape[-1])
            img_feats = self.vision_backbone(flat_imgs).squeeze(-1).squeeze(-1)
            img_feats = self.image_proj(img_feats).view(batch_size, num_cameras * self.hidden_dim)
        else:
            img_feats = torch.zeros((batch_size, num_cameras * self.hidden_dim), device=device)

        if uncond_prob > 0.0:
            drop_mask = (torch.rand(batch_size, device=device) > uncond_prob).float().unsqueeze(1)
    ...
    def predict_action_chunk(
        ...
        x_0 = torch.randn((batch_size, self.config.max_horizon, self.action_dim), device=device)
        action_seq = solver.solve(
            vector_field_fn=vector_field_wrapper, x_0=x_0, conditions=conditions, omega=self.config.omega
        )
