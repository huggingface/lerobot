...
from lerobot_policy_sarm.sarm_utils import normalize_stage_tau
...
def load_sarm_resources(
    ...
    image_keys = list(getattr(reward_model.config, "image_keys", None) or [reward_model.config.image_key])
    image_key = image_keys[0]  # primary key for visualization/single-cam paths
    ...
    for k in image_keys:
        delta_timestamps[k] = [idx / fps for idx in delta_indices]
...
def visualize_sarm_predictions(
    ...
    Args:
        ...
        preprocess: Preprocessor from make_sarm_ext_pre_post_processors
    ...
    for episode_idx in episode_indices:
        ...
        for frame_idx in tqdm(frame_indices, desc=f"Episode {episode_idx}", leave=False):
            ...
            batch = {
                **{k: sample[k] for k in image_keys},
                "task": task,
                "index": frame_idx,
