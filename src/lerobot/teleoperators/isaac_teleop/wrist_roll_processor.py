...
carries an explicit engage-relative ``wrist_roll`` angle [rad] (from
...
class OverwriteWristRollFromAngle(RobotActionProcessorStep):
    """Overwrites the ``wrist_roll`` joint target from an engage-relative angle [rad].
    ...
    emits ``wrist_roll`` every frame, but if the key is absent (e.g. this step is
    reused in a pipeline that does not produce a roll) it leaves the IK-produced
    ``wrist_roll.pos`` untouched rather than snapping the wrist.

    Input keys:
        - ``wrist_roll``: ``float`` — engage-relative wrist-roll angle [rad]. Optional: when
