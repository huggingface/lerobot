...
class KeyboardRoverTeleop(Teleoperator):
    ...
    def get_action(self) -> dict[str, Any]:
        ...
        Returns:
            dict with 'linear_velocity' and 'angular_velocity' keys
        """
        before_read_t = time.perf_counter()
        ...
        # Speed adjustment - only process once per key press
        if '+' in active_keys or '=' in active_keys:
            if '+' not in self.current_pressed or not self.current_pressed.get('+', False):
        ...
        if '-' in active_keys:
            if not self.current_pressed.get('-', False):
