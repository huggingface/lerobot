...
class BiSO101Leader(Teleoperator):
    ...
    def action_features(self) -> dict[str, type]:
        features: dict[str, type] = {f"left_{motor}.pos": cast(type, float) for motor in self.left_arm.bus.motors} | {
            f"right_{motor}.pos": cast(type, float) for motor in self.right_arm.bus.motors
        }
        return features
