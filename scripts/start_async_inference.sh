...
if [ -n "$RTC_CONFIG_INDEX" ]; then
    echo "RTC Sweep: config_index=$RTC_CONFIG_INDEX, batch=$RTC_BATCH"
    echo "Results will be saved to: results/rtc_sweep/batch${RTC_BATCH}_config${RTC_CONFIG_INDEX}.csv"
fi
echo ""
...
uv run python examples/tutorial/async-inf/robot_client_improved.py
