...
class OrchestratorRTCXVLAConfig:
    ...
    def __post_init__(self):
        ...
        if self.policy_type.lower() != "xvla":
            raise ValueError("This script is dedicated to RTC + X-VLA. Please set --policy_type=xvla")
...
def _get_actions_thread(
    ...
    while not shutdown_event.is_set():
        ...
        autocast_ctx = (
            torch.autocast(device_type=device_type, dtype=amp_dtype) if use_autocast else nullcontext()
        )
