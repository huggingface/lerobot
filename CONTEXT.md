...
The single video file from a rollout artifact that is logged as run media, chosen by a stable sort over `(video_key, chunk_index, file_index)`. In Dataset v3 one `.mp4` holds however many episodes fit under the writer's file-size target, so a representative video is an episode *span*, not one episode. Every other video stays in the Artifact only.
