...
def visualize_episode(repo_id, episode_idx, root=None):
    ...
    if episode_idx < 0 or episode_idx >= total_episodes:
        print(f"Error: Episode {episode_idx} out of range (0-{total_episodes - 1})")
        return

    print(f"Visualizing Episode {episode_idx}")
    print("Initializing Rerun...")
    rr.init(f"LeRobot Episode {episode_idx} Visualizer", spawn=True)
    ...
    stride = 1  # 1x speed for single episode inspection by default? Or keep 7? Let's use 1.

    for global_step, i in enumerate(range(from_idx, to_idx, stride)):
        # Set timelines
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stream a single LeRobot episode directly to Rerun.")
    ...
    visualize_episode(args.repo_id, args.episode, args.root)
