...
def measure_component_times():
    ...
    try:
        robot_config = SO100FollowerConfig(
            port="/dev/tty.usbmodem58760434091",
    ...
    try:
        teleop_config = SO100LeaderConfig(
            port="/dev/tty.usbmodem58CD1771421"
        )
    ...
    try:
        for i in range(60):
            ...
            if i % 10 == 0:
                ...
                print(f"  Iteration {i:2d}: {total_time*1000:6.1f}ms total, "
    ...
    if timing_data["total_loop"]:
        for component, times in timing_data.items():
            ...
            if component == "total_loop":
                ...
                print(f"{component:20s}: {avg_ms:6.1f}ms avg ({actual_fps:5.1f} FPS), "
    ...
    if timing_data["total_loop"] and timing_data["robot_observation"]:
        avg_total = sum(t for t in timing_data["total_loop"] if t > 0) / len([t for t in timing_data["total_loop"] if t > 0])
        avg_obs = sum(t for t in timing_data["robot_observation"] if t > 0) / len([t for t in timing_data["robot_observation"] if t > 0])
